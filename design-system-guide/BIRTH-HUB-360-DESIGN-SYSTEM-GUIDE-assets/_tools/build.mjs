...
const val = [['aplicação executada', true, 'tsx server.ts :3024'], ['navegador utilizado', true, 'Microsoft Edge via Playwright'], ['rotas reais descobertas', ctx.routes.length > 0, `${ctx.routes.length} rotas`], ['screenshots reais capturados', nShots > 0, `${nShots} PNGs`], ['medidas DOM coletadas', ctx.routes.some((r) => ctx.shots[r][1440]?.groups), 'getBoundingClientRect + getComputedStyle'],
