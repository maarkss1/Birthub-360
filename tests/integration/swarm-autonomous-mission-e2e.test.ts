  ...
  return {
    ...
    getAiModel: () => ({
      invoke: async () => ({
        ...
        response_metadata: {
          model: 'stub-model-e2e',
          tokenUsage: { totalTokens: 420, promptTokens: 300, completionTokens: 120 },
        },
      }),
    }),
  };
...
const { runSwarmScheduler, getSwarmSloSnapshot } =
  await import('@/features/intelligence/services/swarmScheduler.service');
...
describe('Missão real do enxame, ponta a ponta (scheduler autônomo → CRM → ledger)', () => {
  ...
  it('detecta o follow-up vencido, roteia para o CRM, executa com o modelo e persiste o traço completo', async () =>
    withTenant(ORG_ID, async () => {
      ...
      const pendingActionsAfterRetry = await prisma.aIPendingAction.findMany({
        where: { organizationId: ORG_ID },
      });
