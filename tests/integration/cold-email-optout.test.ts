...
vi.mock(
  '../../src/features/prospecting/services/enrichment/domainGuess.js',
  async (importOriginal) => ({
    ...(await importOriginal<
      typeof import('../../src/features/prospecting/services/enrichment/domainGuess.js')
    >()),
...
const { sendColdEmail } =
  await import('../../src/features/prospecting/services/cold-email.service');
...
describe('cold-email.service.ts x opt-out unificado (Postgres real)', () => {
  ...
  it('opt-out restrito a scope "voice"/"whatsapp" NÃO bloqueia e-mail (regressão contra super-bloqueio)', async () => {
    ...
    const result = await asOrg(ORG_ID, () =>
      sendColdEmail(
        baseCampaign(ORG_ID, { targetEmail: 'nao-restrito-a-email@empresa-cliente.com.br' }),
      ),
    );
