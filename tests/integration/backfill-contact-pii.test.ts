...
describe('backfill-contact-pii — migra Contact legado (texto puro) para cifrado + índice cego', () => {
  it('cifra e indexa uma linha "legada" simulada, de forma idempotente, sem tocar linhas já migradas', async () => {
    ...
    const legacy = await pool.query('SELECT email, "emailIndex" FROM "Contact" WHERE id = $1', [
      contactId,
    ]);
    ...
    await requestContext.run({ tenantId: ORG }, async () => {
      const found = await prisma.contact.findFirst({
        where: { emailIndex: contactEmailIndex('ANA@legacy.com') },
      });
    ...
    runBackfill();
    const afterSecondRun = await pool.query('SELECT email FROM "Contact" WHERE id = $1', [
      contactId,
    ]);
