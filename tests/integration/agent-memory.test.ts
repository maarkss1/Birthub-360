...
async function cleanupAgentMemory() {
  await asOrg(ORG, () => prisma.agentMemory.deleteMany({ where: { organizationId: ORG } }));
}

beforeAll(async () => {
  await withRlsBypass(() =>
    prisma.organization.create({ data: { id: ORG, name: 'Test Org (agent memory)' } }),
  );
});
...
describe('agentMemory.store — upsert atômico (Postgres real)', () => {
  ...
  it('upsert sobrescreve o estado anterior (failed → completed)', async () => {
    ...
    const memory = await asOrg(ORG, () =>
      loadAgentMemory({ sessionId, agentType: 'SDR', organizationId: ORG }),
    );
    expect(memory?.status).toBe('Completed');
    expect(memory?.errorMessage).toBeNull();
    expect(memory?.messages).toEqual([
      { role: 'assistant', content: 'sucesso na segunda tentativa' },
    ]);
  ...
  it('recordAgentFailure nunca lança, mesmo se a escrita falhar', async () => {
    ...
    await expect(
      recordAgentFailure({
        sessionId: 'x',
        agentType: 'SDR',
        organizationId: 'org-inexistente-xyz',
        errorMessage: 'erro',
      }),
...
describe('GET /api/intelligence/agents/sdr/status/:sessionId — contrato de 3 estados (Postgres real)', () => {
  ...
  it('RLS: não enxerga a sessão de outra organização (continua pending do ponto de vista dela)', async () => {
    ...
    await withRlsBypass(() =>
      prisma.organization.create({ data: { id: otherOrg, name: 'Other Org' } }),
    );
    ...
    try {
      ...
      await asOrg(otherOrg, () =>
        prisma.agentMemory.deleteMany({ where: { organizationId: otherOrg } }),
      );
