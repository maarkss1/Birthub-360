...
async function createTestOrg(): Promise<string> {
  ...
  await withRlsBypass(() =>
    prisma.organization.create({ data: { id, name: `Test Org (cadence runtime ${id})` } }),
  );
  return id;
}
...
async function seedLeadWithWhatsApp(orgId: string, phone: string) {
  return asOrg(orgId, async () => {
    const company = await prisma.company.create({
      data: {
        legalName: 'Empresa Teste Cadência',
        tradeName: 'Empresa Teste Cadência',
...
afterEach(async () => {
  if (createdOrgIds.length > 0) {
    await withRlsBypass(async () => {
      await prisma.cadenceTouchAttempt.deleteMany({
...
describe('scanAndAdvanceCadenceRuns — runtime real de cadência (CYC-008/onda-19, Postgres + Redis reais)', () => {
  ...
  it('despacha o toque via WhatsApp real, grava o attempt e conclui o run (sequência de 1 toque)', async () => {
    ...
    const updatedRun = await asOrg(org, () =>
      prisma.cadenceRun.findFirstOrThrow({ where: { organizationId: org, leadId: lead.id } }),
    );
    expect(updatedRun.status).toBe('Completed');
    expect(updatedRun.stopReason).toBe('Completed');

    const attempts = await asOrg(org, () =>
      prisma.cadenceTouchAttempt.findMany({
        where: { organizationId: org, cadenceRunId: updatedRun.id },
      }),
    );
  ...
  it('CadenceSequence.touches malformado encerra o run como failed/policy-guardrail (CYC-002) — não fica preso em Active para sempre', async () => {
    ...
    const { run } = await seedActiveRun(org, lead.id, [
      { order: 1, channel: 'sms-invalido', delayHoursFromPrevious: 0 },
    ]);
  ...
  it('varredura cross-tenant: runs ativos de DUAS organizações diferentes são ambos processados, sem vazar dado entre elas', async () => {
    ...
    const runA = await asOrg(orgA, () =>
      prisma.cadenceRun.findFirstOrThrow({ where: { organizationId: orgA, leadId: leadA.id } }),
    );
    const runB = await asOrg(orgB, () =>
      prisma.cadenceRun.findFirstOrThrow({ where: { organizationId: orgB, leadId: leadB.id } }),
    );
    ...
    const runAFromOrgB = await asOrg(orgB, () =>
      prisma.cadenceRun.findFirst({ where: { id: runA.id } }),
    );
