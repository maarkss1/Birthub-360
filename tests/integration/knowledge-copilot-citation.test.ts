...
vi.mock('../../src/lib/ai/gateway', async () => {
  const actual = await vi.importActual<typeof import('../../src/lib/ai/gateway')>(
    '../../src/lib/ai/gateway',
  );
  return {
    ...actual,
...
const { KnowledgeCopilotService } =
  await import('../../src/features/knowledge/services/knowledge-copilot.service');
...
describe('AI-010: KnowledgeCopilotService cita a fonte real (Postgres/pgvector reais)', () => {
  ...
  beforeEach(async () => {
    await requestContext.run({ bypassRls: true }, async () => {
      ...
      if (!exists)
        await prisma.organization.create({ data: { id: ORG_B, name: 'Test Org Copilot B' } });
    });
    ...
    const doc = await requestContext.run({ tenantId: ORG_A }, () =>
      ingestionService.ingestText({
        ...
        content:
          'O módulo M400 suporta até 4 sondas de temperatura 1-Wire DS18B20 em barramento único com calibração digital de -55°C a +125°C.',
      }),
    );
    documentId = doc.id;
  });
