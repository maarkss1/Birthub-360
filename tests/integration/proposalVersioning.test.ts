...
beforeAll(async () => {
  setupDI();
  await withRlsBypass(() =>
    prisma.organization.create({ data: { id: ORG, name: 'Test Org (proposal versioning)' } }),
  );
});
    ...
    const updated = await asOrg(ORG, () =>
      repo.updateDocumentContent(
        ...
        doc.id,
        {
          ...
          currency: 'BRL',
          discount: 0,
          lineItems: [{ ...LINE_ITEM, quantity: 2 }],
          changeReason: 'Cliente pediu +1 unidade',
        } as never,
        'user-2',
      ),
    );
    ...
    for (const title of ['v2', 'v3', 'v4']) {
      await asOrg(ORG, () =>
        repo.updateDocumentContent(
          ORG,
          doc.id,
          { title, currency: 'BRL', discount: 0, lineItems: [LINE_ITEM] } as never,
          'user-1',
        ),
      );
    }
...
describe('CYC-005 — rastreamento real de visualização (publicToken)', () => {
  ...
  it('token inexistente devolve 404, sem lançar erro cru', async () => {
    ...
    const res = await request(app).get(
      '/api/public/proposals/00000000-0000-0000-0000-000000000000/view',
    );
  ...
  it('RLS: a rota pública enxerga e atualiza um documento de QUALQUER organização (o publicToken é a credencial, não o tenant)', async () => {
    ...
    await withRlsBypass(() =>
      prisma.organization.create({ data: { id: orgB, name: 'Outra org' } }),
    );
    try {
      const doc = await asOrg(orgB, () =>
        repo.createDocument(orgB, {
          type: 'Proposta',
          title: 'Doc de outra org',
          currency: 'BRL',
          discount: 0,
          lineItems: [LINE_ITEM],
          status: 'Rascunho',
