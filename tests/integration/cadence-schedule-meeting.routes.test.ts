...
async function seedLead(
  ...
  contactEmail: string | null = 'contato@exemplo.com',
) {
  return asOrg(organizationId, async () => {
    ...
    const contact = await prisma.contact.create({
      data: { name: 'Lead Teste', email: contactEmail, companyId: company.id, organizationId },
    });
...
beforeAll(async () => {
  await withRlsBypass(() =>
    prisma.organization.create({ data: { id: ORG, name: 'Test Org (schedule meeting)' } }),
  );
});
...
describe('POST /api/cadence/leads/:leadId/schedule-meeting (Postgres real)', () => {
  it('confirmação manual válida: cria a Note de evidência e o CadenceCalendarEvent real', async () => {
    ...
    const event = await asOrg(ORG, () =>
      prisma.cadenceCalendarEvent.findFirstOrThrow({ where: { leadId: lead.id } }),
    );
    ...
    const note = await asOrg(ORG, () =>
      prisma.note.findUniqueOrThrow({ where: { id: res.body.data.noteId } }),
    );
  ...
  it('vincula o CadenceRun ativo do lead quando existe', async () => {
    ...
    const seqRes = await request(buildApp())
      ...
      .send({
        ...
        touches: [
          { order: 1, channel: 'whatsapp', delayHoursFromPrevious: 0, templateRef: 'Olá!' },
        ],
      });
  ...
  it('422 quando o horário proposto já passou — nenhuma Note nem CadenceCalendarEvent é criado', async () => {
    ...
    const events = await asOrg(ORG, () =>
      prisma.cadenceCalendarEvent.findMany({ where: { leadId: lead.id } }),
    );
