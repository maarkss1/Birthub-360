 ...
 */
const { redactAndTrackPiiLeak } =
  await import('../../src/features/intelligence/services/guardrails.service');
const { getEvaluationMetricsSnapshot } =
  await import('../../src/features/intelligence/services/evaluationMetrics.service');
...
describe('AI-006: AIGuardrailEvent + evaluationMetrics.service (Postgres real, RLS incluída)', () => {
  beforeAll(async () => {
    await requestContext.run({ bypassRls: true }, async () => {
      ...
      if (!exists)
        await prisma.organization.create({ data: { id: ORG_B, name: 'Test Org Eval Metrics B' } });
    });
  });
  ...
  it('getEvaluationMetricsSnapshot: piiLeakageRate real = eventos de PII / total de chamadas de IA (AILog) na janela', async () => {
    ...
    if (snapshot.dimensions.piiLeakageRate.available) {
      expect(snapshot.dimensions.piiLeakageRate.value).toEqual({
        value: 1 / 4,
        numerator: 1,
        denominator: 4,
      });
    ...
    expect(snapshot.dimensions.factuality).toEqual({
      available: false,
      reason: expect.stringContaining('AI-005'),
    });
  });
});
