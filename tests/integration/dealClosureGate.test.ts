...
async function seedLead(status = 'Lead_Recebido') {
  return asOrg(ORG, async () => {
    ...
    const contact = await prisma.contact.create({
      data: { name: 'Lead Teste', companyId: company.id, organizationId: ORG },
    });
...
beforeAll(async () => {
  await withRlsBypass(() =>
    prisma.organization.create({ data: { id: ORG, name: 'Test Org (deal closure)' } }),
  );
});
...
describe('CYC-007 — LeadUseCases.updateLeadStatus (Kanban drag)', () => {
  it('humano real: move o lead e grava Note + DealClosureEvent reais', async () => {
    ...
    const updated = await asOrg(ORG, () =>
      leadUseCases.updateLeadStatus(ORG, lead.id, 'Negócios Ganhos', USER_ID),
    );
    expect((updated as { status: string }).status).toBe('Negócios Ganhos');

    const events = await asOrg(ORG, () =>
  ...
  it('sem actorUserId: rejeitado (401), lead não é movido', async () => {
    ...
    await expect(
      asOrg(ORG, () => leadUseCases.updateLeadStatus(ORG, lead.id, 'Negócios Ganhos', undefined)),
    ).rejects.toThrow();

    const untouched = await asOrg(ORG, () =>
  ...
  it('actorUserId com cara de agente de IA: rejeitado (403), lead não é movido, nenhum evento nem nota gravados', async () => {
    ...
    await expect(
      asOrg(ORG, () =>
        leadUseCases.updateLeadStatus(ORG, lead.id, 'Negócios Ganhos', 'agent:closer-swarm'),
      ),
    ).rejects.toThrow();
  ...
  it('mover para um status que NÃO é "Negócios Ganhos" nunca aciona o gate', async () => {
    ...
    await asOrg(ORG, () =>
      leadUseCases.updateLeadStatus(ORG, lead.id, 'Qualificação (SDR)', undefined),
    );
...
describe('CYC-007 — LeadUseCases.updateLead (edição completa)', () => {
  it('humano real: move o lead via update completo e grava evidência real', async () => {
    ...
    await asOrg(ORG, () =>
      leadUseCases.updateLead(ORG, lead.id, { status: 'Negócios Ganhos' } as never, USER_ID),
    );
...
describe('CYC-007 — PrismaCrm360Repository.updateLeadStage (módulo CRM360)', () => {
  it('humano real: mover para a etapa "Negócios Ganhos" grava evidência real', async () => {
    ...
    const wonStage = await asOrg(ORG, () =>
      prisma.crmPipelineStage.findFirstOrThrow({
        where: { pipeline: { organizationId: ORG }, code: 'ganho' },
      }),
    );

    const updated = await asOrg(ORG, () =>
      crm360Repository.updateLeadStage(ORG, lead.id, wonStage.id, undefined, USER_ID),
    );
    expect((updated as { status: string }).status).toBe('Negócios Ganhos');
  ...
  it('sem actorUserId: rejeitado, lead permanece na etapa original', async () => {
    ...
    await expect(
      asOrg(ORG, () =>
        crm360Repository.updateLeadStage(ORG, lead.id, wonStage.id, undefined, undefined),
      ),
    ).rejects.toThrow();
  ...
  it('mover para uma etapa que NÃO é "Negócios Ganhos" nunca aciona o gate', async () => {
    ...
    const openStage = await asOrg(ORG, () =>
      prisma.crmPipelineStage.findFirstOrThrow({
        where: { pipeline: { organizationId: ORG }, code: 'nova-oportunidade' },
      }),
    );

    await asOrg(ORG, () =>
      crm360Repository.updateLeadStage(ORG, lead.id, openStage.id, undefined, undefined),
    );
