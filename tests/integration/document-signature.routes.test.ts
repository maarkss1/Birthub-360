...
vi.mock('../../src/config/env.js', async (importOriginal) => {
  ...
  return {
    ...actual,
    env: { ...actual.env, SIGNATURE_INBOUND_WEBHOOK_SECRET: 'segredo-signature-integration-teste' },
  };
});
...
async function seedDocumentWithContact(
  organizationId: string,
  contactEmail: string | null = 'signer@exemplo.com',
) {
...
beforeAll(async () => {
  setupDI();
  await withRlsBypass(() =>
    prisma.organization.create({ data: { id: ORG, name: 'Test Org (document signature)' } }),
  );
});
...
describe('POST /api/crm/documents/:id/request-signature (Postgres real)', () => {
  ...
  it('422 quando o documento não tem contato vinculado nem signerEmail no body', async () => {
    ...
    const requests = await asOrg(ORG, () =>
      prisma.crmDocumentSignatureRequest.findMany({ where: { documentId: doc.id } }),
    );
  ...
  it('404 quando o documento não existe nesta organização', async () => {
    const res = await request(buildApp())
      .post('/api/crm/documents/doc-inexistente/request-signature')
      .send({});
...
describe('POST /api/webhooks/signature/webhook (Postgres real, fluxo completo)', () => {
  it('created → sent → viewed → signed: cada transição real é aplicada e persistida', async () => {
    ...
    const signedRes = await postWebhook({
      provider: 'govbr',
      providerRequestId,
      status: 'signed',
      evidenceRef: 'cert-abc-123',
    });
    expect(signedRes.body).toEqual({ success: true, outcome: 'applied' });

    const persisted = await asOrg(ORG, () =>
      prisma.crmDocumentSignatureRequest.findUniqueOrThrow({
    ...
    expect(persisted.rawWebhookPayload).toMatchObject({
      status: 'signed',
      evidenceRef: 'cert-abc-123',
    });
  });
