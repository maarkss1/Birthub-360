...
describe('AILog: contrato RLS da Onda 2.5', () => {
  ...
  it('bloqueia tenant que tenta fabricar telemetria não atribuída', async () => {
    await expect(
      requestContext.run({ tenantId: ORG_A }, () => createLog(null, 'onda-2.5-ailog-tenant-null')),
    ).rejects.toThrow();
  });

  it('bloqueia Prisma create NULL sem o corredor interno sem RETURNING', async () => {
    await expect(createLog(null, 'onda-2.5-ailog-null-without-internal-path')).rejects.toThrow();
  });
