...
import {
  isSuppressed,
  recordOptOut as recordVoiceOptOut,
} from '../../src/features/integrations/birth-voice/callSuppression.service';
import { prismaOptOutRepository } from '../../src/features/cadence/infra/PrismaOptOutRepository';
import {
  isOptedOut,
  recordOptOut as recordUnifiedOptOut,
} from '../../src/features/cadence/application/optOutService';
    ...
    const [suppressionRow, optOutRow, blocksVoiceDirect, blocksEmail] = await asOrg(ORG_A, () =>
      Promise.all([
        prisma.callSuppression.findUnique({
          where: {
            organizationId_phoneE164: { organizationId: ORG_A, phoneE164: '+5511922221111' },
          },
        }),
