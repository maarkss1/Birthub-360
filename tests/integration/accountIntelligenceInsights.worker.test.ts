...
describe('scanAndGenerateAccountInsights — score e Next Best Action reais, contra Postgres', () => {
  ...
  it('roda duas vezes sem duplicar AccountScore nem AccountRecommendation (idempotente)', async () => {
    ...
    const companyId = await asOrg(orgId, async () => {
      const company = await prisma.company.create({
        data: {
          legalName: 'Conta Idempotente Ltda.',
          tradeName: 'Conta Idempotente',
          organizationId: orgId,
          lookalikeScore: 40,
        },
      });
  ...
  it('nunca escreve fora do tenant dono da conta', async () => {
    ...
    const companyId = await asOrg(orgA, async () => {
      const company = await prisma.company.create({
        data: {
          legalName: 'Conta Isolada Ltda.',
          tradeName: 'Conta Isolada',
          organizationId: orgA,
          lookalikeScore: 55,
        },
      });
  ...
  it('D.3: classifica um DecisionMaker real a partir do Contact, nunca reclassifica um já existente', async () => {
    ...
    const { companyId, contactId, preExistingDecisionMakerId } = await asOrg(orgId, async () => {
      const company = await prisma.company.create({
        data: {
          legalName: 'Conta com Decisores Ltda.',
          tradeName: 'Conta Decisores',
          organizationId: orgId,
        },
      });
      await prisma.accountIntelligenceSnapshot.create({
      ...
      return {
        companyId: company.id,
        contactId: classifiableContact.id,
        preExistingDecisionMakerId: preExisting.id,
      };
    });
    ...
    await asOrg(orgId, async () => {
      const generated = await prisma.decisionMaker.findUnique({
        where: {
          organizationId_companyId_contactId: { organizationId: orgId, companyId, contactId },
        },
      });
      ...
      const untouched = await prisma.decisionMaker.findUnique({
        where: { id: preExistingDecisionMakerId },
      });
      ...
      const total = await prisma.decisionMaker.count({
        where: { organizationId: orgId, companyId },
      });
      expect(total).toBe(2);
    });
  ...
  it('D.3: não cria DecisionMaker para Contact sem cargo, senioridade ou departamento reconhecível', async () => {
    ...
    const companyId = await asOrg(orgId, async () => {
      const company = await prisma.company.create({
        data: {
          legalName: 'Conta Sem Sinal de Decisor Ltda.',
          tradeName: 'Conta Sem Sinal',
          organizationId: orgId,
        },
      });
      await prisma.accountIntelligenceSnapshot.create({
    ...
    await asOrg(orgId, async () => {
      const count = await prisma.decisionMaker.count({
        where: { organizationId: orgId, companyId },
      });
      expect(count).toBe(0);
    });
  ...
  it('D.4: liga duas contas do mesmo tenant com a mesma raiz de CNPJ como MATRIZ_FILIAL, idempotente', async () => {
    ...
    await asOrg(orgId, async () => {
      const relationships = await prisma.economicRelationship.findMany({
  ...
  it('D.4: nunca liga empresas com a mesma raiz de CNPJ em organizações diferentes', async () => {
    ...
    await asOrg(orgA, () =>
      prisma.company.create({
        data: {
          legalName: 'Empresa Org A Ltda.',
          tradeName: 'Empresa Org A',
          organizationId: orgA,
          cnpj: `${cnpjRootDigits}000191`,
        },
      }),
    ...
    await asOrg(orgB, () =>
      prisma.company.create({
        data: {
          legalName: 'Empresa Org B Ltda.',
          tradeName: 'Empresa Org B',
          organizationId: orgB,
          cnpj: `${cnpjRootDigits}000272`,
        },
      }),
