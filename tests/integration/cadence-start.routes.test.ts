...
async function seedLead(organizationId: string) {
  return asOrg(organizationId, async () => {
    const company = await prisma.company.create({
      data: { legalName: 'Empresa Teste', tradeName: 'Empresa Teste', organizationId },
    });
    const contact = await prisma.contact.create({
      data: { name: 'Lead Teste', companyId: company.id, organizationId },
    });
    return prisma.lead.create({
      data: { companyId: company.id, contactId: contact.id, organizationId },
    });
  });
}
...
beforeAll(async () => {
  await withRlsBypass(() =>
    prisma.organization.create({ data: { id: ORG, name: 'Test Org (cadence start)' } }),
  );
});
...
const VALID_TOUCHES = [
  { order: 1, channel: 'whatsapp', delayHoursFromPrevious: 0, templateRef: 'Olá! Tudo bem?' },
];
...
describe('POST /api/cadence/sequences (Postgres real)', () => {
  it('cria uma sequência válida', async () => {
    ...
    const row = await asOrg(ORG, () =>
      prisma.cadenceSequence.findUniqueOrThrow({ where: { id: res.body.data.id } }),
    );
  ...
  it('rejeita sequência com ordem de toques com lacuna (validateSequence)', async () => {
    const res = await request(buildApp())
      .post('/api/cadence/sequences')
      .send({
        name: 'Sequência inválida',
        touches: [{ order: 2, channel: 'email', delayHoursFromPrevious: 0, templateRef: 'x' }],
      });
...
describe('POST /api/cadence/runs (Postgres real)', () => {
  it('inicia um run real para um lead existente', async () => {
    ...
    const row = await asOrg(ORG, () =>
      prisma.cadenceRun.findUniqueOrThrow({ where: { id: res.body.data.id } }),
    );
  ...
  it('RLS: não é possível iniciar run para lead de outra organização', async () => {
    ...
    await withRlsBypass(() =>
      prisma.organization.create({ data: { id: otherOrg, name: 'Outra org' } }),
    );
    try {
...
describe('GET /api/cadence/sequences (Postgres real)', () => {
  it('lista só as sequências ativas da própria organização', async () => {
    await request(buildApp())
      .post('/api/cadence/sequences')
      .send({ name: 'Seq A', touches: VALID_TOUCHES });
    await request(buildApp())
      .post('/api/cadence/sequences')
      .send({ name: 'Seq B', touches: VALID_TOUCHES });
...
describe('POST /api/cadence/runs/:id/pause|resume|stop (Postgres real)', () => {
  it('pausa um run ativo — status muda para paused e pausedAt é gravado', async () => {
    ...
    const row = await asOrg(ORG, () =>
      prisma.cadenceRun.findUniqueOrThrow({ where: { id: run.id } }),
    );
  ...
  it('RLS: não é possível pausar um run de outra organização', async () => {
    ...
    try {
      const leadOther = await seedLead(otherOrg);
      const seqOtherRes = await request(buildApp('GESTOR', otherOrg))
        .post('/api/cadence/sequences')
        .send({ name: 'Sequência outra org', touches: VALID_TOUCHES });
      const runOtherRes = await request(buildApp('GESTOR', otherOrg))
        .post('/api/cadence/runs')
        .send({ leadId: leadOther.id, sequenceId: seqOtherRes.body.data.id });
      expect(runOtherRes.status).toBe(201);

      const res = await request(buildApp()).post(
        `/api/cadence/runs/${runOtherRes.body.data.id}/pause`,
      );
      expect(res.status).toBe(404);
    } finally {
      await withRlsBypass(async () => {
  ...
  it('VISUALIZADOR não pode pausar/retomar/parar (requireRole)', async () => {
    ...
    const pauseRes = await request(buildApp('VISUALIZADOR')).post(
      `/api/cadence/runs/${run.id}/pause`,
    );
