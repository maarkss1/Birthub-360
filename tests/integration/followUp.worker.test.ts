...
async function createTestOrg(): Promise<string> {
  ...
  await withRlsBypass(() =>
    prisma.organization.create({ data: { id, name: `Test Org (follow-up ${id})` } }),
  );
  return id;
}
...
async function seedEligibleLead(
  orgId: string,
  phone: string,
  overrides: { status?: string; lastInteraction?: Date; customFields?: object } = {},
) {
  return asOrg(orgId, async () => {
    const company = await prisma.company.create({
      data: {
        legalName: 'Empresa Teste Follow-up',
        tradeName: 'Empresa Teste Follow-up',
    ...
    const lead = await prisma.lead.create({
      data: {
        ...
        lastInteraction:
          overrides.lastInteraction ?? new Date(Date.now() - 2 * 24 * 60 * 60 * 1000),
...
describe('runDailyFollowUpScan — job real de follow-up de WhatsApp (Postgres + Redis reais)', () => {
  ...
  it('varredura cross-tenant real: leads de DUAS organizações diferentes são ambos encontrados e mensageados', async () => {
    ...
    const updatedA = await asOrg(orgA, () =>
      prisma.lead.findUniqueOrThrow({ where: { id: leadA.id } }),
    );
    const updatedB = await asOrg(orgB, () =>
