        ...
        .send({
          ...
          context: {
            ...
            recentLogs: [
              {
                level: 'error',
                message: 'fetch falhou com Bearer abc123.def456-ghi_789',
  ...
  describe('GET /api/bug-reports', () => {
    ...
    it('isolamento de tenant: ADMIN da Org B nunca vê relatos da Org A', async () => {
      await request(app)
        ...
        .set('Cookie', viewerA.cookie)
        .send({
          title: 'Relato exclusivo da Org A',
          description: 'Não deve vazar para outra organização.',
        });
      ...
      expect(
        resB.body.data.every(
          (r: { organizationId: string }) => r.organizationId === adminB.organizationId,
        ),
      ).toBe(true);
      expect(
        resB.body.data.some((r: { title: string }) => r.title === 'Relato exclusivo da Org A'),
      ).toBe(false);
    });
  });
