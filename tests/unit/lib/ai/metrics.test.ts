...
describe('src/lib/ai/metrics.ts', () => {
  ...
  it('records estimated AI cost labeled by provider and tenant', async () => {
    ...
    const groqOrg1 = values?.find(
      (v) => v.labels.provider === 'groq' && v.labels.tenant === 'org-1',
    );
    const openaiOrg2 = values?.find(
      (v) => v.labels.provider === 'openai' && v.labels.tenant === 'org-2',
    );
  ...
  it('falls back to "unattributed" tenant outside an HTTP request (worker/script calls)', async () => {
    ...
    const unattributed = values?.find(
      (v) => v.labels.provider === 'groq' && v.labels.tenant === 'unattributed',
    );
  ...
  it('publishes ai_usage_budget_usd_total only when AI_MONTHLY_BUDGET_USD is configured', async () => {
    ...
    expect(
      (budget as unknown as { values: Array<{ value: number }> } | undefined)?.values[0]?.value,
    ).toBe(500);
  });
