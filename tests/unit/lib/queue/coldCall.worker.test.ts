...
vi.mock('bullmq', () => {
  class MockQueue {
    upsertJobScheduler(...args: unknown[]) {
      return upsertJobScheduler(...args);
    }
    getJobSchedulers(...args: unknown[]) {
      return getJobSchedulers(...args);
    }
    removeJobScheduler(...args: unknown[]) {
      return removeJobScheduler(...args);
    }
    on(...args: unknown[]) {
      return queueOn(...args);
    }
...
const { scheduleColdCallCampaigns, createColdCallWorker, COLD_CALL_QUEUE_NAME } =
  await import('../../../../src/lib/queue/coldCall.worker');
