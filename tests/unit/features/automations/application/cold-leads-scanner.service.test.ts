...
const { runColdLeadsScan, createColdLeadsScannerWorker, COLD_LEADS_SCANNER_QUEUE_NAME } =
  await import('../../../../../src/features/automations/application/cold-leads-scanner.service.js');
  ...
  it('processa cada organização habilitada isoladamente e soma o total varrido', async () => {
    ...
    leadFindMany
      .mockResolvedValueOnce([
        { id: 'lead-1', company: { segment: 'X', size: 'Y', tradeName: 'Z' } },
      ])
      .mockResolvedValueOnce([
        { id: 'lead-2', company: null },
        { id: 'lead-3', company: null },
      ]);
...
describe('createColdLeadsScannerWorker — dead-letter na última tentativa', () => {
  ...
  it('não registra dead-letter quando ainda restam tentativas', () => {
    ...
    failedHandler(
      { id: 'job-1', name: 'daily-cold-leads-scan', attemptsMade: 1, opts: { attempts: 3 } },
