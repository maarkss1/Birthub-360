...
const acquireDistributedLockMock = vi.fn(async () => ({
  runId: 'run-1',
...
const { runStagnationScan, createStagnationScannerWorker, STAGNATION_SCANNER_QUEUE_NAME } =
  await import('../../../../../src/features/automations/application/stagnation-scanner.service.js');
...
describe('runStagnationScan', () => {
  it('pula a execução inteira quando outra instância já detém a trava distribuída', async () => {
    ...
    expect(result).toEqual(
      expect.objectContaining({ automationsEvaluated: 0, leadsScanned: 0, fired: 0 }),
    );
    ...
    automationFindMany.mockResolvedValue([
      {
        ...
        conditions: { status: 'Proposta Enviada', daysSinceLastInteraction: { gte: 3 } },
      },
    ]);
    ...
    automationFindMany.mockResolvedValue([
      {
        id: 'auto-1',
        name: 'Negócio parado',
        conditions: { daysSinceLastInteraction: { gte: 5 } },
      },
    ]);
  ...
  it('uma falha ao reavaliar uma automação não impede as demais automações/organizações', async () => {
    ...
    leadFindMany.mockRejectedValueOnce(new Error('banco fora do ar')).mockResolvedValueOnce([]);
...
describe('createStagnationScannerWorker — dead-letter na última tentativa', () => {
  ...
  it('registra recordDeadLetter com os campos do job quando a falha é a última tentativa', () => {
    ...
    expect(recordDeadLetterMock).toHaveBeenCalledWith(
      expect.objectContaining({
  ...
  it('não registra dead-letter quando ainda restam tentativas', () => {
    ...
    failedHandler(
      { id: 'job-1', name: 'daily-stagnation-scan', attemptsMade: 1, opts: { attempts: 3 } },
