...
vi.mock('../../../../../../src/config/env.js', () => ({
  ...
  get env() {
    return {
      BITRIX_EXTRACTION_PURGE_ENABLED: purgeEnabled,
      BITRIX_EXTRACTION_RETENTION_DAYS: retentionDays,
    };
  },
}));
...
import { runBitrixExtractionPurgeSweep } from '../../../../../../src/features/integrations/bitrix/jobs/bitrixExtractionPurge.worker';
...
describe('runBitrixExtractionPurgeSweep — janela de retenção', () => {
  ...
  it('filtra por createdAt <= cutoff derivado de BITRIX_EXTRACTION_RETENTION_DAYS e só por status terminal', async () => {
    ...
    const [args] = bitrixExtractionRunFindMany.mock.calls[0] as [
...
describe('runBitrixExtractionPurgeSweep — anonimização (DEC-04 opção B: nunca hard delete)', () => {
  ...
  it('remove os arquivos em disco e redige filters.search, mas preserva id/status/contadores/entities/requestedBy', async () => {
    ...
    bitrixExtractionRunFindMany.mockResolvedValue([
      {
        ...
        progress: {
          entities: [{ entity: 'lead', status: 'done', checkpointTo: '2026-01-01T00:00:00.000Z' }],
        },
      },
    ]);
    ...
    const [updateArgs] = bitrixExtractionRunUpdate.mock.calls[0] as [
    ...
    expect(progress.entities).toEqual([
      { entity: 'lead', status: 'done', checkpointTo: '2026-01-01T00:00:00.000Z' },
    ]);
  ...
  it('arquivo é removido ANTES da linha ser marcada como expurgada (nunca a linha sozinha)', async () => {
    ...
    deleteExtractionRunFiles.mockImplementation(async () => {
      callOrder.push('deleteFiles');
    });
    bitrixExtractionRunUpdate.mockImplementation(async () => {
...
describe('runBitrixExtractionPurgeSweep — idempotência', () => {
  ...
  it('rodar a varredura duas vezes seguidas com o mesmo lote não duplica efeito nem falha', async () => {
    ...
    bitrixExtractionRunFindMany.mockResolvedValueOnce([
      { id: 'run-1', filters: { search: 'x' }, progress: null },
    ]);
    ...
    const purgedProgress = (
      bitrixExtractionRunUpdate.mock.calls[0][0] as { data: { progress: unknown } }
    ).data.progress;
    bitrixExtractionRunFindMany.mockResolvedValueOnce([
      { id: 'run-1', filters: {}, progress: purgedProgress },
    ]);
