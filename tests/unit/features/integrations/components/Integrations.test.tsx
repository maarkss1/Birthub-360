...
function mockAllIntegrationEndpoints() {
  server.use(
    http.get('/api/whatsapp/status', () =>
      HttpResponse.json({ success: true, data: { status: 'disconnected', qr: null } }),
    ),
    http.get('/api/google/status', () =>
      HttpResponse.json({ success: true, data: { connected: false, email: null } }),
    ),
    http.get('/api/bitrix/connections', () => HttpResponse.json({ success: true, data: [] })),
    http.get('/api/integrations/3cx/connections', () =>
...
describe('Integrations — tela de Integrações não quebra ao montar (estado/imports)', () => {
  it(
    'renderiza as 4 abas sem lançar erro (WhatsApp, Google, Bitrix24, 3CX)',
    async () => {
      render(<Integrations />);
  ...
  it(
    'troca de aba sem lançar erro e mostra o conteúdo de cada uma (activeTab via useState)',
    async () => {
      ...
      expect(
        await screen.findByText(
          'Gmail em modo leitura; Calendar tem leitura e escrita real (via Cadência); a Agenda do produto continua local.',
        ),
      ).toBeInTheDocument();

      await user.click(screen.getByText('PABX 3CX')).catch(() => {});
      expect(
        await screen.findByText(/Cadastro de PABX 3CX e teste de comunicação/),
      ).toBeInTheDocument();
    },
    TEST_TIMEOUT_MS,
  );
  ...
  it(
    'mostra estado desconectado quando os endpoints de status falham (sem tela em branco)',
    async () => {
      server.use(
        http.get('/api/whatsapp/status', () =>
        ...
        http.get('/api/bitrix/connections', () =>
          HttpResponse.json({ success: false, error: 'boom' }, { status: 500 }),
        ),
        http.get('/api/integrations/3cx/connections', () =>
          HttpResponse.json({ success: false, error: 'boom' }, { status: 500 }),
  ...
  it(
    'declara maturidade real por integração — escrita real no Google Calendar (CYC-004) com prova, sem prometer 3CX 24h sem prova',
    async () => {
      const user = userEvent.setup();
      server.use(
        http.get('/api/google/status', () =>
          HttpResponse.json({
            success: true,
            data: { connected: true, email: 'agenda@example.com', hasCalendarWriteScope: true },
          }),
        ),
        http.get('/api/google/calendar/upcoming', () =>
          HttpResponse.json({ success: true, data: [] }),
        ),
        http.get('/api/bitrix/connections', () =>
          HttpResponse.json({
      ...
      expect(
        screen.getByText(/não prova, sozinha, gravação de chamadas ou prospecção 24h/i),
      ).toBeInTheDocument();
    },
    TEST_TIMEOUT_MS,
  );
  ...
  it('avisa (sem prometer escrita) quando a conexão Google é anterior ao escopo calendar.events', async () => {
    ...
    server.use(
      http.get('/api/google/status', () =>
        HttpResponse.json({
          success: true,
          data: { connected: true, email: 'agenda@example.com', hasCalendarWriteScope: false },
        }),
      ),
      http.get('/api/google/calendar/upcoming', () =>
        HttpResponse.json({ success: true, data: [] }),
  ...
  it(
    'VISUALIZADOR vê o status mas não consegue conectar (achado do inventário de navegação da Onda 1)',
    async () => {
