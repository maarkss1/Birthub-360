...
// `claimWebhookDelivery` (dedupe de entrega, Task 5) — inclusive os próprios testes de idempotência
// abaixo, que reenviam o mesmo corpo de propósito para testar a idempotência de NEGÓCIO
// (Activity/TimelineEvent), não a dedupe de transporte deste guard. Nunca visto localmente porque
// sem `REDIS_URL` o guard sempre faz fail-open ('unavailable'). A dedupe de entrega em si já tem
// sua própria cobertura em tests/unit/shared/security/webhookReplayGuard.test.ts.
...
const { birthVoiceWebhookRoutes } =
  await import('../../../../../src/features/integrations/birth-voice/birthVoice.webhook');
...
function sign(body: string, secret = 'segredo-hub-teste'): string {
  return createHmac('sha256', body ? secret : secret)
    .update(body)
    .digest('hex');
}
...
describe('POST /api/integrations/birth-voice/webhook', () => {
  ...
  describe('estado honesto do resultado (mapa provedor → Activity.status)', () => {
    it.each([
      ...
      'outcome do provedor "%s" (duração %ss) vira Activity.status "%s"',
      async (providerOutcome, duration, expectedStatus) => {
        const body = JSON.stringify(
          endedEvent({ outcome: providerOutcome, durationSeconds: duration, transcript: [] }),
        );
      ...
      const body = JSON.stringify(
        endedEvent({
          ...
          transcript: [
            {
              role: 'user',
              content: 'Você atingiu a caixa postal, deixe seu recado após o sinal.',
            },
          ],
        }),
      );
  ...
  describe('fallback WhatsApp', () => {
    it('dispara o fallback quando a ligação não resultou em conversa', async () => {
      const body = JSON.stringify(
        endedEvent({ outcome: 'Não atendida', durationSeconds: 0, transcript: [] }),
      );
