...
const { initWhatsApp, getWhatsAppStatus, logoutWhatsApp, sendWhatsAppMessage } =
  await import('../../../../../src/features/integrations/whatsapp/whatsapp.service');
...
describe('WhatsApp service — sessão por tenant', () => {
  ...
  it('sendWhatsAppMessage envia pro número formatado quando conectado', async () => {
    ...
    expect(mockSocket.sendMessage).toHaveBeenCalledWith('fake-jid@s.whatsapp.net', {
      text: 'Olá!',
    });
  });
