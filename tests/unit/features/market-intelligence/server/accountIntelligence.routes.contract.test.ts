...
describe('API LDR — autenticação, autorização e origem do tenant', () => {
  it('nega sessão ausente antes de consultar qualquer dado LDR', async () => {
    const response = await request(buildApp({ authenticatedAs: null })).get(
      '/api/market-intelligence/accounts/company-a/intelligence',
    );
  ...
  it('responde o mesmo 404 seguro para empresa inexistente', async () => {
    ...
    const response = await request(buildApp()).get(
      '/api/market-intelligence/accounts/company-missing/intelligence',
    );
  ...
  it('permite leitura a VISUALIZADOR, mas bloqueia refresh para papel abaixo de SDR', async () => {
    const readResponse = await request(
      buildApp({ authenticatedAs: session('org-a', 'VISUALIZADOR') }),
    ).get('/api/market-intelligence/accounts/company-a/intelligence');
  ...
  it.each<Role>(['SDR', 'CLOSER', 'GESTOR', 'ADMIN'])('%s pode solicitar refresh', async (role) => {
    const response = await request(buildApp({ authenticatedAs: session('org-a', role) })).post(
      '/api/market-intelligence/accounts/company-a/refresh',
    );
...
describe('API LDR — paginação das coleções', () => {
  it.each(collectionCases)(
    '$path devolve uma coleção paginada e converte query para números',
    async ({ path, method }) => {
      const result = {
      ...
      const response = await request(buildApp()).get(
        `/api/market-intelligence/accounts/company-a/${path}?page=2&limit=25`,
      );
  ...
  it.each(collectionCases)(
    '$path rejeita page menor que 1 sem consultar o service',
    async ({ path, method }) => {
      const response = await request(buildApp()).get(
        `/api/market-intelligence/accounts/company-a/${path}?page=0&limit=20`,
      );
  ...
  it.each(collectionCases)(
    '$path rejeita limit acima de 100 sem consultar o service',
    async ({ path, method }) => {
      const response = await request(buildApp()).get(
        `/api/market-intelligence/accounts/company-a/${path}?page=1&limit=101`,
      );
  ...
  it.each(collectionCases)(
    '$path rejeita paginação não numérica sem consultar o service',
    async ({ path, method }) => {
      const response = await request(buildApp()).get(
        `/api/market-intelligence/accounts/company-a/${path}?page=abc&limit=vinte`,
      );
...
describe('API LDR — refresh idempotente e ausência de dados fabricados', () => {
  it('retorna 201 somente quando um snapshot novo foi criado', async () => {
    ...
    const response = await request(buildApp({ authenticatedAs: session('org-a', 'SDR') })).post(
      '/api/market-intelligence/accounts/company-a/refresh',
    );
  ...
  it('retorna 200 e reutiliza o mesmo snapshot quando o refresh é idempotente', async () => {
    ...
    const first = await request(buildApp({ authenticatedAs: session('org-a', 'SDR') })).post(
      '/api/market-intelligence/accounts/company-a/refresh',
    );
    const second = await request(buildApp({ authenticatedAs: session('org-a', 'SDR') })).post(
      '/api/market-intelligence/accounts/company-a/refresh',
    );
...
describe('API LDR — proveniência e sanitização', () => {
  it('mantém FACT, INFERENCE e RECOMMENDATION distinguíveis na coleção de evidências', async () => {
    ...
    const response = await request(buildApp()).get(
      '/api/market-intelligence/accounts/company-a/evidence',
    );

    expect(response.status).toBe(200);
    expect(
      response.body.data.items.map((item: { evidenceType: string }) => item.evidenceType),
  ...
  it.each([
    ...
    const response = await request(buildApp()).get(
      `/api/market-intelligence/accounts/company-a/${path}`,
    );
