...
describe('gamification routes', () => {
  ...
  it('ignora um role inválido enviado no body em vez de repassá-lo à IA', async () => {
    const app = buildApp();
    await request(app).post('/api/gamification/coaching/weekly').send({ role: 'papel-inventado' });
  ...
  it('aceita um role válido enviado no body', async () => {
    const app = buildApp();
    await request(app).post('/api/gamification/coaching/weekly').send({ role: 'SDR / Hunter' });
