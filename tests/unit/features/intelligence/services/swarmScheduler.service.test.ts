...
const { enabledOrganizations, runSwarmScheduler } =
  await import('../../../../../src/features/intelligence/services/swarmScheduler.service');
...
describe('runSwarmScheduler', () => {
  ...
  it('analisa follow-up vencido em modo read-only e registra decisão auditável', async () => {
      ...
      .mockResolvedValueOnce([
        {
          ...
          score: 66,
          nextAction: new Date('2026-08-01T00:00:00Z'),
          contact: { email: null },
        },
      ])
      .mockResolvedValueOnce([]);
    ...
    expect(runAutonomyRole).toHaveBeenCalledWith(
      'CRM',
  ...
  it('prioriza sinal de conversa e aciona o Closer em uma proposta', async () => {
      ...
      .mockResolvedValueOnce([
        {
          ...
          score: 91,
          nextAction: new Date('2026-08-01T00:00:00Z'),
          contact: { email: 'buyer@empresa.com' },
        },
      ])
      .mockResolvedValueOnce([]);
    signalFindMany.mockResolvedValueOnce([
      {
        ...
        urgency: 'alta',
        summary: 'Comprador pediu condição final.',
        confidence: 0.93,
    ...
    expect(runAutonomyRole).toHaveBeenCalledWith(
      'CLOSER',
      expect.stringContaining('objeção'),
      expect.any(String),
    );
    expect(pendingActionCreate).toHaveBeenCalledWith({
      data: expect.objectContaining({
  ...
  it('liga o produtor do SDR outbound e libera autoexecução apenas com score e janela válidos', async () => {
    ...
    leadFindMany.mockResolvedValueOnce([]).mockResolvedValueOnce([
      {
        id: 'lead-hot',
      ...
      .mockResolvedValueOnce([
        {
          ...
          status: 'Qualificacao_SDR',
          score: 50,
          nextAction: new Date('2026-08-01'),
          contact: { email: null },
        },
        {
          id: 'lead-2',
