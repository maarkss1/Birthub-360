...
describe('getSwarmSloSnapshot — painel de SLO por agente', () => {
  ...
  it('OPS agora produz AIPendingAction como os demais papéis (GOV-13) — cobertura/conversão calculadas do ledger, não mais um estado vazio fixo', async () => {
    pendingActionFindMany.mockResolvedValue([
      // OPS: 2 propostas (create_follow_up/notify_team), 1 executada após aprovação humana.
      {
        ...
        discardedAt: null,
        executed: true,
        executedAt: new Date('2026-08-10T10:05:00Z'),
        executionError: null,
        attempts: 1,
  ...
  it('com dados reais, calcula cobertura/conversão/override/erro por papel a partir de AIPendingAction', async () => {
    pendingActionFindMany.mockResolvedValue([
      ...
      },
      {
        agentRole: 'SDR',
        approved: false,
        discardedAt: new Date('2026-08-10T11:00:00Z'),
        executed: false,
        executedAt: null,
        executionError: null,
        attempts: 0,
      ...
      // CRM: 1 recomendação, tentativa de execução falhou.
      {
        ...
        executed: false,
        executedAt: null,
        executionError: 'SMTP indisponível',
        attempts: 1,
  ...
  it('isola por organização e por janela de tempo ao consultar o banco', async () => {
    ...
    expect(pendingActionFindMany).toHaveBeenCalledWith(
    ...
    expect(aiLogAggregate).toHaveBeenCalledWith(
      expect.objectContaining({
