...
describe('GET /api/agent/swarm/learn/history (GOV-13/onda-39)', () => {
  ...
  it('tenant/ator vêm sempre de req.user, nunca de querystring', async () => {
    getLearningProfileHistoryMock.mockResolvedValue({
      activeVersion: 0,
      lastAuditLogAt: null,
      versions: [],
    });

    await request(buildApp({ id: 'user-real', organizationId: 'org-real', role: 'ADMIN' })).get(
      '/api/agent/swarm/learn/history?organizationId=org-outra-tentativa&actorId=outro-user',
    );
...
describe('POST /api/agent/swarm/learn/rollback (GOV-13/onda-39)', () => {
  it('reverte para uma versão existente e devolve 200', async () => {
    rollbackLearningProfileMock.mockResolvedValue({
      success: true,
      activeVersion: 1,
      guidelines: 'Estilo A',
    });
    ...
    const res = await request(buildApp())
      .post('/api/agent/swarm/learn/rollback')
      .send({ targetVersion: 1 });

    expect(res.status).toBe(200);
    expect(res.body).toEqual({
      success: true,
      data: { success: true, activeVersion: 1, guidelines: 'Estilo A' },
    });
  ...
  it('versão inexistente devolve 404 com o motivo, sem lançar', async () => {
    rollbackLearningProfileMock.mockResolvedValue({
      success: false,
      reason: 'Versão 9 não encontrada no histórico deste perfil.',
    });

    const res = await request(buildApp())
      .post('/api/agent/swarm/learn/rollback')
      .send({ targetVersion: 9 });

    expect(res.status).toBe(404);
    expect(res.body).toEqual({
      success: false,
      error: 'Versão 9 não encontrada no histórico deste perfil.',
    });
  });
