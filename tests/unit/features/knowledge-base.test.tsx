...
describe('Base de Conhecimento', () => {
  ...
  it('não chama a API com busca curta demais', async () => {
    ...
    server.use(
      http.post(`${KNOWLEDGE_URL}/search`, () => {
        ...
        return HttpResponse.json({
          success: true,
          data: { query: '', semanticAvailable: true, hits: [] },
        });
      }),
    );
  ...
  it('indexa texto colado e recarrega a lista', async () => {
    ...
    fireEvent.change(screen.getByPlaceholderText(/Cole aqui o conteúdo/), {
      target: { value: 'Frota até 50 veículos: R$ 90/mês.' },
    });
    await user.click(screen.getByRole('button', { name: 'Indexar' }));

    await waitFor(() =>
      expect(ingestCalledWith).toEqual({
