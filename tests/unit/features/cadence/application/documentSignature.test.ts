...
import {
  applySignatureStatusUpdate,
  requestDocumentSignature,
} from '@/features/cadence/application/documentSignature';
import type {
...
function buildRepository(
  overrides: Partial<SignatureRequestRepositoryPort> = {},
): SignatureRequestRepositoryPort {
  return {
...
describe('applySignatureStatusUpdate', () => {
  ...
  it('transição inválida (webhook fora de ordem/reentregue): não aplica, devolve invalid-transition', async () => {
    const repository = buildRepository({
      findByProviderRequestId: vi.fn(async () => ({
        id: 'request-1',
        organizationId: 'org-1',
        status: 'signed',
      })),
