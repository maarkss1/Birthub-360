...
function buildPorts(
  overrides: {
    notes?: Partial<MeetingConfirmationNotePort>;
    scheduler?: Partial<CalendarSchedulerPort>;
  } = {},
) {
  return {
    notes: {
...
describe('scheduleVerifiedMeeting', () => {
  ...
  it('horário no passado: rejeita e NUNCA cria nota nem chama o scheduler', async () => {
    ...
    const result = await scheduleVerifiedMeeting(
      ...
      {
        ...baseInput,
        proposedStart: new Date('2020-01-01T00:00:00Z'),
        proposedEnd: new Date('2020-01-01T01:00:00Z'),
      },
      NOW,
    );
