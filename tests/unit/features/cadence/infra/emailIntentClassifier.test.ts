...
const { emailIntentClassifier } =
  await import('../../../../../src/features/cadence/infra/emailIntentClassifier');
...
describe('emailIntentClassifier', () => {
  it('extrai o sinal do JSON devolvido pelo modelo', async () => {
    ...
    const result = await emailIntentClassifier.classify(
      'Cliente: Fechado, pode mandar o contrato.',
    );
  ...
  it('descarta um intent fora do vocabulário permitido em vez de propagar lixo', async () => {
    invoke.mockResolvedValueOnce(
      aiResponse(JSON.stringify({ intent: 'inventado_pelo_modelo', urgency: 'urgentissimo' })),
    );
