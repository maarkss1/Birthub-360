...
const { prismaConversationSignalPort } =
  await import('../../../../../src/features/cadence/infra/PrismaConversationSignalPort');
...
describe('prismaConversationSignalPort', () => {
  ...
  it('registra o timeline quando há summary', async () => {
    ...
    expect(timelineEventCreate).toHaveBeenCalledWith({
      data: {
        type: 'email',
        description: 'Sinal de conversa (IA, e-mail): Cliente confirmou fechamento.',
        leadId: 'lead-1',
      },
    });
  });
