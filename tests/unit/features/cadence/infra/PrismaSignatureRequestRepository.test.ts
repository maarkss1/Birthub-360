...
const { prismaSignatureRequestRepository } =
  await import('../../../../../src/features/cadence/infra/PrismaSignatureRequestRepository');
...
describe('prismaSignatureRequestRepository', () => {
  ...
  it('updateStatus: grava status/respondedAt/evidenceRef/rawWebhookPayload, escopado pelo tenant resolvido', async () => {
    await prismaSignatureRequestRepository.updateStatus({
      id: 'request-1',
      organizationId: 'org-1',
      status: 'signed',
      evidenceRef: 'cert-123',
      rawWebhookPayload: { a: 1 },
    });
