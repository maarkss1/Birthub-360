    ...
    attempts: [
      {
        ...
        attemptNumber: 1,
        channel: 'email',
        attemptedAt: '2026-08-01T10:05:00Z',
        result: 'sent',
        error: null,
      },
    ],
...
describe('CadenceHub', () => {
  it('renderiza o título da tela e busca as três seções ao montar', async () => {
    ...
    expect(
      screen.getByRole('heading', { name: /Cadência & Ciclo de Receita/i }),
    ).toBeInTheDocument();
    await waitFor(() => expect(getMock).toHaveBeenCalledWith('/api/cadence/opt-outs'));
    await waitFor(() =>
      expect(getMock).toHaveBeenCalledWith(expect.stringContaining('/api/cadence/runs')),
    );
  ...
  it('mostra erro recuperável com ação de retry quando /api/cadence/opt-outs falha, sem travar a seção de runs', async () => {
    mockApiByUrl({
      optOutsError: new Error('Falha ao carregar opt-outs'),
      runs: RUN_FIXTURE,
      sequences: [],
    });
    render(<CadenceHub />);
  ...
  describe('Sequências', () => {
    ...
    it('oculta o botão "Encerrar sequência" para um papel sem permissão de escrita (VISUALIZADOR)', async () => {
      ...
      expect(screen.queryByRole('button', { name: /Encerrar sequência/i })).not.toBeInTheDocument();
    });
    ...
    it('encerra a sequência ao confirmar, chama a rota certa e recarrega a lista', async () => {
      ...
      await waitFor(() =>
        expect(
          getMock.mock.calls.filter((c) => c[0] === '/api/cadence/sequences').length,
        ).toBeGreaterThan(1),
      );
