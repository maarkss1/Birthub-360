...
import {
  searchGooglePlace,
  searchGooglePlaceDetailed,
} from '@/features/prospecting/services/places.service.js';
...
describe('searchGooglePlaceDetailed', () => {
  it('devolve o lugar encontrado, sem error', async () => {
    const fetchMock = vi
      ...
      .mockResolvedValue(
        jsonResponse(200, {
          places: [{ id: 'place-1', displayName: { text: 'Transportadora Exemplo' }, rating: 4.5 }],
        }),
      );
    ...
    expect(place).toMatchObject({
      id: 'place-1',
      displayName: 'Transportadora Exemplo',
      rating: 4.5,
    });
  });
  ...
  it('achado corrigido: resposta não-ok preenche `.error` em vez de ficar indistinguível de "não achou"', async () => {
    const fetchMock = vi
      .fn()
      .mockResolvedValue(jsonResponse(403, { error: { message: 'API key revoked' } }));
    vi.stubGlobal('fetch', fetchMock);
