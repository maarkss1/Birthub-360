...
const withRlsContextMock = vi.fn(
  (fn: (tx: { $queryRaw: typeof txQueryRaw; $executeRaw: typeof txExecuteRaw }) => unknown) =>
...
const { buildCompanyProfileText, updateCompanyProfileEmbedding, computeLookalikeScore } =
  await import('../../../../../src/features/prospecting/services/lookalike-scoring.service');
...
describe('buildCompanyProfileText', () => {
  it('joins only the fields that are actually present, never inventing data', () => {
    ...
    expect(text).toBe(
      'Transportadora Exemplo | Segmento: Transporte de cargas | Localização: Ribeirão Preto, SP | Tecnologias: SAP',
    );
  });
});
...
describe('computeLookalikeScore', () => {
  ...
  it('returns null (cold start) when there are fewer won reference companies than the minimum', async () => {
    ...
    txQueryRaw
      ...
      .mockResolvedValueOnce([
        // only 2 matches, below MIN_REFERENCE_COMPANIES (3)
  ...
  it('scores from the average of the top matches and persists an auditable match list', async () => {
    ...
    generateEmbedding.mockResolvedValueOnce([0.1, 0.2]);
    txQueryRaw.mockResolvedValueOnce([{ profileEmbedding: '[0.1,0.2]' }]).mockResolvedValueOnce([
      { id: 'c2', tradeName: 'Ganha 1', similarity: 0.9 },
      { id: 'c3', tradeName: 'Ganha 2', similarity: 0.8 },
