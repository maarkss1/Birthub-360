...
describe('enrichCompany', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    vi.mocked(prisma.company.update).mockImplementation(
  ...
  describe('dedupe de decisores (Apollo/Hunter) — LGPD/Onda 7 (05): não recriar contato já existente', () => {
    it('não recria um Contact cujo e-mail já existe para esta empresa', async () => {
      ...
      const call = vi.mocked(prisma.contact.createMany).mock.calls[0][0] as {
  ...
  describe('proveniência (dataOrigin/appliedToCompany) — LGPD/Onda 2 (05): rotulagem inferido vs. confirmado', () => {
    function logDataFor(source: string) {
      const call = vi
        .mocked(prisma.enrichmentLog.create)
        .mock.calls.find(([arg]) => (arg as { data: { source: string } }).data.source === source);
    ...
    it('grava dataOrigin "confirmado" e appliedToCompany:true quando a Receita Federal (BrasilAPI-CNPJ) encontra o CNPJ', async () => {
      ...
      expect(logDataFor('BrasilAPI-CNPJ')?.data).toMatchObject({
...
describe('isEnrichmentFresh', () => {
  ...
  it('true quando enriquecido com sucesso e dentro do TTL', () => {
    ...
    expect(
      isEnrichmentFresh({ enrichmentStatus: 'Enriquecido', enrichedAt }, now, ONE_DAY_MS),
    ).toBe(true);
  });
  ...
  it('ignora o cache e roda o enriquecimento completo quando options.force = true', async () => {
    ...
    expect(prisma.company.update).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({ enrichmentStatus: 'Enriquecendo' }),
      }),
    );
