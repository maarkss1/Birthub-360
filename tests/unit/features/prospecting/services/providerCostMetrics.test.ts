...
describe('src/features/prospecting/services/providerCostMetrics.ts', () => {
  ...
  it('getCostPerCallUsd devolve a estimativa default documentada quando nenhuma env var está setada', async () => {
    const { getCostPerCallUsd, DEFAULT_PROVIDER_COST_PER_CALL_USD } =
      await import('../../../../../src/features/prospecting/services/providerCostMetrics.js');
  ...
  it('getCostPerCallUsd respeita override por env var por provider', async () => {
    const { getCostPerCallUsd } =
      await import('../../../../../src/features/prospecting/services/providerCostMetrics.js');

    expect(getCostPerCallUsd('apollo', { PROSPECTING_APOLLO_COST_PER_CALL_USD: '0.05' })).toBe(
      0.05,
    );
  ...
  it('getCostPerCallUsd ignora env var inválida/negativa e cai para o default', async () => {
    ...
    expect(
      getCostPerCallUsd('apollo', { PROSPECTING_APOLLO_COST_PER_CALL_USD: 'not-a-number' }),
    ).toBe(DEFAULT_PROVIDER_COST_PER_CALL_USD.apollo);
  ...
  it('recordProviderCallCost incrementa prospecting_provider_cost_usd_total com o valor certo, rotulado por provider', async () => {
    ...
    const values = (
      cost as unknown as { values: Array<{ value: number; labels: Record<string, string> }> }
    )?.values;
