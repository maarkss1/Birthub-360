...
describe('CnpjResultCard — sinal territorial RNTRC (ANTT)', () => {
  ...
  it('mostra o tier, o total de transportadoras e a explicação do que o número significa', () => {
    const result = baseResult({
      marketRisk: {
        ...
        metadata: {
          dataset: 'ANTT RNTRC',
          competencia: '2026-07',
          sourceUrl: 'https://dados.antt.gov.br/',
          hash: 'a'.repeat(64),
    ...
    render(
      <CnpjResultCard result={result} onPromote={vi.fn()} isPromoting={false} promoted={false} />,
    );
    ...
    expect(
      screen.getByText(/253\.129 transportadoras registradas no RNTRC em SP/),
    ).toBeInTheDocument();
    ...
    expect(
      screen.getByText(/não é uma medida de sinistralidade \(roubo\/furto\)/),
    ).toBeInTheDocument();
  });
  ...
  it('nunca mostra um número solto: quando indisponível, explica o motivo em vez de esconder ou inventar', () => {
    ...
    expect(
      screen.getByText(
        /RNTRC territorial NÃO DISPONÍVEL — Nenhum dado RNTRC \(ANTT\) publicado para AP/,
      ),
    ).toBeInTheDocument();
    expect(
      screen.queryByText('Concentração alta de transportadoras RNTRC'),
