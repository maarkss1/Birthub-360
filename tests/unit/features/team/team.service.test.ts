...
describe('deleteTeamMember', () => {
  it('bloqueia excluir o único ADMIN da organização (achado real do Piloto 024)', async () => {
    ...
    await expect(deleteTeamMember(ORG, 'admin-1', 'requester-1')).rejects.toThrow(/único ADMIN/);
  ...
  it('continua bloqueando auto-exclusão antes mesmo de checar o papel', async () => {
    await expect(deleteTeamMember(ORG, 'self-1', 'self-1')).rejects.toThrow(/própria conta/);
