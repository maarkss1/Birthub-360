...
describe('AnalyticsService.overview — janela de fechamento do mês', () => {
  it('conta "ganho no mês" e "perdido no mês" filtrando por closedAt, não updatedAt', async () => {
    ...
    const wonCall = lead.count.mock.calls.find(
      ([args]) => typeof args.where.status === 'string' && args.where.status.includes('Ganho'),
    );
    const lostCall = lead.count.mock.calls.find(
      ([args]) => typeof args.where.status === 'string' && args.where.status.includes('Perdido'),
    );
  ...
  it('agrupa um lead fechado no mês do closedAt dele, não do createdAt/updatedAt', async () => {
    lead.findMany
      ...
      .mockResolvedValueOnce([
        { closedAt: new Date('2026-08-10T00:00:00Z'), status: 'Negocios_Ganhos' },
      ]); // closed
