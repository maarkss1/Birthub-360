...
import {
  ...
  formatAssistantRecordContext,
  getAssistantRouteContext,
} from '../../../src/hooks/assistantContext';
...
describe('assistantContext', () => {
  it('descreve a rota comercial atual para o copiloto', () => {
    ...
    expect(getAssistantRouteContext('/app/companies/abc').label).toBe(
      'Empresas / inteligência de contas',
    );
  });
  ...
  it('inclui tipo, id, nome e resumo do registro aberto sem inventar dados externos', () => {
    expect(
      ...
      'REGISTRO ABERTO NA TELA (lead): Renovação Frota Atlas\nID interno: lead-123\nResumo visível: Status: Proposta — temperatura: Quente',
    );
  });
