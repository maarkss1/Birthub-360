...
| 2   | **Render + Neon + Cloudflare** (monólito Express e worker) | `render.yaml`, `docs/deploy/producao.md`, `docs/deploy/render.md`, `docs/deploy/worker-service-render.md`                               | **Ativo e Canônico** — caminho de produção oficial do produto. O worker dedicado (`prospector-atlas-worker`) está documentado em [`docs/deploy/worker-service-render.md`](worker-service-render.md). |
...
O caminho de produção consolidado em Render opera o monólito Express e o worker dedicado BullMQ para processamento assíncrono em segundo plano (ver [`docs/deploy/worker-service-render.md`](worker-service-render.md)).
