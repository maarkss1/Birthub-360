...
> **Decisão de Arquitetura e Deploy (2026-09-20): Transição para 100% LOCAL-FIRST.**
...
> O **único ambiente ativo, homologado e canônico** da plataforma é o **Local-First** (`docker-compose.yml`), operando Postgres local com pgvector, Redis local, LiteLLM, Ollama, Meilisearch e MinIO local.

## 1. Caminho canônico por ambiente (estado real, 2026-09-20)

| Ambiente                                                 | Caminho canônico hoje                                                                                                                                              | Fonte de verdade                                                                                                                         | Status                                                                                                                                                                                                                                               |
| -------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Desenvolvimento e Execução (Canônico)**                | **Local-First (`docker-compose.yml`)** (Postgres+pgvector, Redis, Meilisearch, MinIO, LiteLLM, Ollama) + `npm run dev`                                            | [`docs/development/LOCAL_FIRST.md`](../development/LOCAL_FIRST.md)                                                                       | **Ativo e Canônico** — único ambiente operacional da Central no momento                                                                                                                                                                             |
| **Produção Cloud (Render + Neon + Cloudflare)**          | ~~Render monólito Express + worker, Neon Postgres, Cloudflare CDN/R2~~                                                                                              | [`docs/deploy/producao.md`](producao.md), [`infrastructure/archive/render.yaml.disabled`](../../infrastructure/archive/render.yaml.disabled) | **Desativado / Arquivado por enquanto** — Blueprint retirado da raiz para suspender auto-deploys                                                                                                                                                     |
| **Homologação Cloud**                                    | Nenhum ambiente cloud ativo. Workflows de CI/CD para nuvem suspensos                                                                                               | [`k8s/README.md`](../../k8s/README.md), [`charts/README.md`](../../charts/README.md)                                                     | **Desativado** — pipeline externo suspenso                                                                                                                                                                                                           |
| ~~**Produção OCI (alvo descontinuado)**~~                | ~~Oracle Cloud Infrastructure self-hosted~~                                                                                                                        | Deletado em 2026-09-18                                                                                                                   | **Retirado (DEVOPS-010)** — histórico arquivado                                                                                                                                                                                                      |

## 2. Caminhos documentados — o que cada um é e seu status real

| #   | Caminho                                                    | Arquivos                                                                                                                                                                | Status                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| --- | ---------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | **Local-first (docker-compose)**                           | `docker-compose.yml`, `docs/development/LOCAL_FIRST.md`                                                                                                                 | **Ativo e Canônico** — único ambiente em execução e suporte ativo                                                                                                                                                                                                                                                                                                                   |
| 2   | **Render + Neon + Cloudflare** (monólito Express + worker dedicado) | `infrastructure/archive/render.yaml.disabled`, `docs/deploy/producao.md`, `docs/deploy/render.md`, `docs/deploy/worker-service-render.md`                               | **Desativado / Arquivado por enquanto** — blueprints preservados como referência; sem deploys ou tráfego de produção ativo em nuvem.                                                                                                                                                                                |
| 3   | **Kubernetes / Helm / ArgoCD**                             | `k8s/`, `charts/prospector-atlas/`, `argocd/`                                                                                                                           | Aspiracional / exploratório — manifests preservados, sem cluster ativo                                                                                                                                                                                                                                                                                                              |
| 4   | **Self-hosted Oracle Cloud (Docker Compose + Caddy)**      | `docs/deploy/oracle-cloud.md`                                                                                                                                           | **Aposentado** — descontinuado conforme DEVOPS-010.                                                                                                                                                                                                                                                                                                                                  |

A plataforma opera exclusivamente no modo Local-First. Todos os caminhos de produção em nuvem (Render, Neon, Cloudflare, Kubernetes, OCI) estão formalmente desativados.
