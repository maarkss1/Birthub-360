...
| 2   | **Self-hosted Oracle Cloud (Docker Compose + Caddy)**      | `docker-compose.oci.yml`, `docs/deploy/oracle-cloud.md`, `scripts/deploy-oci.sh`, `scripts/backup-oci.sh`, `scripts/restore-oci.sh`, `.github/workflows/deploy-oci.yml` | **Alvo definitivo de produção (ADR-004), instância real já provisionada e recebendo tráfego.** Deploy automático via GitHub Actions existe (`deploy-oci.yml`, gate por `ci.yml` verde em `main`, mesmo padrão de `docker-publish.yml`) — **ativação pendente** dos secrets SSH (`OCI_SSH_HOST`/`OCI_SSH_USER`/`OCI_SSH_PRIVATE_KEY`/`OCI_DEPLOY_PATH`, ver seção 3.3 de `docs/deploy/oracle-cloud.md`); até lá, deploy continua manual (seção 3.2) |
| 3   | **Render + Supabase/Neon + Cloudflare** (monólito Express) | `render.yaml`, `docs/deploy/producao.md`, `docs/deploy/render.md`                                                                                                       | **Fallback/rollback durante a transição para Oracle** — continua recebendo tráfego real hoje, não recebe mais investimento de infraestrutura novo, não desligar antes do cutover Oracle validado. O worker dedicado (`prospector-atlas-worker`) desse caminho está preparado mas não criado — ver [`docs/deploy/worker-service-render.md`](worker-service-render.md)                                                                                                                                                                                                                                                   |
