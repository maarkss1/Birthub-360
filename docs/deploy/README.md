...
> **Atualização de destino de produção (2026-09-18/20, ver DEVOPS-010):** a tentativa
...
| 2   | **Render + Neon + Cloudflare** (monólito Express + worker dedicado) | `render.yaml`, `docs/deploy/producao.md`, `docs/deploy/render.md`, `docs/deploy/worker-service-render.md` | **Ativo e Canônico** — caminho de produção oficial do produto. O worker dedicado (`prospector-atlas-worker`) desse caminho está documentado em [`docs/deploy/worker-service-render.md`](worker-service-render.md). |
| 3   | **Kubernetes / Helm / ArgoCD**                             | `k8s/`, `charts/prospector-atlas/`, `argocd/`                                                                                                                           | Aspiracional / exploratório — chart e manifests preservados, sem cluster ativo no momento                                                                                                                                                               |
| 4   | **Self-hosted Oracle Cloud (Docker Compose + Caddy)**      | `docs/deploy/oracle-cloud.md`                                                                                                                                           | **Aposentado** — descontinuado conforme DEVOPS-010 e consolidado no Render como único alvo de produção oficial. |

O caminho canônico de produção ativo é Render + Neon + Cloudflare (com monólito Express e worker dedicado). O caminho Oracle Cloud foi aposentado e arquivado como referência histórica conforme DEVOPS-010.
