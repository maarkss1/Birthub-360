...
3. Em **Environment**, preencha todas as variáveis marcadas `sync: false` no `render.yaml`:
   ...
   | `ALLOWED_ORIGINS`                                                                           | `https://app.birthhub360.com.br` (mais qualquer outro domínio real que sirva o frontend)  |
...
1. [Dashboard Cloudflare](https://dash.cloudflare.com) → **Add a site** → `birthhub360.com.br` (se
...
3. No Render, **Settings → Custom Domains** → adicione `app.birthhub360.com.br` e siga a verificação
...
5. Atualize `ALLOWED_ORIGINS`, `BETTER_AUTH_URL` e `PUBLIC_BASE_URL` no Render para
   `https://app.birthhub360.com.br` e faça um novo deploy (ou "Clear build cache & deploy") para a
   mudança de env var surtir efeito.

> Atualizado 2026-09-22 (pedido explícito do usuário): os domínios corporativos antigos
> (`birthhub360.com.br`/`birthhub360.com.br`) foram substituídos por `birthhub360.com.br` em todo o
> código e documentação — não há mais vínculo com essas duas operações. Se um domínio diferente
> (ex.: `birthhub.ai`) acabar sendo o registrado de fato, repita os passos acima substituindo o
> hostname.
...
- [ ] `GET https://app.birthhub360.com.br/health/live` → `200 { status: "ok" }`
- [ ] `GET https://app.birthhub360.com.br/health/ready` → `200` (confirma conexão real com o Neon)
- [ ] `GET https://app.birthhub360.com.br/` → carrega o SPA (index.html do build do Vite)
- [ ] Login (e-mail/senha e "Entrar com Google") funcionando via Better Auth
- [ ] `GET https://app.birthhub360.com.br/api/companies` (autenticado) retorna `200`, não `404`/`503`
...
- [ ] Certificado TLS válido (cadeado no navegador) em `app.birthhub360.com.br`
...
| Serviço                         | URL                                                                                           |
| ------------------------------- | --------------------------------------------------------------------------------------------- |
| Aplicação (frontend + API)      | `https://app.birthhub360.com.br` (após seção 3) — até lá, `https://prospector-atlas.onrender.com` |
