# BIRTH HUB 360 — ONDA F — PRODUCT ANALYTICS & CUSTOMER SUCCESS

| | |
|---|---|
| **Prioridade** | P2 |
| **Trilha** | T5 — EXPERIÊNCIA E ESCALA |
| **Posição na ordem recomendada** | 25 de 34 |
| **Depende de** | H, B |
| **Habilita** | T, V, X, Y |
...
Instrumentar ativação, adoção, retenção e valor com um dicionário de eventos que não muda de sentido a cada release.

**Por que agora:** Sem eventos confiáveis, CS (V), onboarding (T) e experimentação (X) tomam decisão com achismo.
...
Crie `docs/waves/post-09/ONDA_F_DISCOVERY.md` contendo mapa técnico, estado atual, evidência por arquivo:linha, lacunas reais, riscos, decisões e plano de execução.
...
Crie `docs/waves/post-09/ONDA_F_REPORT.md` seguindo `templates/TEMPLATE_REPORT.md`.
...
## CRITÉRIOS DE ACEITE ESPECÍFICOS DA ONDA F

Cada critério é uma afirmação falsificável. Para cada um, anexe a evidência correspondente.

1. Dicionário de eventos existe como arquivo tipado no repositório; emissão de evento não declarado falha no typecheck ou em teste.
2. Definição de ativação está escrita, versionada e implementada numa única consulta reprodutível anexada ao relatório.
3. Eventos são idempotentes por chave; reenvio não duplica métrica — teste automatizado.
4. Validação automatizada garante que nenhum evento envia campo classificado como PII sem base declarada.
5. Todo evento carrega tenant_id e é isolado por tenant na leitura — teste negativo cross-tenant.
6. Health score tem fórmula documentada com pesos e é reproduzível a partir de dados brutos.
...
```
ONDA F — PRODUCT ANALYTICS & CUSTOMER SUCCESS
