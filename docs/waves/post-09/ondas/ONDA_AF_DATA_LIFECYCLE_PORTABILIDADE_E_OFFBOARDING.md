# BIRTH HUB 360 — ONDA AF — DATA LIFECYCLE, PORTABILIDADE E OFFBOARDING

| | |
|---|---|
| **Prioridade** | P1 |
| **Trilha** | T0 — FUNDAÇÃO E CONTROLE |
| **Posição na ordem recomendada** | 4 de 34 |
| **Depende de** | J, H |
| **Habilita** | Z, V |
...
Fechar o ciclo de vida do dado: retenção, exclusão, exportação e encerramento de contrato — com prova de execução.

**Por que agora:** LACUNA DO PACOTE ORIGINAL. A LGPD garante exclusão e portabilidade, e contrato B2B exige saída limpa. Sem isso, cada cliente que sai vira risco jurídico.
...
Crie `docs/waves/post-09/ONDA_AF_DISCOVERY.md` contendo mapa técnico, estado atual, evidência por arquivo:linha, lacunas reais, riscos, decisões e plano de execução.
...
Crie `docs/waves/post-09/ONDA_AF_REPORT.md` seguindo `templates/TEMPLATE_REPORT.md`.
...
## CRITÉRIOS DE ACEITE ESPECÍFICOS DA ONDA AF

Cada critério é uma afirmação falsificável. Para cada um, anexe a evidência correspondente.

1. Existe mapa de onde cada categoria de dado pessoal reside, incluindo backup, cache, índice vetorial, logs e sub-processadores.
2. Exclusão de titular executa de ponta a ponta em ambiente de teste e a verificação posterior não encontra o dado nos destinos mapeados — evidência anexada.
3. Dado em backup tem tratamento declarado (exclusão na restauração ou expiração do backup) — política escrita e testada.
4. Exportação gera arquivo em formato aberto, completo e reimportável; teste de ida e volta anexado.
5. Rotinas de retenção rodam agendadas, registram o que removeram e são idempotentes — teste automatizado.
6. Anonimização é testada contra reidentificação trivial (combinação de campos quase únicos) e o resultado está no relatório.
7. Encerramento de contrato tem prazos definidos e a destruição é registrada com evidência.
...
```
ONDA AF — DATA LIFECYCLE, PORTABILIDADE E OFFBOARDING
