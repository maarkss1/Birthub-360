# BIRTH HUB 360 — ONDA AE — I18N, FUSOS, MOEDA E FORMATAÇÃO

| | |
|---|---|
| **Prioridade** | P2 |
| **Trilha** | T5 — EXPERIÊNCIA E ESCALA |
| **Posição na ordem recomendada** | 23 de 34 |
| **Depende de** | E, H |
| **Habilita** | M, P, Y |
...
Acertar tempo, moeda, número e idioma — os erros silenciosos que só aparecem no relatório do cliente.

**Por que agora:** LACUNA DO PACOTE ORIGINAL. Cadência, agendamento, fechamento de período e relatório dependem de fuso correto. Um erro de fuso vira reunião perdida.
...
Crie `docs/waves/post-09/ONDA_AE_DISCOVERY.md` contendo mapa técnico, estado atual, evidência por arquivo:linha, lacunas reais, riscos, decisões e plano de execução.
...
Crie `docs/waves/post-09/ONDA_AE_REPORT.md` seguindo `templates/TEMPLATE_REPORT.md`.
...
## CRITÉRIOS DE ACEITE ESPECÍFICOS DA ONDA AE

Cada critério é uma afirmação falsificável. Para cada um, anexe a evidência correspondente.

1. Varredura mostra zero uso de data local do servidor em lógica de negócio; todo instante persistido é UTC — evidência anexada.
2. Existem testes com fusos distintos e com transição de horário de verão para agendamento, cadência e fechamento de período.
3. Textos da interface estão extraídos em catálogo; texto codificado no componente falha lint ou auditoria.
4. Valores monetários carregam moeda explícita; conversão registra taxa e data — teste automatizado.
5. Formatação usa a localidade do usuário, não a do servidor — teste com locale forçado.
6. Relatórios por período declaram o fuso usado no cabeçalho.
...
```
ONDA AE — I18N, FUSOS, MOEDA E FORMATAÇÃO
