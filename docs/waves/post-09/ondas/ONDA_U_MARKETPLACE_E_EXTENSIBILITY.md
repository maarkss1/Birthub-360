# BIRTH HUB 360 — ONDA U — MARKETPLACE & EXTENSIBILITY

| | |
|---|---|
| **Prioridade** | P3 |
| **Trilha** | T6 — PLATAFORMA E CRESCIMENTO |
| **Posição na ordem recomendada** | 31 de 34 |
| **Depende de** | G, N, S |
| **Habilita** | X |
...
Preparar extensões seguras de conectores, agentes, skills e templates — com isolamento real, não confiança.

**Por que agora:** Extensão de terceiro é código de terceiro rodando perto do dado do cliente. Sem sandbox e revisão, é uma superfície de ataque com loja.
...
Crie `docs/waves/post-09/ONDA_U_DISCOVERY.md` contendo mapa técnico, estado atual, evidência por arquivo:linha, lacunas reais, riscos, decisões e plano de execução.
...
Crie `docs/waves/post-09/ONDA_U_REPORT.md` seguindo `templates/TEMPLATE_REPORT.md`.
...
## CRITÉRIOS DE ACEITE ESPECÍFICOS DA ONDA U

Cada critério é uma afirmação falsificável. Para cada um, anexe a evidência correspondente.

1. Manifesto de extensão é validado por schema; extensão sem permissões declaradas não instala.
2. Teste de isolamento: extensão tenta acessar recurso fora do escopo e é bloqueada, com registro do evento.
3. Limites de CPU, memória, tempo e chamadas externas são aplicados no runtime — teste de estouro.
4. Existe interruptor por extensão e por tenant que desativa em produção sem deploy — teste automatizado.
5. Atualização de extensão exige nova aprovação quando as permissões aumentam.
6. Falha de extensão não derruba o fluxo principal — teste de degradação.
...
```
ONDA U — MARKETPLACE & EXTENSIBILITY
