# BIRTH HUB 360 — ONDA R — PERFORMANCE & SCALABILITY

| | |
|---|---|
| **Prioridade** | P2 |
| **Trilha** | T5 — EXPERIÊNCIA E ESCALA |
| **Posição na ordem recomendada** | 24 de 34 |
| **Depende de** | D, Q |
| **Habilita** | AH, Z |
...
Preparar crescimento com orçamentos explícitos de latência, throughput e custo — medidos, não estimados.

**Por que agora:** Antes de vender escala é preciso saber onde ela quebra. E quebra quase sempre em N+1 e em índice ausente.
...
Crie `docs/waves/post-09/ONDA_R_DISCOVERY.md` contendo mapa técnico, estado atual, evidência por arquivo:linha, lacunas reais, riscos, decisões e plano de execução.
...
Crie `docs/waves/post-09/ONDA_R_REPORT.md` seguindo `templates/TEMPLATE_REPORT.md`.
...
## CRITÉRIOS DE ACEITE ESPECÍFICOS DA ONDA R

Cada critério é uma afirmação falsificável. Para cada um, anexe a evidência correspondente.

1. Orçamento de latência p95 definido por rota crítica e verificado por teste automatizado que falha ao estourar.
2. Relatório com as consultas mais lentas, plano de execução e correção aplicada ou dívida registrada.
3. Nenhuma rota de listagem sem paginação obrigatória e limite máximo — varredura anexada.
4. Teste de carga executado com carga crescente até saturação; gráfico ou tabela de resultado anexado ao relatório com o ponto de quebra nomeado.
5. Chaves de cache incluem tenant e versão; invalidação testada por cenário de escrita.
6. Orçamento de tamanho de bundle definido e verificado no CI.
...
```
ONDA R — PERFORMANCE & SCALABILITY
