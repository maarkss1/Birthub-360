# BIRTH HUB 360 — ONDA H — DATA GOVERNANCE & MASTER DATA

| | |
|---|---|
| **Prioridade** | P1 |
| **Trilha** | T0 — FUNDAÇÃO E CONTROLE |
| **Posição na ordem recomendada** | 3 de 34 |
| **Depende de** | J |
...
Criar verdade canônica para entidades centrais: identidade, deduplicação, linhagem e qualidade de dado.

**Por que agora:** Toda análise, IA e cobrança acima disso herda o erro daqui. Duplicata de conta vira duplicata de fatura.
...
Crie `docs/waves/post-09/ONDA_H_DISCOVERY.md` contendo mapa técnico, estado atual, evidência por arquivo:linha, lacunas reais, riscos, decisões e plano de execução.
...
Crie `docs/waves/post-09/ONDA_H_REPORT.md` seguindo `templates/TEMPLATE_REPORT.md`.
...
## CRITÉRIOS DE ACEITE ESPECÍFICOS DA ONDA H

Cada critério é uma afirmação falsificável. Para cada um, anexe a evidência correspondente.

1. Documento de modelo canônico existe e mapeia cada entidade central para tabela, chave natural e dono.
2. Existe rotina de detecção de duplicatas com resultado medido antes e depois; o número de duplicatas candidatas está no relatório.
3. Merge de registros preserva histórico e é reversível — teste automatizado de merge e undo.
4. Campos críticos carregam origem e timestamp; consulta de linhagem demonstrada no relatório.
5. Validadores de CNPJ/CPF/telefone/CEP rejeitam entradas inválidas com teste de casos reais e casos limite.
6. Toda tabela com dado de tenant tem coluna de tenant e índice correspondente; varredura de schema anexada — exceções justificadas uma a uma.
7. Nenhuma migration desta onda é destrutiva sem plano de rollback escrito e testado em cópia.
...
```
ONDA H — DATA GOVERNANCE & MASTER DATA
