# BIRTH HUB 360 — ONDA AH — FINOPS DE INFRAESTRUTURA E MARGEM POR TENANT

| | |
|---|---|
| **Prioridade** | P2 |
| **Trilha** | T6 — PLATAFORMA E CRESCIMENTO |
| **Posição na ordem recomendada** | 33 de 34 |
| **Depende de** | P, W, R |
| **Habilita** | B, Y, Z |
...
Saber quanto custa servir cada cliente e qual é a margem real por plano — antes de escalar o prejuízo.

**Por que agora:** LACUNA DO PACOTE ORIGINAL. A Onda W cuida do custo de IA; falta o custo total (banco, storage, provedores pagos, egress) confrontado com a receita da Onda P.
...
Crie `docs/waves/post-09/ONDA_AH_DISCOVERY.md` contendo mapa técnico, estado atual, evidência por arquivo:linha, lacunas reais, riscos, decisões e plano de execução.
...
Crie `docs/waves/post-09/ONDA_AH_REPORT.md` seguindo `templates/TEMPLATE_REPORT.md`.
...
## CRITÉRIOS DE ACEITE ESPECÍFICOS DA ONDA AH

Cada critério é uma afirmação falsificável. Para cada um, anexe a evidência correspondente.

1. Existe método de alocação declarado por recurso; nenhum custo relevante fica sem atribuição ou sem rateio explicado.
2. Relatório de margem por plano e por tenant é reproduzível por comando, com a consulta anexada.
3. Consumo de provedores pagos (Apollo, Hunter, Google Maps, IA) é atribuído ao tenant que o originou — teste de atribuição.
4. Alerta de custo anômalo por tenant dispara em cenário simulado.
5. Ambientes não produtivos têm política de desligamento e o custo deles está separado do custo de servir clientes.
6. Nenhum relatório de margem expõe dado de um tenant a outro — teste negativo.
...
```
ONDA AH — FINOPS DE INFRAESTRUTURA E MARGEM POR TENANT
