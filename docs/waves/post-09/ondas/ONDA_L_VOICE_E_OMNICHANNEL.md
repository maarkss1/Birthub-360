# BIRTH HUB 360 — ONDA L — VOICE & OMNICHANNEL

| | |
|---|---|
| **Prioridade** | P2 |
| **Trilha** | T5 — EXPERIÊNCIA E ESCALA |
| **Posição na ordem recomendada** | 27 de 34 |
| **Depende de** | H, I |
| **Habilita** | V, AG |
...
Unificar voz, e-mail, WhatsApp e telefonia numa timeline operacional única por conta e contato.

**Por que agora:** Canal fragmentado gera duplicidade de contato com o mesmo cliente — o erro mais caro em operação comercial.
...
Crie `docs/waves/post-09/ONDA_L_DISCOVERY.md` contendo mapa técnico, estado atual, evidência por arquivo:linha, lacunas reais, riscos, decisões e plano de execução.
...
Crie `docs/waves/post-09/ONDA_L_REPORT.md` seguindo `templates/TEMPLATE_REPORT.md`.
...
## CRITÉRIOS DE ACEITE ESPECÍFICOS DA ONDA L

Cada critério é uma afirmação falsificável. Para cada um, anexe a evidência correspondente.

1. Existe modelo único de conversa; nenhum canal grava em tabela própria paralela — schema anexado.
2. Mensagem duplicada pelo provedor não duplica na timeline — teste com webhook repetido.
3. Opt-out registrado em qualquer canal bloqueia envio nos demais canais aplicáveis — teste automatizado.
4. Gravação e transcrição só ocorrem com consentimento registrado; sem consentimento, o sistema recusa e registra o motivo.
5. Transcrição e resumo trazem referência ao trecho de origem quando afirmam fato.
6. Timeline isolada por tenant; teste negativo cross-tenant por conversa.
