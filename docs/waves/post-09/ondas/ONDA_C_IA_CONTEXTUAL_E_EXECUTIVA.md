# BIRTH HUB 360 — ONDA C — IA CONTEXTUAL E EXECUTIVA

| | |
|---|---|
| **Prioridade** | P1 |
| **Trilha** | T3 — IA E AUTOMAÇÃO |
| **Posição na ordem recomendada** | 16 de 34 |
| **Depende de** | N, H |
| **Habilita** | K, W, Y |
...
Fazer a IA operar sobre contexto real do tenant, citar evidência e propor ações aprováveis em vez de texto solto.

**Por que agora:** IA sem contexto e sem ação é chat caro. Depende do runtime da Onda N e das defesas da Onda AD.
...
Crie `docs/waves/post-09/ONDA_C_DISCOVERY.md` contendo mapa técnico, estado atual, evidência por arquivo:linha, lacunas reais, riscos, decisões e plano de execução.
...
Crie `docs/waves/post-09/ONDA_C_REPORT.md` seguindo `templates/TEMPLATE_REPORT.md`.
...
## CRITÉRIOS DE ACEITE ESPECÍFICOS DA ONDA C

Cada critério é uma afirmação falsificável. Para cada um, anexe a evidência correspondente.

1. Nenhuma resposta de IA sobre dados do cliente é emitida sem lista de fontes com identificadores consultáveis; teste automatizado verifica presença e validade dos ids.
2. Toda ação executável proposta pela IA passa por um envelope tipado e validado; execução sem aprovação é impossível para ações classificadas como alto impacto.
3. Existe registro persistido por execução com: tenant, usuário, modelo, versão do prompt, ids do contexto, tokens, custo, latência e resultado.
4. Teste de isolamento: contexto montado para o tenant A nunca inclui registro do tenant B, inclusive em cache e em busca vetorial.
5. Teste de degradação: com o gateway de IA indisponível, a interface mostra estado degradado explícito e não inventa resposta.
6. Existe conjunto mínimo de casos de abstenção onde a resposta correta é 'não há dado suficiente' — verificado por eval automatizada.
...
```
ONDA C — IA CONTEXTUAL E EXECUTIVA
