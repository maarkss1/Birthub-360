# BIRTH HUB 360 — ONDA D — PRODUCTION HARDENING & DISASTER RECOVERY

| | |
|---|---|
| **Prioridade** | P1 |
| **Trilha** | T2 — CONFIABILIDADE |
| **Posição na ordem recomendada** | 10 de 34 |
| **Depende de** | AC, J |
| **Habilita** | AB, R, Z |
...
Provar recuperação diante de falhas reais e perda controlada de componentes — com restore executado, não documentado.

**Por que agora:** Backup não testado é superstição. Antes de escalar (R) e antes de vender (Z), é preciso saber quanto tempo custa voltar.
...
Crie `docs/waves/post-09/ONDA_D_DISCOVERY.md` contendo mapa técnico, estado atual, evidência por arquivo:linha, lacunas reais, riscos, decisões e plano de execução.
...
Crie `docs/waves/post-09/ONDA_D_REPORT.md` seguindo `templates/TEMPLATE_REPORT.md`.
...
## CRITÉRIOS DE ACEITE ESPECÍFICOS DA ONDA D

Cada critério é uma afirmação falsificável. Para cada um, anexe a evidência correspondente.

1. Restore completo do banco executado em ambiente isolado com dump real anonimizado; log do processo, tempo total e verificação de integridade anexados ao relatório.
2. Para cada dependência externa crítica existe um teste (unit/integration) que simula indisponibilidade e verifica o comportamento degradado esperado.
3. Readiness e liveness são distintos e readiness reflete dependências obrigatórias — comprovado por manifesto e por teste.
4. Toda chamada de saída tem timeout explícito; varredura mostra zero chamadas HTTP sem timeout configurado.
5. Existe runbook por cenário (perda de banco, fila travada, gateway de IA fora, vazamento de credencial) com comandos exatos, validado por execução ao menos em dry-run.
6. RPO e RTO declarados por componente em documento, com a medição correspondente e a lacuna nomeada quando existir.
...
```
ONDA D — PRODUCTION HARDENING & DISASTER RECOVERY
