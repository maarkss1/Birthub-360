# BIRTH HUB 360 — ONDA E — UX TOTAL, MOBILE E ACESSIBILIDADE

| | |
|---|---|
| **Prioridade** | P2 |
| **Trilha** | T5 — EXPERIÊNCIA E ESCALA |
| **Posição na ordem recomendada** | 22 de 34 |
| **Depende de** | A |
| **Habilita** | T, F, Z |
...
Elevar toda a interface a um padrão coerente de marca, utilizável em mobile e acessível de fato.

**Por que agora:** A interface é o que o cliente vê primeiro. Com Capacitor (android/ios) no repositório, mobile deixou de ser hipótese.
...
Crie `docs/waves/post-09/ONDA_E_DISCOVERY.md` contendo mapa técnico, estado atual, evidência por arquivo:linha, lacunas reais, riscos, decisões e plano de execução.
...
Crie `docs/waves/post-09/ONDA_E_REPORT.md` seguindo `templates/TEMPLATE_REPORT.md`.
...
## CRITÉRIOS DE ACEITE ESPECÍFICOS DA ONDA E

Cada critério é uma afirmação falsificável. Para cada um, anexe a evidência correspondente.

1. Auditoria de acessibilidade automatizada (axe via Playwright) integrada ao CI, falhando em violações de severidade séria ou crítica nas telas principais.
2. Toda tela principal tem os cinco estados implementados e capturados em Storybook ou em teste de regressão visual.
3. Navegação completa por teclado nas jornadas críticas, com foco visível — verificado por teste automatizado.
4. Nenhuma cor ou espaçamento hardcoded fora dos tokens nas telas tocadas por esta onda — verificado por lint/varredura.
5. Build mobile (Capacitor) gera artefato executável e a jornada crítica é validada em viewport mobile.
6. Zero referência visual ou textual a AtlasGR / Total Trac na interface — varredura anexada.
...
```
ONDA E — UX TOTAL, MOBILE E ACESSIBILIDADE
