# BIRTH HUB 360 — ONDA B — SAAS, PLANOS, ASSINATURAS E ENTITLEMENTS

| | |
|---|---|
| **Prioridade** | P1 |
| **Trilha** | T1 — RECEITA REAL |
| **Posição na ordem recomendada** | 6 de 34 |
| **Depende de** | A, J |
| **Habilita** | P, U, X, AH |
...
Construir a camada comercial SaaS: planos, assinaturas, limites e entitlements aplicados no servidor.

**Por que agora:** Sem entitlement server-side não existe plano — existe sugestão. Billing (P) e marketplace (U) dependem deste contrato.
...
Crie `docs/waves/post-09/ONDA_B_DISCOVERY.md` contendo mapa técnico, estado atual, evidência por arquivo:linha, lacunas reais, riscos, decisões e plano de execução.
...
Crie `docs/waves/post-09/ONDA_B_REPORT.md` seguindo `templates/TEMPLATE_REPORT.md`.
...
## CRITÉRIOS DE ACEITE ESPECÍFICOS DA ONDA B

Cada critério é uma afirmação falsificável. Para cada um, anexe a evidência correspondente.

1. Existe uma única função/serviço de autorização de entitlement no servidor; nenhuma rota protegida decide acesso por conta própria — comprovado por varredura de código no relatório.
2. Teste automatizado: usuário em plano sem a feature recebe 402/403 tipado ao chamar a rota diretamente, mesmo com a interface escondendo o botão.
3. Teste de downgrade: após rebaixar o plano, o acesso removido é negado no próximo request, sem restart e sem depender de cache com TTL longo (ou o TTL é documentado e testado).
4. Todo limite quantitativo tem contador persistido, consulta de uso atual e teste de comportamento no estouro.
5. Mudança de plano gera evento de auditoria imutável com actor, origem e diff de entitlements.
6. Catálogo de planos é dado versionado (não constante espalhada no código) e existe migration para alterá-lo.
7. Cross-tenant: contadores de uso de um tenant nunca são lidos ou decrementados por outro — teste negativo.
...
```
ONDA B — SAAS, PLANOS, ASSINATURAS E ENTITLEMENTS
