# BIRTH HUB 360 — ONDA O — AUTOMATION & WORKFLOW ENGINE

| | |
|---|---|
| **Prioridade** | P1 |
| **Trilha** | T3 — IA E AUTOMAÇÃO |
| **Posição na ordem recomendada** | 17 de 34 |
| **Depende de** | N, I |
| **Habilita** | V, T, AG |
...
Consolidar workflows duráveis, versionados e reprocessáveis, com efeito colateral controlado.

**Por que agora:** Automação comercial roda sem ninguém olhando. Se não for durável e idempotente, o erro chega ao cliente antes de chegar ao log.
...
Crie `docs/waves/post-09/ONDA_O_DISCOVERY.md` contendo mapa técnico, estado atual, evidência por arquivo:linha, lacunas reais, riscos, decisões e plano de execução.
...
Crie `docs/waves/post-09/ONDA_O_REPORT.md` seguindo `templates/TEMPLATE_REPORT.md`.
...
## CRITÉRIOS DE ACEITE ESPECÍFICOS DA ONDA O

Cada critério é uma afirmação falsificável. Para cada um, anexe a evidência correspondente.

1. Teste de durabilidade: matar o worker no meio da execução e retomar sem duplicar efeito externo — evidência de execução anexada.
2. Execuções em andamento continuam na versão em que começaram; publicar nova versão não altera execução ativa — teste automatizado.
3. Cada passo com efeito externo tem chave de idempotência derivada de dado estável, não de timestamp — verificado por revisão e teste.
4. Existe dry-run que mostra os efeitos previstos sem aplicá-los, disponível para todo workflow com efeito externo.
5. Limites de concorrência e profundidade são obrigatórios; workflow sem limite não ativa.
6. Falha em passo compensável dispara compensação testada; a compensação também é idempotente.
7. Workflows são isolados por tenant, inclusive nas filas — teste negativo.
...
```
ONDA O — AUTOMATION & WORKFLOW ENGINE
