# BIRTH HUB 360 — ONDA K — KNOWLEDGE & RAG EXCELLENCE

| | |
|---|---|
| **Prioridade** | P2 |
| **Trilha** | T3 — IA E AUTOMAÇÃO |
| **Posição na ordem recomendada** | 18 de 34 |
| **Depende de** | H, C |
| **Habilita** | C, W, L |
...
Consolidar conhecimento e recuperação com qualidade medida, evidência rastreável e isolamento por tenant.

**Por que agora:** RAG sem avaliação é sorte. Com dois caminhos de embedding no repositório (LiteLLM e legado Gemini), o risco de inconsistência é real.
...
Crie `docs/waves/post-09/ONDA_K_DISCOVERY.md` contendo mapa técnico, estado atual, evidência por arquivo:linha, lacunas reais, riscos, decisões e plano de execução.
...
Crie `docs/waves/post-09/ONDA_K_REPORT.md` seguindo `templates/TEMPLATE_REPORT.md`.
...
## CRITÉRIOS DE ACEITE ESPECÍFICOS DA ONDA K

Cada critério é uma afirmação falsificável. Para cada um, anexe a evidência correspondente.

1. Existe conjunto dourado versionado no repositório com no mínimo 30 casos cobrindo acertos, ambiguidade e ausência de resposta.
2. Avaliação automatizada roda por comando e publica métricas; regressão além do limiar definido falha o CI.
3. Versão de modelo de embedding está registrada por vetor; consulta que mistura versões é impossível ou explicitamente bloqueada.
4. Exclusão de documento remove os vetores correspondentes — teste automatizado de ingestão, remoção e nova consulta.
5. Recuperação filtra por tenant e por permissão do usuário; teste negativo com usuário sem acesso ao documento.
6. O caminho legado de embedding foi removido ou está isolado atrás de flag com data de remoção declarada.
...
```
ONDA K — KNOWLEDGE & RAG EXCELLENCE
