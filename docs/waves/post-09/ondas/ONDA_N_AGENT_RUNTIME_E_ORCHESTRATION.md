# BIRTH HUB 360 — ONDA N — AGENT RUNTIME & ORCHESTRATION

| | |
|---|---|
| **Prioridade** | P1 |
| **Trilha** | T3 — IA E AUTOMAÇÃO |
| **Posição na ordem recomendada** | 14 de 34 |
| **Depende de** | J, H |
| **Habilita** | C, O, W, AD, U |
...
Evoluir agentes de catálogo para runtime seguro, observável e efetivamente capaz de agir.

**Por que agora:** Existem `.agents`, `AGENTS.md` e `skills-lock.json` no repositório. É a hora de provar que isso é runtime e não catálogo.
...
Crie `docs/waves/post-09/ONDA_N_DISCOVERY.md` contendo mapa técnico, estado atual, evidência por arquivo:linha, lacunas reais, riscos, decisões e plano de execução.
...
Crie `docs/waves/post-09/ONDA_N_REPORT.md` seguindo `templates/TEMPLATE_REPORT.md`.
...
## CRITÉRIOS DE ACEITE ESPECÍFICOS DA ONDA N

Cada critério é uma afirmação falsificável. Para cada um, anexe a evidência correspondente.

1. Existe teste que carrega cada agente do registro e falha se não houver caminho de execução real (não apenas prompt armazenado).
2. Chamada a ferramenta fora da lista permitida é bloqueada no runtime e registrada — teste com tentativa explícita de escalada.
3. Toda execução possui orçamento obrigatório; execução sem limite de tokens, tempo e custo é rejeitada na inicialização — teste automatizado.
4. Ações classificadas como alto impacto exigem aprovação humana registrada; não existe caminho de código que as execute sem aprovação — varredura e teste.
5. Memória do agente é isolada por tenant e por agente; teste negativo de leitura cruzada.
6. Cancelamento interrompe a execução e deixa o estado consistente — teste de cancelamento no meio de uma cadeia de ferramentas.
7. Cada execução gera traço com id correlacionável até a chamada de ferramenta e até o custo.
...
```
ONDA N — AGENT RUNTIME & ORCHESTRATION
