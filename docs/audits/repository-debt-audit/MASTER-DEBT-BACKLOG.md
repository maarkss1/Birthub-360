...
| DOCBRAND-002 / DOCBRAND-005 / DOCBRAND-012 | RESOLVED (2026-09-20) | DOCBRAND | Vertical-specific (Atlas GR) content, e.g. the Proposta Comercial feature, shipped to every tenant despite the product's ICP having broadened to vertical-agnostic. Verified during fix/docbrand-012: the module (`PropostaComercialHub.tsx`, its GR/logistics/insurance proposal catalog, the `cockpit-atlas`/`cockpit-totaltrac` tabs) and its static assets (`public/tools/portal-comercial/`, `public/tools/propostas/`) no longer exist — retired wholesale in `b06ba649`/`edcf6b0d`. `EXTERNAL_LINKS` in `module-catalog.ts` no longer hardcodes `*.atlasgr.com.br` URLs either. See `docs/audits/repository-debt-audit/agents/DOCBRAND.md` for full evidence and the choice made (removal, not generalization — content was legacy vendor demo/one-client material, not real tenant configuration). | RC-06 |
