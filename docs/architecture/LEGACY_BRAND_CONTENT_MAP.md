...
- **Data:** 2026-09-10.
- **Método:** varredura completa (grep/glob case-insensitive) por Birth Hub 360/"birthhub360",
  Birth Hub 360/"birthhub360", `atlasgroficial`, domínios antigos, cores hardcoded da
  ...
  lido e classificado.
- **Escala encontrada:** 444 arquivos com "birthhub360" (1.042 ocorrências) e 93 arquivos com
  "birthhub360" (305 ocorrências). Dado o volume, agrupado por módulo/padrão em vez de listado
...
do enum `'birthhub360' | 'birthhub360'`, ~30 arquivos confirmados). **Não fazer search-and-replace cego**
...
| Local                                                                   | Achado                                                                             | Risco                                                                                                                                                                                                                                               |
| ----------------------------------------------------------------------- | ---------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `charts/prospector-atlas/values.yaml`                                   | `repository: ghcr.io/maarksn/central-de-inteligencia-comecial-birthhub360`             | **Alto** — se o registro de containers (GHCR) já foi renomeado para acompanhar o rebranding, `pull` da imagem falha. Se não foi renomeado, é dependência real viva. **Não confirmado nesta rodada — requer verificação antes de qualquer decisão.** |
| `argocd/application-production.yaml`, `argocd/application-homolog.yaml` | `repoURL: https://github.com/MaarksN/CENTRAL-DE-INTELIGENCIA-COMECIAL-BIRTH HUB 360.git` | **Alto** — mesmo risco: o diretório local já se chama `Birthub-360`; se o remoto real também mudou de nome, o GitOps de produção/homologação pode estar rodando contra uma URL que só funciona por redirecionamento temporário do GitHub.           |
...
| Local                                                                     | Achado                                                                                                                                                                                                                           |
| ------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `src/lib/email/meetingInvite.ts:127`                                      | Botão inline no HTML do e-mail transacional real de convite de reunião ("Acessar Google Meet") usa `background:#FF5618` (laranja Birth Hub 360) em vez do token `--brand`/Antique Gold atual. **E-mail real enviado a clientes hoje.** |
...
`'birthhub360'|'birthhub360'` por substring de domínio de e-mail para exibição de segmento — heurística
...
`infrastructure/opa/policies/tenancy.rego` (package `birthhub360.tenancy`, regras
...
`infrastructure/opa/policies/rbac.rego` (package `birthhub360.rbac`) é diferente: **esse sim é
...
- `public/tools/treinamento-birthhub360/` — portal de treinamento comercial completo (Next.js export
...
- `public/tools/propostas/`, `public/tools/social-selling/`, `public/tools/hub-inteligencia-marketing/`
  — mesma situação em menor escala (`Birth Hub 360 Pipeline.html`, `Birth Hub 360 Kit Campanha LinkedIn
...
(arquivos `birthhub360-*.html` espelhando o padrão de duas-marcas antigo) não tem nenhuma referência
...
`.agents/prompts/11-marca-institucional.md` cita `identidade-visual/birthhub360/`,
`identidade-visual/birthhub360/`, `public/atlas-logo.svg`, `public/totaltrack-logo.png` como pastas
...
- **C confirmada, risco aceito e documentado** (~15 itens): `BrandContext.tsx` (limpeza de estado
  ...
  Android/iOS/Capacitor (`br.com.birthhub360.prospector`), domínio `app.birthhub360.com.br` (débito
...
- **H** (1 item): `.gitleaksignore` referencia um nome de variável de segredo antigo
  (`BIRTHHUB360_WEBHOOK_SECRET`), já triado pelo processo de segurança do projeto como não-sensível.
