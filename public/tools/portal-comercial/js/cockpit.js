...
function cockpitTickerItens() {
  ...
  if (!cache)
    return [
      'Sem dados carregados nesta sessão — abra um relatório abaixo ou configure o webhook para atualizar automaticamente.',
    ];
  ...
  itens.push(
    nAlertas
      ? `🔔 ${nAlertas} alerta(s) ativo(s)${nCriticos ? ` (${nCriticos} crítico(s))` : ''}`
      : '🟢 Nenhum alerta ativo',
  );
  return itens;
}
...
function cockpitAtualizarTicker() {
  ...
  const html = itens
    .map((t) => `<span class="cockpit-ticker-item">${escapeHtmlRelatorio(t)}</span>`)
    .join('');
  track.innerHTML = html + html;
  const label = document.querySelector('#cockpitTicker .cockpit-ticker-label');
  if (label)
    label.title = cockpitState.ultimaAtualizacao
      ? `Atualizado às ${cockpitState.ultimaAtualizacao.toLocaleTimeString('pt-BR')}`
      : 'Ainda sem dados nesta sessão';
}
...
function cockpitAtualizarResumoHome() {
  ...
  el.innerHTML = [
    cockpitKpiCard(
      'Fechado no mês',
      moedaRelatorio(c.resultadoMes.fechadoMes),
      null,
      '',
      `${cockpitND(c.resultadoMes.pctMeta, (v) => `${v}%`)} da meta`,
    ),
    ...
    cockpitKpiCard(
      'Win Rate',
      cockpitND(c.eficiencia.winRate, (v) => `${v}%`),
      null,
    ),
  ].join('');
}
...
function aoTrocarPeriodoCockpit() {
  ...
  document
    .querySelectorAll('.cockpit-periodo-custom')
    .forEach((el) => el.classList.toggle('oculto', preset !== 'personalizado'));
}
...
async function carregarVendedoresCockpit() {
  ...
  try {
    ...
    mostrarErro(
      'Não consegui carregar a lista de vendedores para o Cockpit.\n\nDetalhe técnico: ' +
        e.message,
    );
  }
}
...
async function carregarOrigensCockpit() {
  ...
  try {
    ...
    mostrarErro(
      'Não consegui carregar a lista de origens para o Cockpit.\n\nDetalhe técnico: ' + e.message,
    );
  }
}
...
async function aplicarFiltroProdutoCockpit() {
  ...
  const webhook = document.getElementById('webhook').value.trim();
  if (!cockpitState.deals.length) {
    mostrarErro('Clique em "Atualizar agora" antes de filtrar por produto.');
    return;
  }
  if (status)
    status.textContent =
      'Buscando produtos dos negócios carregados (pode levar alguns segundos)...';
  ...
  for (let i = 0; i < base.length; i++) {
    ...
    try {
      ...
      if (linhas.some((x) => normalizarTextoChave(x.PRODUCT_NAME || '').includes(normTermo)))
        comProduto.push(d);
    } catch (e) {
      /* ignora negócio com erro pontual, não interrompe o filtro */
    }
    await aguardar(60);
  }
  cockpitState.dealsFiltrados = comProduto;
  if (status)
    status.textContent = `${comProduto.length} negócio(s) com produto correspondente a "${escapeHtmlRelatorio(termo)}" entre os ${base.length} carregados.`;
  renderizarCockpit();
}
...
async function atualizarCockpit() {
  ...
  try {
    ...
    atualizarRelogioCockpit();
    atualizarStatus(
      `Cockpit atualizado: ${enriquecidos.length} negócio(s) do Comercial carregado(s).`,
    );
  } catch (e) {
...
function cockpitAgingAtualDias(d, refISO) {
  ...
  return Math.max(
    0,
    Math.floor((new Date(`${refISO}T12:00:00`) - new Date(`${mt}T12:00:00`)) / 86400000),
  );
}
...
function cockpitVerificarElegibilidade(d, refISO) {
  const motivos = [];
  if (ehEstagioPiloto(d.STAGE_ID, d._ESTAGIO))
    motivos.push('Estágio Piloto (fora do critério de elegibilidade)');
  ...
  if (aging != null && aging > COCKPIT_AGING_CRITICO_ELEGIBILIDADE_DIAS) {
    motivos.push(
      `Aging acima do crítico (${aging}d > ${COCKPIT_AGING_CRITICO_ELEGIBILIDADE_DIAS}d)`,
    );
  }
...
function cockpitCalcular() {
  ...
  const ganhosMes = deals.filter(
    (d) => d._SEMANTICA === 'success' && dentroPeriodoCatalogo(d._FECHAMENTO, mes),
  );
  ...
  const abertosMes = deals.filter(
    (d) =>
      d._SEMANTICA === 'process' &&
      !ehEstagioPiloto(d.STAGE_ID, d._ESTAGIO) &&
      dentroPeriodoCatalogo(d.CLOSEDATE, mes),
  );
  let commit = 0,
    bestCase = 0,
    pipelineForecast = 0,
  ...
  abertosMes.forEach((d) => {
    ...
    if (bucket === 'Commit') {
      commit += d._VALOR;
      linhasCommit.push(d);
    } else if (bucket === 'Best Case') {
      bestCase += d._VALOR;
      linhasBest.push(d);
    } else if (bucket === 'Pipeline') {
      pipelineForecast += d._VALOR;
      pipelinePonderado += (d._VALOR * prob) / 100;
      linhasPipe.push(d);
    } else {
      upside += d._VALOR;
      linhasUpside.push(d);
  ...
  const periodoFiltro =
    periodoSelecionado.inicio || periodoSelecionado.fim ? periodoSelecionado : mes;
  ...
  const fechadosPeriodo = deals.filter(
    (d) => d._SEMANTICA !== 'process' && dentroPeriodoCatalogo(d._FECHAMENTO, periodoFiltro),
  );
  ...
  const winRate =
    ganhosPeriodo.length + perdidosPeriodo.length > 0
      ? Math.round(
          (ganhosPeriodo.length / (ganhosPeriodo.length + perdidosPeriodo.length)) * 1000,
        ) / 10
      : null;
  const receitaGanhaPeriodo = ganhosPeriodo.reduce((a, d) => a + d._VALOR, 0);
  const ticketMedioVendido = ganhosPeriodo.length
    ? receitaGanhaPeriodo / ganhosPeriodo.length
    : null;
  const ciclos = ganhosPeriodo
    .map((d) => Number(d._CICLO))
    .filter((n) => Number.isFinite(n))
    .sort((a, b) => a - b);
  const cicloMedia = ciclos.length
    ? Math.round((ciclos.reduce((a, b) => a + b, 0) / ciclos.length) * 10) / 10
    : null;
  const cicloMediana = ciclos.length
    ? ciclos.length % 2
      ? ciclos[(ciclos.length - 1) / 2]
      : Math.round(((ciclos[ciclos.length / 2 - 1] + ciclos[ciclos.length / 2]) / 2) * 10) / 10
    : null;
  ...
  const elegibilidadeTodos = abertosTodos.map((d) => ({
    deal: d,
    check: cockpitVerificarElegibilidade(d, mes.hojeISO),
  }));
  const abertosElegiveis = elegibilidadeTodos
    .filter(
      ({ deal, check }) => check.elegivel && dentroPeriodoCatalogo(deal.CLOSEDATE, periodoFiltro),
    )
    .map(({ deal }) => deal);
  ...
  for (let i = 0; i < 4; i++) {
    ...
    const elegiveisM = abertosTodos.filter(
      (d) =>
        !ehEstagioPiloto(d.STAGE_ID, d._ESTAGIO) &&
        dentroPeriodoCatalogo(d.CLOSEDATE, { inicio: inicioM, fim: fimM }),
    );
    ...
    protecao.push({
      label: `${i === 0 ? 'M' : `M+${i}`} (${mesAnoBR(inicioM)})`,
      meta: metaM,
      pipeline: pipelineM,
      coverage: coverageM,
      coverageRecomendado,
  ...
  abertosTodos.forEach((d) => {
    ...
    if (!porEstagio[k])
      porEstagio[k] = { estagio: k, qtd: 0, valor: 0, agingSoma: 0, agingN: 0, deals: [] };
  ...
  const estagiosLista = Object.values(porEstagio)
    .map((g) => ({
      ...g,
  ...
  estagiosLista.forEach((g, i) => {
    drill[`estagio_${i}`] = g.deals;
  });

  const mesAnteriorObj = new Date(
    Number(mes.inicio.split('-')[0]),
    Number(mes.inicio.split('-')[1]) - 1,
    1,
  );
  ...
  const ganhosMesAnterior = deals.filter(
    (d) => d._SEMANTICA === 'success' && dentroPeriodoCatalogo(d._FECHAMENTO, mesAnterior),
  );
  ...
  function fmtMom(atual, ant) {
    ...
    if (pct === 0)
      return `<span style="color:var(--ink-2); font-weight:bold;">— 0%</span> vs mês anterior`;
    const sinal = pct > 0 ? '▲' : '▼';
  ...
  return {
    ...
    saude: {
      pipelineTotal,
      pipelineElegivel,
      pipelineInelegivelQtd: inelegiveisComMotivo.length,
      coverage,
      coverageRecomendado,
      pipelineCriadoPeriodo,
      ticketMedioPipeline,
      qtdAberto: abertosTodos.length,
      periodoFiltro,
    },
    protecao,
    eficiencia: {
      ...
      perdidos: perdidosPeriodo.length,
      ticketMedioVendido,
      cicloMedia,
      cicloMediana,
      amostraCiclo: ciclos.length,
      periodoFiltro,
    },
    estagios: estagiosLista,
    totalEstagios,
  };
}
...
function cockpitCalcularGeracaoPipeline(c) {
  ...
  const gapGeracao =
    pipelineNecessario != null ? Math.max(0, pipelineNecessario - pipelineCriado) : null;
  const creationCoverage =
    pipelineNecessario != null && pipelineNecessario > 0
      ? pipelineCriado / pipelineNecessario
      : null;
  ...
  if (pipelineNecessario != null && diasUteisTotal > 0) {
    ...
    paceRitmoPct =
      paceEsperadoAteHoje > 0
        ? Math.round((pipelineCriado / paceEsperadoAteHoje) * 1000) / 10
        : null;
  }
...
function cockpitCalcularResumoSdr(c) {
  ...
  const pctViaLead = criadosPeriodo.length
    ? Math.round((viaLead.length / criadosPeriodo.length) * 1000) / 10
    : null;
  cockpitDrill.sdrViaLead = viaLead;
  cockpitDrill.sdrSemLead = semLead;
  return {
    totalCriados: criadosPeriodo.length,
    viaLeadQtd: viaLead.length,
    viaLeadValor: valorViaLead,
    pctViaLead,
    leadsTrabalhados: null,
    reunioes: null,
    conversaoLeadOportunidade: null, // não disponível nesta extração — ver Análise SDR completa
  };
}
...
function cockpitCalcularQualidadeDados(c) {
  ...
  const camposCompletude = [
    {
      label: 'Valor (OPPORTUNITY)',
      pct: pct(comValor.length, baseCompletude.length),
      lista: comValor,
    },
    {
      label: 'Responsável',
      pct: pct(comResponsavel.length, baseCompletude.length),
      lista: comResponsavel,
    },
    { label: 'Estágio', pct: pct(comEstagio.length, baseCompletude.length), lista: comEstagio },
    {
      label: 'CLOSEDATE',
      pct: pct(comCloseDate.length, baseCompletude.length),
      lista: comCloseDate,
    },
    { label: 'Origem', pct: pct(comOrigem.length, baseCompletude.length), lista: comOrigem },
  ];
  camposCompletude.forEach((f, i) => {
    cockpitDrill[`qualidade_${i}`] = f.lista;
  });
  ...
  const dataQualityScore = valoresValidos.length
    ? Math.round((valoresValidos.reduce((a, b) => a + b, 0) / valoresValidos.length) * 10) / 10
    : null;

  return {
    baseTotal: baseCompletude.length,
    campos: camposCompletude,
    motivoPerdaPct,
...
function cockpitStatusProtecao(coverage) {
  if (coverage === null || coverage === undefined || !Number.isFinite(coverage))
    return { rotulo: 'não disponível', classe: '' };
...
function cockpitCalcularAlertas(c, g) {
  ...
  if (c.resultadoMes.gapMeta === 0 && c.resultadoMes.metaMensal > 0) {
    lista.push({
      nivel: 'positivo',
      motivo: 'Meta do mês já foi batida',
      valor: moedaRelatorio(c.resultadoMes.fechadoMes),
      acao: 'Nenhuma ação necessária para bater a meta deste mês — considere reforçar M+1/M+2.',
      tipo: 'scroll',
      alvo: 'cockpitResultadoMes',
    });
    ...
    if (status.rotulo === 'crítico') {
      lista.push({
        nivel: 'critico',
        motivo: 'Coverage do pipeline elegível para a meta do mês está crítico',
        valor: `${c.saude.coverage.toFixed(2)}x (limite crítico <2x)`,
        acao: 'Acelerar geração/qualificação de pipeline elegível para o mês ou revisar a meta.',
        tipo: 'scroll',
        alvo: 'cockpitSaudePipeline',
      });
    } else if (status.rotulo === 'atenção') {
      lista.push({
        nivel: 'atencao',
        motivo: 'Coverage do pipeline elegível para a meta do mês está em atenção',
        valor: `${c.saude.coverage.toFixed(2)}x`,
        acao: 'Monitorar de perto; reforçar geração de pipeline elegível se não melhorar.',
        tipo: 'scroll',
        alvo: 'cockpitSaudePipeline',
      });
    }
  ...
  if (vencidas.length) {
    ...
    lista.push({
      nivel: 'critico',
      motivo: 'Oportunidades abertas com CLOSEDATE vencida (data de fechamento no passado)',
      valor: `${vencidas.length} negócio(s) · ${moedaRelatorio(valorVencidas)}`,
      acao: 'Atualizar a data de fechamento ou mover o negócio para ganho/perda.',
      tipo: 'drill',
      alvo: 'alertaVencidas',
      titulo: 'Oportunidades com CLOSEDATE vencida',
    });
  }
  ...
  if (ticketBase > 0) {
    ...
    if (grandesInativos.length) {
      ...
      lista.push({
        nivel: 'critico',
        motivo: `Grandes negócios (>= 2x Ticket Médio) sem interação há >${diasInativoMax} dias`,
        valor: `${grandesInativos.length} negócio(s) · ${moedaRelatorio(valTotal)} em risco`,
        acao: 'Retomar contato urgentemente para evitar perda da oportunidade (SLA vencido).',
        tipo: 'drill',
        alvo: 'alertaGrandesInativos',
  ...
  (c.estagios || []).forEach((eg, idx) => {
    if (eg.agingMedio != null && eg.agingMedio > ALERTA_AGING_ALTO_DIAS) {
      ...
      lista.push({
        nivel: 'atencao',
        motivo: `Estágio "${eg.estagio}" com aging médio acima de ${ALERTA_AGING_ALTO_DIAS} dias`,
        valor: `${eg.agingMedio}d de aging médio · ${eg.qtd} negócio(s) sem movimento recente`,
        acao: 'Revisar as oportunidades paradas nesse estágio — follow-up ou reclassificação.',
        tipo: 'drill',
        alvo: `estagio_${idx}`,
  ...
  if (g && g.pipelineNecessario != null) {
    if (g.creationCoverage != null && g.creationCoverage < 1) {
      lista.push({
        nivel: g.creationCoverage < 0.5 ? 'critico' : 'atencao',
        motivo: 'Pipeline criado no período está abaixo do necessário para sustentar a Meta M+1',
        valor: `Criado ${moedaRelatorio(g.pipelineCriado)} de ${moedaRelatorio(g.pipelineNecessario)} necessário (${(g.creationCoverage * 100).toFixed(1)}%)`,
        acao: 'Intensificar prospecção/geração de pipeline neste período.',
        tipo: 'scroll',
        alvo: 'cockpitGeracaoPipeline',
      });
    } else if (g.paceRitmoPct != null && g.paceRitmoPct < 80) {
      lista.push({
        nivel: 'atencao',
        motivo:
          'Ritmo de criação de pipeline abaixo do esperado para os dias úteis já decorridos no mês',
        valor: `${g.paceRitmoPct}% do ritmo esperado até hoje`,
        acao: 'Acompanhar diariamente; ritmo abaixo de 100% indica atraso frente ao necessário.',
        tipo: 'scroll',
        alvo: 'cockpitGeracaoPipeline',
      });
    }
  ...
  if (m1 && m1.meta > 0 && m1.status) {
    if (m1.status.rotulo === 'crítico') {
      lista.push({
        nivel: 'critico',
        motivo: 'Coverage de M+1 (Proteção de Receita) está crítico',
        valor: m1.coverage != null ? `${m1.coverage.toFixed(2)}x` : 'não disponível',
        acao: 'Priorizar geração de pipeline elegível para o mês seguinte.',
        tipo: 'scroll',
        alvo: 'cockpitProtecaoTabela',
      });
    } else if (m1.status.rotulo === 'atenção') {
      lista.push({
        nivel: 'atencao',
        motivo: 'Coverage de M+1 (Proteção de Receita) está em atenção — risco de virar crítico',
        valor: m1.coverage != null ? `${m1.coverage.toFixed(2)}x` : 'não disponível',
        acao: 'Monitorar de perto nas próximas semanas.',
        tipo: 'scroll',
        alvo: 'cockpitProtecaoTabela',
      });
    }
  ...
  if (!lista.some((a) => a.nivel === 'critico' || a.nivel === 'atencao')) {
    lista.push({
      nivel: 'positivo',
      motivo: 'Nenhum alerta crítico ou de atenção identificado nos indicadores monitorados agora',
      valor: '',
      acao: 'Manter o ritmo atual e seguir acompanhando os blocos abaixo.',
      tipo: 'scroll',
      alvo: 'cockpit-executivo',
    });
  }
...
function cockpitRenderAlertas(info) {
  ...
  const classe = {
    critico: 'cockpit-alerta-critico',
    atencao: 'cockpit-alerta-atencao',
    positivo: 'cockpit-alerta-positivo',
  };
  el.innerHTML = info.lista
    .map((a) => {
...
function cockpitGerarSituacaoAgora() {
  ...
  if (!cache) {
    mostrarErro(
      'Clique em "↻ Atualizar agora" no Cockpit antes de gerar a Situação Comercial Agora.',
    );
    return;
  }
  const { c, g, alertasInfo } = cache;
  ...
  const dd = String(agora.getDate()).padStart(2, '0'),
    mm = String(agora.getMonth() + 1).padStart(2, '0');
  const hh = String(agora.getHours()).padStart(2, '0'),
    mi = String(agora.getMinutes()).padStart(2, '0');
  ...
  const campos = [
    ...
    [
      'Coverage atual (elegível ÷ gap)',
      c.saude.coverage === 'meta batida'
        ? 'meta já batida'
        : c.saude.coverage != null
          ? `${c.saude.coverage.toFixed(2)}x`
          : 'não disponível',
    ],
    [
      'Coverage recomendado (Win Rate histórico)',
      c.saude.coverageRecomendado != null
        ? `${c.saude.coverageRecomendado.toFixed(2)}x`
        : 'não disponível',
    ],
    ...
    [
      'Sales Cycle (média)',
      c.eficiencia.cicloMedia != null ? `${c.eficiencia.cicloMedia}d` : 'não disponível',
    ],
  ...
  const html = [
    `<div class="cockpit-situacao-grid">` +
      campos
        .map(
          ([label, valor]) =>
            `<div class="cockpit-situacao-item"><span class="cockpit-situacao-label">${escapeHtmlRelatorio(label)}</span><span class="cockpit-situacao-valor">${escapeHtmlRelatorio(String(valor))}</span></div>`,
        )
        .join('') +
      `</div>`,
...
function cockpitRenderAlertasEm(idContainer, info) {
  ...
  el.innerHTML = info.lista
    .map(
      (a) => `<div class="cockpit-alerta ${classe[a.nivel]}">
      <span class="cockpit-alerta-icone">${icone[a.nivel]}</span>
      <div class="cockpit-alerta-corpo">
        <div class="cockpit-alerta-motivo">${escapeHtmlRelatorio(a.motivo)}</div>
        ${a.valor ? `<div class="cockpit-alerta-valor">${escapeHtmlRelatorio(a.valor)}</div>` : ''}
...
function cockpitGerarResumoIA() {
  ...
  if (pct >= 100)
    texto += `O mês está excelente! Já batemos a meta com ${pct}% de atingimento e R$ ${c.resultadoMes.fechadoMes.toLocaleString('pt-BR')} em novos negócios.\n`;
  else if (pct >= 80)
    texto += `Estamos muito perto da meta! Já atingimos ${pct}%. O gap atual é de R$ ${c.resultadoMes.gapMeta.toLocaleString('pt-BR')}.\n`;
  else if (pct > 0)
    texto += `Até agora, alcançamos ${pct}% da meta. Faltam R$ ${c.resultadoMes.gapMeta.toLocaleString('pt-BR')} para chegarmos ao objetivo do mês.\n`;
  ...
  const cov =
    typeof c.saude.coverage === 'number' ? c.saude.coverage.toFixed(2) + 'x' : 'indisponível';
...
function cockpitKpiCard(rotulo, valor, chaveDrill, extraClasse = '', subTexto = '') {
  const clique = chaveDrill
    ? ` onclick="cockpitAbrirDrill('${chaveDrill}','${escapeHtmlRelatorio(rotulo).replace(/'/g, "\\'")}')"`
    : '';
  const cls = chaveDrill ? 'cockpit-kpi cockpit-kpi-clicavel' : 'cockpit-kpi';
  const sub = subTexto
    ? `<div style="font-size:10px; margin-top:4px; color:var(--ink-2); line-height:1.2;">${subTexto}</div>`
    : '';
...
const COCKPIT_CONTAINERS_KPI = [
  'cockpitResultadoMes',
  'cockpitForecast',
  'cockpitSaudePipeline',
  'cockpitEficiencia',
  'cockpitGeracaoPipeline',
  'cockpitSdrResumo',
  'cockpitQualidadeDados',
];
...
function cockpitRenderEstadoVazio() {
  const msg =
    'Sem dados carregados nesta sessão. Preencha o webhook (seção 1, abaixo) e clique em "↻ Atualizar agora".';
  COCKPIT_CONTAINERS_KPI.forEach((id) => {
    const el = cockpitEl(id);
    if (el) el.innerHTML = cockpitPlaceholderVazio(msg);
  });
  COCKPIT_CONTAINERS_LISTA.forEach((id) => {
    const el = cockpitEl(id);
    if (el) el.innerHTML = cockpitPlaceholderVazio(msg);
  });
...
function renderizarCockpit() {
  ...
  cockpitEl('cockpitResultadoMes').innerHTML = [
    cockpitKpiCard(
      'Meta New MRR (mês)',
      c.resultadoMes.metaMensal ? moedaRelatorio(c.resultadoMes.metaMensal) : 'não informada',
      null,
    ),
    cockpitKpiCard(
      'Fechado no mês',
      ...
      'resultadoMesFechado',
      '',
      c.resultadoMes.fechadoMesMom,
    ),
    cockpitKpiCard(
      '% da Meta',
      cockpitND(c.resultadoMes.pctMeta, (v) => `${v}%`),
      'resultadoMesFechado',
    ),
    cockpitKpiCard(
      'Gap para a meta',
      cockpitND(c.resultadoMes.gapMeta, moedaRelatorio),
      'resultadoMesGap',
    ),
    cockpitKpiCard(
      'Negócios ganhos',
      ...
      'resultadoMesFechado',
      '',
      c.resultadoMes.qtdMom,
    ),
    cockpitKpiCard(
      'Ticket médio (mês)',
      cockpitND(c.resultadoMes.ticketMedioMes, moedaRelatorio),
      'resultadoMesFechado',
      '',
      c.resultadoMes.ticketMom,
    ),
  ].join('');
  ...
  cockpitEl('cockpitForecast').innerHTML = [
    ...
    cockpitKpiCard(
      'Best Case (valor cheio)',
      moedaRelatorio(c.forecast.bestCase),
      'forecastBestCase',
    ),
    cockpitKpiCard(
      'Pipeline (bruto)',
      moedaRelatorio(c.forecast.pipelineForecast),
      'forecastPipeline',
    ),
    cockpitKpiCard(
      'Pipeline (ponderado — entra no forecast)',
      moedaRelatorio(c.forecast.pipelinePonderado),
      'forecastPipeline',
    ),
    cockpitKpiCard(
      'Upside (não entra no forecast)',
      moedaRelatorio(c.forecast.upside),
      'forecastUpside',
    ),
    cockpitKpiCard(
      'Forecast total do mês',
      moedaRelatorio(c.forecast.forecastTotal),
      'forecastTotal',
      'cockpit-kpi-destaque',
    ),
    cockpitKpiCard(
      'Gap do Forecast',
      cockpitND(c.forecast.gapForecast, moedaRelatorio),
      'forecastTotal',
    ),
  ].join('');
  ...
  const covTxt =
    c.saude.coverage === 'meta batida'
      ? 'meta já batida'
      : cockpitND(c.saude.coverage, (v) => `${v.toFixed(2)}x`);
  ...
  cockpitEl('cockpitSaudePipeline').innerHTML = [
    ...
    cockpitKpiCard(
      'Pipeline Elegível (filtro)',
      moedaRelatorio(c.saude.pipelineElegivel),
      'pipelineElegivel',
    ),
    cockpitKpiCard(
      'Pipeline inelegível (com motivo)',
      c.saude.pipelineInelegivelQtd,
      'pipelineInelegivel',
    ),
    ...
    cockpitKpiCard(
      ...
      moedaRelatorio(c.saude.pipelineCriadoPeriodo),
      'pipelineCriado',
    ),
    cockpitKpiCard(
      'Ticket médio do pipeline',
      cockpitND(c.saude.ticketMedioPipeline, moedaRelatorio),
      'pipelineTotal',
    ),
  ].join('');
  ...
  cockpitEl('cockpitProtecaoTabela').innerHTML =
    `<table><thead><tr><th>Mês</th><th>Meta</th><th>Pipeline Elegível</th><th>Coverage</th><th>Status (chão fixo)</th><th>Recomendado (Win Rate)</th></tr></thead><tbody>` +
    c.protecao
      .map(
        (p, i) =>
          `<tr class="cockpit-linha-clicavel" onclick="cockpitAbrirDrill('protecao_${i}','Pipeline elegível — ${escapeHtmlRelatorio(p.label)}')">` +
  ...
  cockpitEl('cockpitEstagios').innerHTML =
    c.estagios
      .map(
        (g, i) => `
    <div class="cockpit-estagio-linha" onclick="cockpitAbrirDrill('estagio_${i}','Negócios em ${escapeHtmlRelatorio(g.estagio)}')">
      ...
      .join('') || `<p class="rodape-nota">Nenhum negócio em aberto no Comercial.</p>`;
  ...
  cockpitEl('cockpitEficiencia').innerHTML =
    [
      ...
      cockpitKpiCard(
        ...
        cockpitND(c.eficiencia.ticketMedioVendido, moedaRelatorio),
        'winRateGanhos',
      ),
      cockpitKpiCard(
        'Sales Cycle (média)',
        cockpitND(c.eficiencia.cicloMedia, (v) => `${v}d`),
        'cicloVenda',
      ),
      cockpitKpiCard(
        'Sales Cycle (mediana)',
        cockpitND(c.eficiencia.cicloMediana, (v) => `${v}d`),
        'cicloVenda',
      ),
    ].join('') +
    `<p class="rodape-nota" style="grid-column:1/-1;">Amostra do ciclo de vendas: ${c.eficiencia.amostraCiclo} negócio(s) ganho(s) com data de criação e de fechamento preenchidas, dentro do período do filtro.</p>`;
  ...
  cockpitEl('cockpitGeracaoPipeline').innerHTML = [
    ...
    cockpitKpiCard(
      'Pipeline necessário (Meta M+1 ÷ Win Rate)',
      cockpitND(g.pipelineNecessario, moedaRelatorio),
      null,
    ),
    cockpitKpiCard('Gap de geração', cockpitND(g.gapGeracao, moedaRelatorio), null),
    cockpitKpiCard(
      'Creation Coverage',
      cockpitND(g.creationCoverage, (v) => `${(v * 100).toFixed(1)}%`),
      null,
    ),
    cockpitKpiCard(
      'Dias úteis decorridos / total do mês',
      `${g.diasUteisDecorridos} / ${g.diasUteisTotal}`,
      null,
    ),
    cockpitKpiCard(
      'Esperado até hoje (pace)',
      cockpitND(g.paceEsperadoAteHoje, moedaRelatorio),
      null,
    ),
    cockpitKpiCard(
      'Gap de ritmo',
      g.paceGap != null ? moedaRelatorio(g.paceGap) : 'não disponível',
      null,
    ),
    cockpitKpiCard(
      'Ritmo de criação',
      cockpitND(g.paceRitmoPct, (v) => `${v}%`),
      null,
      g.paceRitmoPct != null && g.paceRitmoPct < 100 ? 'cockpit-status-atencao' : '',
    ),
  ].join('');
  ...
  cockpitEl('cockpitQualidadeDados').innerHTML =
    [
      ...
      cockpitKpiCard(
        'Motivo de perda informado',
        q.perdidosQtd ? '0% (campo não existe no projeto)' : 'sem negócios perdidos no período',
        'qualidadeMotivoPerda',
      ),
      cockpitKpiCard(
        'Data Quality Score',
        cockpitND(q.dataQualityScore, (v) => `${v}%`),
        null,
        'cockpit-kpi-destaque',
      ),
    ].join('') +
    `<p class="rodape-nota" style="grid-column:1/-1;">Base: ${q.baseTotal} negócio(s) (aberto(s), quando houver; senão todos os filtrados). Data Quality Score = completude de cadastro no CRM, não é confiança de forecast.</p>`;
...
function cockpitAbrirDrill(chave, titulo) {
  ...
  if (chave === 'pipelineInelegivel')
    colunas.push({ label: 'Motivo(s) de inelegibilidade', valor: '_MOTIVOS_INELEGIBILIDADE' });
...
function cockpitExigirCache() {
  ...
  if (!cache) {
    mostrarErro(
      'Clique em "↻ Atualizar agora" no Cockpit antes de exportar (nada foi calculado ainda nesta sessão).',
    );
    return null;
  }
  return cache;
...
function cockpitListaKpisExport(cache) {
  ...
  const linhas = [];
  const add = (bloco, indicador, valor, unidade) =>
    linhas.push({ bloco, indicador, valor: valor == null ? 'não disponível' : valor, unidade });
  ...
  add(
    'Forecast',
    'Pipeline (ponderado — entra no forecast)',
    moeda(c.forecast.pipelinePonderado),
    'R$',
  );
  ...
  add(
    ...
    c.saude.coverage === 'meta batida'
      ? 'meta batida'
      : c.saude.coverage != null
        ? String(Math.round(c.saude.coverage * 100) / 100)
        : null,
    'x',
  );
  add(
    ...
    'Coverage recomendado (Win Rate histórico)',
    c.saude.coverageRecomendado != null
      ? String(Math.round(c.saude.coverageRecomendado * 100) / 100)
      : null,
    'x',
  );
  add(
    'Saúde do Pipeline',
    'Pipeline criado no período',
    moeda(c.saude.pipelineCriadoPeriodo),
    'R$',
  );
  ...
  (c.protecao || []).forEach((p) => {
    ...
    add(
      'Proteção de Receita',
      `Coverage — ${p.label}`,
      p.coverage != null ? String(Math.round(p.coverage * 100) / 100) : null,
      'x',
    );
    add('Proteção de Receita', `Status (chão fixo) — ${p.label}`, p.status?.rotulo || null, '');
    add(
      'Proteção de Receita',
      `Recomendado (Win Rate) — ${p.label}`,
      p.coverageRecomendado != null ? String(Math.round(p.coverageRecomendado * 100) / 100) : null,
      'x',
    );
  });
  ...
  add(
    ...
    'Ticket médio vendido',
    moeda(c.eficiencia.ticketMedioVendido),
    'R$',
  );
  ...
  if (g) {
    ...
    add(
      'Geração de Pipeline',
      'Creation Coverage',
      g.creationCoverage != null ? String(Math.round(g.creationCoverage * 1000) / 10) : null,
      '%',
    );
  ...
  if (q) {
    (q.campos || []).forEach((f) =>
      add('Qualidade dos Dados', `Completude — ${f.label}`, pct(f.pct), '%'),
    );
...
function cockpitExportarCSV() {
  ...
  baixarArquivo(
    '﻿' + linhasCSVDe(campos, linhas),
    `cockpit_comercial_kpis_${dataHoje()}.csv`,
...
function cockpitExportarJSON() {
  ...
  baixarArquivo(
    ...
    `cockpit_comercial_${dataHoje()}.json`,
...
function cockpitHtmlKpiBlocos(cache, blocosIncluir) {
  ...
  return Object.entries(porBloco)
    .map(([bloco, itens]) => {
      const cards = itens
        .map(
          (it) =>
            `<div class="kpi"><div class="label">${escapeHtmlRelatorio(it.indicador)}</div><div class="value">${escapeHtmlRelatorio(String(it.valor))}${it.unidade && it.valor !== 'não disponível' ? ` <span style="font-size:.6em;color:var(--muted)">${escapeHtmlRelatorio(it.unidade)}</span>` : ''}</div></div>`,
        )
        .join('');
...
function cockpitGerarHTMLExport(completo) {
  ...
  const agora = new Date();
  const carimbo =
    formatarDataBR(formatarDataISO(agora)) +
    ' ' +
  ...
  corpo += cockpitHtmlKpiBlocos(cache, [
    'Resultado do Mês',
    'Forecast',
    'Saúde do Pipeline',
    'Eficiência da Máquina',
  ...
  if (completo) {
    corpo +=
      `<h2 class="section">Outras análises (relatórios completos do projeto)</h2>` +
  ...
  return (
    `<!doctype html><html lang="pt-BR"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${escapeHtmlRelatorio(titulo)} — ${escapeHtmlRelatorio(marca.nome)}</title><style>${modeloExecutivoCssParaMarca(marca)}</style></head><body>` +
...
function cockpitBaixarRelatorioExecutivo() {
  const h = cockpitGerarHTMLExport(true);
  if (h)
    baixarArquivo(h, `relatorio_executivo_completo_${dataHoje()}.html`, 'text/html;charset=utf-8;');
}
...
function cockpitRenderizarGrafico(c) {
  ...
  const ganhosMes =
    c.resultadoMesFechado ||
    (c.deals || []).filter(
      (d) => d._SEMANTICA === 'success' && dentroPeriodoCatalogo(d._FECHAMENTO, c.mes),
    );
  ...
  graficoCockpitInstance = new Chart(ctx, {
    ...
    data: {
      ...
      datasets: [
        {
          label: 'Fechado Acumulado (R$)',
          data: acumulado,
          borderColor: '#053eff',
          backgroundColor: 'rgba(5, 62, 255, 0.1)',
          fill: true,
          tension: 0.1,
        },
        {
...
function cockpitRenderizarMetasDesdobradas(c) {
  ...
  let salvo = {};
  try {
    salvo = JSON.parse(localStorage.getItem('atlas-metas-desdobradas')) || {};
  } catch (e) {}
...
window.cockpitSalvarMetasIndividuais = function () {
  ...
  const metas = {};
  inputs.forEach((i) => {
    metas[i.dataset.vendedor] = parseFloat(i.value) || 0;
  });
  try {
    localStorage.setItem('atlas-metas-desdobradas', JSON.stringify(metas));
  } catch (e) {}
...
function salvarOrdemLayout() {
  ...
  try {
    localStorage.setItem('atlas-layout-ordem', JSON.stringify(ids));
  } catch (e) {}
}
