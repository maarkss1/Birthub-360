...
function baixarCSVForecastNegocios() {
  baixarCsvDatasetEspecial(
    resultadoForecastSemanal?.negocios,
    `bitrix_forecast_semanal_negocios_${dataHoje()}.csv`,
  );
}
function baixarCSVForecastVendedores() {
  baixarCsvDatasetEspecial(
    resultadoForecastSemanal?.vendedores,
    `bitrix_forecast_semanal_vendedores_${dataHoje()}.csv`,
  );
}
function baixarJSONForecast() {
  if (!resultadoForecastSemanal?.resumo) return;
  baixarArquivo(
    JSON.stringify(resultadoForecastSemanal, null, 2),
    `bitrix_forecast_semanal_${dataHoje()}.json`,
...
function baixarCSVAnaliseSdrDiario() {
  baixarCsvDatasetEspecial(
    resultadoAnaliseSDR?.diario,
    `bitrix_analise_sdr_atividades_diarias_${dataHoje()}.csv`,
  );
}
function baixarCSVAnaliseSdrClientes() {
  baixarCsvDatasetEspecial(
    resultadoAnaliseSDR?.clientes_atividades,
    `bitrix_analise_sdr_clientes_${dataHoje()}.csv`,
  );
}
function baixarCSVAnaliseSdrConversoes() {
  baixarCsvDatasetEspecial(
    resultadoAnaliseSDR?.conversoes,
    `bitrix_analise_sdr_conversoes_${dataHoje()}.csv`,
  );
}
function baixarCSVAnaliseSdrJornada() {
  baixarCsvDatasetEspecial(
    resultadoAnaliseSDR?.jornada_leads,
    `bitrix_analise_sdr_jornada_leads_${dataHoje()}.csv`,
  );
}
function baixarCSVAnaliseSdrBacklog() {
  baixarCsvDatasetEspecial(
    resultadoAnaliseSDR?.backlog,
    `bitrix_analise_sdr_backlog_${dataHoje()}.csv`,
  );
}
function baixarJSONAnaliseSdr() {
  if (!resultadoAnaliseSDR?.resumo) return;
  baixarArquivo(
    JSON.stringify(resultadoAnaliseSDR, null, 2),
    `bitrix_analise_sdr_semanal_mensal_${dataHoje()}.json`,
    'application/json;charset=utf-8;',
  );
}
...
function baixarCSVAtividadesDiarioSDR() {
  baixarCsvDatasetEspecial(
    resultadoDiarioSDR?.atividades_sdr,
    `bitrix_diario_sdr_atividades_${dataHoje()}.csv`,
  );
}
function baixarCSVLeadsAtendidosDiarioSDR() {
  baixarCsvDatasetEspecial(
    resultadoDiarioSDR?.leads_atendidos,
    `bitrix_diario_sdr_leads_atendidos_${dataHoje()}.csv`,
  );
}
function baixarCSVPotenciaisLeadsSDR() {
  baixarCsvDatasetEspecial(
    resultadoDiarioSDR?.potenciais_leads,
    `bitrix_diario_sdr_potenciais_leads_${dataHoje()}.csv`,
  );
}
function baixarCSVPotenciaisNegociosSDR() {
  baixarCsvDatasetEspecial(
    resultadoDiarioSDR?.potenciais_negocios,
    `bitrix_diario_sdr_potenciais_negocios_${dataHoje()}.csv`,
  );
}
function baixarJSONDiarioSDR() {
  if (!resultadoDiarioSDR?.resumo) return;
  baixarArquivo(
    JSON.stringify(resultadoDiarioSDR, null, 2),
    `bitrix_diario_sdr_${dataHoje()}.json`,
    'application/json;charset=utf-8;',
  );
}
...
function baixarCSVEntidade(chave) {
  ...
  baixarArquivo(
    '﻿' + linhasCSVDe(r.campos, r.dados),
    `bitrix_tudo_${chave}_${dataHoje()}.csv`,
...
function baixarJSONEntidade(chave) {
  ...
  baixarArquivo(
    ...
    `bitrix_tudo_${chave}_${dataHoje()}.json`,
    'application/json;charset=utf-8;',
  );
}
...
function baixarJSONCompleto() {
  ...
  Object.entries(resultadoCompleto).forEach(([chave, r]) => {
    combinado[chave] = {
      campos: r.campos,
      total_no_bitrix: r.total,
      registros_extraidos: r.dados.length,
      dados: r.dados,
    };
  });
  baixarArquivo(
    JSON.stringify(combinado, null, 2),
    `bitrix_extracao_completa_${dataHoje()}.json`,
    'application/json;charset=utf-8;',
  );
}
...
function coletarDadosParaPrompt() {
  if (resultadoRelatorioCatalogo && resultadoRelatorioCatalogo.titulo) {
    return {
      modo: `Relatório: ${resultadoRelatorioCatalogo.titulo}`,
      conteudo: resultadoRelatorioCatalogo,
    };
  }
  if (resultadoAnaliseSDR && resultadoAnaliseSDR.resumo) {
    return {
      modo: 'Análise SDR semanal e mensal (atividades, jornada e conversões)',
      conteudo: resultadoAnaliseSDR,
    };
  }
  ...
  if (resultadoDiarioSDR && resultadoDiarioSDR.resumo) {
    return {
      modo: 'Relatório diário SDR (atividades, leads atendidos e potenciais)',
      conteudo: resultadoDiarioSDR,
    };
  }
  if (resultadoCompleto && Object.keys(resultadoCompleto).length) {
    ...
    return {
      modo: 'extração completa (negócios, leads, empresas, contatos, atividades e usuários)',
      conteudo: obj,
    };
  }
  if (dadosExtraidos && dadosExtraidos.length) {
    const selEntidade = document.getElementById('entidade');
    const rotulo = selEntidade.options[selEntidade.selectedIndex]
      ? selEntidade.options[selEntidade.selectedIndex].textContent
      : '';
