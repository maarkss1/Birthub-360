function canalAtividadeSDR(a) {
  const prov = normalizarTextoChave(
    `${a.PROVIDER_TYPE_ID || ''} ${a.PROVIDER_ID || ''} ${a.SUBJECT || ''}`,
  );
...
async function buscarAtividadesPorOwner(
  webhook,
  ownerTypeId,
  ids,
  completado,
...
async function buscarHistoricoEntidadeSDR(webhook, entityTypeId, idsEntidade) {
  ...
  for (const lote of dividirEmLotes(ids, 100)) {
    ...
    while (true) {
      ...
      chunk.forEach((r) => {
        const k = String(
          r.ID || `${entityTypeId}_${r.OWNER_ID}_${r.CREATED_TIME}_${r.STATUS_ID || r.STAGE_ID}`,
        );
        if (!vistos.has(k)) {
          vistos.add(k);
          todos.push(r);
...
function resumoAtividadesPeriodoSDR(atividades, inicio, fim, dealsRelacionados = []) {
  ...
  const leadsComOportunidade = new Set(
    oportunidadesPeriodo.map((d) => String(d.LEAD_ID || '')).filter(Boolean),
  );
...
async function extrairAnaliseSDR(webhook) {
  ...
  try {
    ...
    statusLeads.forEach((s) => {
      mapaStatusLead[String(s.STATUS_ID)] = s;
    });
    ...
    leadsAtribuidos.forEach((l) => {
      leadsAtribuidosMap[String(l.ID)] = l;
    });
    ...
    const leadsMesAtribuidos = leadsAtribuidos.filter((l) =>
      dataDentroFaixa(l.DATE_CREATE, mesInicio, mesFim),
    );
    ...
    const idsLeadsProximaAtividade = [
      ...new Set([...idsJornadaLista, ...leadsAtribuidos.map((l) => String(l.ID))]),
    ];
    const [pendentesLeadsSdr, pendentesDealsSdr] = await Promise.all([
      buscarAtividadesPorOwner(
        webhook,
        1,
        idsLeadsProximaAtividade,
        'N',
        ['ID', 'OWNER_ID', 'TYPE_ID', 'SUBJECT', 'START_TIME', 'DEADLINE'],
        { DEADLINE: 'ASC' },
    ...
    dealsRelacionados.forEach((d) => {
      todosDealsMap[String(d.ID)] = d;
    });
    ...
    const empresasSdrMap = await buscarEntidadesPorIds(
      webhook,
      'crm.company.list',
      idsEmpresasSdr,
      ['ID', 'TITLE'],
    ...
    const infoClienteLead = (lead) => {
      ...
      if (idBitrixValido(lead.COMPANY_ID)) {
        ...
        return {
          key: `COMPANY:${id}`,
          nome: empresasSdrMap[id]?.TITLE || lead.COMPANY_TITLE || lead.TITLE || `Empresa ${id}`,
        };
      }
      const nome =
        lead.COMPANY_TITLE ||
    ...
    const infoClienteDeal = (deal) => {
      ...
      if (idBitrixValido(deal.COMPANY_ID)) {
        ...
        return {
          key: `COMPANY:${id}`,
          nome: empresasSdrMap[id]?.TITLE || deal.TITLE || `Empresa ${id}`,
        };
      }
      ...
      const nome = deal.TITLE || '',
        norm = normalizarTextoChave(nome);
    ...
    const infoClienteContato = (contato) => {
      ...
      const nome = `${contato.NAME || ''} ${contato.LAST_NAME || ''}`.trim(),
        norm = normalizarTextoChave(nome);
      return {
        key: norm ? `CONTATO_NOME:${norm}` : `CONTACT:${contato.ID}`,
        nome: nome || `Contato ${contato.ID}`,
      };
    };
    const infoClienteEmpresa = (id) => {
      const sid = idBitrixString(id);
      if (!sid) return null;
    ...
    atividadesMes.forEach((a) => {
      ...
      bindingsDaAtividade(a).forEach((b) => {
        let info = null;
        if (String(b.OWNER_TYPE_ID) === '1')
          info = infoClienteLead(
            leadsJornadaMap[String(b.OWNER_ID)] || leadsTocadosMap[String(b.OWNER_ID)],
          );
        if (String(b.OWNER_TYPE_ID) === '2')
          info = infoClienteDeal(todosDealsMap[String(b.OWNER_ID)]);
        if (String(b.OWNER_TYPE_ID) === '3')
          info = infoClienteContato(contatosSdrMap[String(b.OWNER_ID)]);
      ...
      [...infos.values()].forEach((info) => {
        if (!aggClientes[info.key])
          aggClientes[info.key] = {
            CLIENTE_KEY: info.key,
            CLIENTE: info.nome,
            ATIVIDADES_MES: 0,
            ATIVIDADES_SEMANA: 0,
            DIAS: new Set(),
        ...
        const dia = parteDataISO(a.END_TIME);
        if (dia) r.DIAS.add(dia);
        const fimAt = String(a.END_TIME || '');
        if (fimAt && (!r.PRIMEIRA || fimAt < r.PRIMEIRA)) r.PRIMEIRA = fimAt;
        if (fimAt && (!r.ULTIMA || fimAt > r.ULTIMA)) r.ULTIMA = fimAt;
      });
    });
    const clientesAtividades = Object.values(aggClientes)
      .map((r) => ({
        CLIENTE_KEY: r.CLIENTE_KEY,
        CLIENTE: r.CLIENTE,
        ATIVIDADES_SEMANA: r.ATIVIDADES_SEMANA,
        ATIVIDADES_MES: r.ATIVIDADES_MES,
        ...
        EMAILS: r.EMAILS,
        WHATSAPP: r.WHATSAPP,
        PRIMEIRA_ATIVIDADE: r.PRIMEIRA,
        PRIMEIRA_ATIVIDADE_BR: formatarDataHoraBR(r.PRIMEIRA),
        ULTIMA_ATIVIDADE: r.ULTIMA,
        ULTIMA_ATIVIDADE_BR: formatarDataHoraBR(r.ULTIMA),
      }))
      .sort(
        (a, b) =>
          b.ATIVIDADES_MES - a.ATIVIDADES_MES || a.CLIENTE.localeCompare(b.CLIENTE, 'pt-BR'),
      );

    const resumoClientesPeriodo = (inicioP, fimP) => {
      const acts = atividadesMes.filter((a) => dataDentroFaixa(a.END_TIME, inicioP, fimP)),
        keys = new Set();
      let semCliente = 0;
      acts.forEach((a) => {
        const infos = clientesPorAtividade[String(a.ID)] || [];
        if (!infos.length) semCliente++;
        infos.forEach((i) => keys.add(i.key));
      });
      const isMes = inicioP === mesInicio;
      const rows = clientesAtividades
        .map((r) => ({ ...r, N: isMes ? r.ATIVIDADES_MES : r.ATIVIDADES_SEMANA }))
        .filter((r) => r.N > 0)
        .sort((a, b) => b.N - a.N);
      return {
        CLIENTES_UNICOS: keys.size,
        ATIVIDADES_SEM_CLIENTE: semCliente,
        MEDIA_ATIVIDADES_POR_CLIENTE: keys.size
          ? Math.round(((acts.length - semCliente) / keys.size) * 100) / 100
          : 0,
        CLIENTES_COM_MULTIPLAS_ATIVIDADES: rows.filter((r) => r.N > 1).length,
        MAIOR_CONCENTRACAO_ATIVIDADES: rows[0]?.N || 0,
        CLIENTE_MAIS_TRABALHADO: rows[0]?.CLIENTE || '',
      };
    };
    ...
    Object.values(histLeadPorId).forEach((arr) =>
      arr.sort((a, b) => String(a.CREATED_TIME).localeCompare(String(b.CREATED_TIME))),
    );

    const histDealPorId = {};
    historicoDeals.forEach((h) => (histDealPorId[String(h.OWNER_ID)] ||= []).push(h));
    Object.values(histDealPorId).forEach((arr) =>
      arr.sort((a, b) => String(a.CREATED_TIME).localeCompare(String(b.CREATED_TIME))),
    );

    const dealsPorLead = {};
    dealsRelacionados.forEach((d) => (dealsPorLead[String(d.LEAD_ID)] ||= []).push(d));
    Object.values(dealsPorLead).forEach((arr) =>
      arr.sort((a, b) => String(a.DATE_CREATE).localeCompare(String(b.DATE_CREATE))),
    );
    ...
    const diario = diasEntreISO(mesInicio, mesFim).map((dia) => {
      ...
      acts.forEach((a) => {
        const infos = clientesPorAtividade[String(a.ID)] || [];
        if (!infos.length) semCliente++;
        infos.forEach((i) => clientes.add(i.key));
      });
      return {
        DATA: dia,
        ...
        ATIVIDADES_SEM_CLIENTE: semCliente,
        MEDIA_ATIVIDADES_POR_CLIENTE: clientes.size
          ? Math.round(((acts.length - semCliente) / clientes.size) * 100) / 100
          : 0,
      };
    });
    ...
    const mix = canais.map((canal) => {
      const count = (arr) =>
        arr.filter((a) => {
          const c = canalAtividadeSDR(a);
          if (canal === 'Outros')
            return !['Ligação', 'Reunião', 'Tarefa', 'E-mail', 'WhatsApp'].includes(c);
          return c === canal;
        }).length;
    ...
    for (const leadId of idsJornadaLista) {
      ...
      linhasJornada.push({
        ...
        EMPRESA: idBitrixValido(lead.COMPANY_ID)
          ? empresasSdrMap[idBitrixString(lead.COMPANY_ID)]?.TITLE || lead.COMPANY_TITLE || ''
          : lead.COMPANY_TITLE || '',
        ...
        GANHOS: ganhosLead,
        PERDAS: perdasLead,
        ATIVIDADE_REALIZADA: textoAtividadeResumo(
          ultimaAtividadeLead[ultimaAtividadeLead.length - 1],
        ),
    ...
    const diasCritico = Math.max(
      1,
      Number(document.getElementById('diasSemAtividadeCriticoSDR')?.value) || 3,
    );
    ...
    const backlog = backlogLeads
      .map((l) => {
        ...
        return {
          ...
          EMPRESA: idBitrixValido(l.COMPANY_ID)
            ? empresasSdrMap[idBitrixString(l.COMPANY_ID)]?.TITLE || l.COMPANY_TITLE || ''
            : l.COMPANY_TITLE || '',
          TITULO: l.TITLE || '',
    ...
    backlog.forEach((x) => {
      ...
      if (!backlogPorStatusObj[key])
        backlogPorStatusObj[key] = { STATUS: key, LEADS: 0, SEM_ATIVIDADE_MES: 0, CRITICOS: 0 };
    ...
    const backlogCriticos = backlog.filter(
      (x) => x.CRITICO === 'S' || x.CRITICO === 'SEM DATA',
    ).length;
    ...
    mostrarErro(
      'Não foi possível montar a Análise SDR semanal/mensal.\n\nDetalhe técnico: ' + e.message,
    );
  } finally {
...
function renderizarAnaliseSDR() {
  ...
  const montarKpis = (obj) =>
    [
      ...
      .map(([rotulo, valor]) => kpiCardHtml(rotulo, valor, 'analiseSdrDiarioTabela'))
      .join('');
  ...
  document.getElementById('analiseSdrConversoesTabela').innerHTML = tabelaRelatorio(
    [
      { label: 'Conversão', valor: 'METRICA' },
      {
        label: 'Semana',
        valor: (x) =>
          `<strong>${x.TAXA_SEMANA}%</strong> <span class="rodape-nota">(${x.NUMERADOR_SEMANA}/${x.DENOMINADOR_SEMANA})</span>`,
        html: true,
      },
      {
        label: 'Mês',
        valor: (x) =>
          `<strong>${x.TAXA_MES}%</strong> <span class="rodape-nota">(${x.NUMERADOR_MES}/${x.DENOMINADOR_MES})</span>`,
        html: true,
      },
    ],
    r.conversoes,
  ...
  document.getElementById('analiseSdrBacklogTabela').innerHTML = tabelaRelatorio(
    [
      ...
      { label: 'Dias sem atividade', valor: 'DIAS_SEM_ATIVIDADE' },
      {
        ...
        valor: (x) =>
          x.ATIVIDADE_NO_MES === 'S'
            ? '<span class="badge-relatorio ok">Sim</span>'
            : '<span class="badge-relatorio alerta">Não</span>',
        html: true,
      },
      { label: 'Próxima atividade', valor: (x) => x.PROXIMA_ATIVIDADE || '—' },
      {
        ...
        valor: (x) =>
          x.CRITICO === 'N'
            ? ''
            : `<span class="badge-relatorio alerta">${escapeHtmlRelatorio(x.CRITICO)}</span>`,
        html: true,
      },
    ],
    r.backlog,
...
function tabelaModelo(headers, rows) {
  return (
    `<div class="model-table-wrap"><table class="model-table"><thead><tr>${headers.map((h) => `<th>${escapeHtmlRelatorio(h.label)}</th>`).join('')}</tr></thead><tbody>` +
    (rows || [])
      .map(
        (r) =>
          `<tr>${headers.map((h) => `<td>${h.html ? h.valor(r) : escapeHtmlRelatorio(h.valor(r) ?? '')}</td>`).join('')}</tr>`,
      )
      .join('') +
    `</tbody></table></div>`
  );
...
function gerarHTMLRelatorioJoao() {
  const r = resultadoAnaliseSDR;
  if (!r?.resumo) return '';
  const s = r.resumo.semana,
    m = r.resumo.mes,
    top = r.clientes_atividades?.[0];
  const insight = m.ATIVIDADES
    ? `${m.ATIVIDADES} atividades no mês correspondem a ${m.CLIENTES_UNICOS} cliente(s) único(s). ${m.CLIENTES_COM_MULTIPLAS_ATIVIDADES} cliente(s) receberam mais de uma atividade.` +
      (top ? ` Maior concentração: ${top.CLIENTE}, com ${top.ATIVIDADES_MES} atividade(s).` : '')
    : 'Sem atividades concluídas no mês.';
  const cards = (r.clientes_atividades || [])
    .slice(0, 100)
    .map(
      (x) =>
  ...
  const diario = tabelaModelo(
    [
      { label: 'Data', valor: (x) => x.DATA_BR || formatarDataBR(x.DATA) },
      { label: 'Dia', valor: (x) => x.DIA_SEMANA },
      { label: 'Atividades', valor: (x) => x.ATIVIDADES },
      { label: 'Clientes únicos', valor: (x) => x.CLIENTES_UNICOS },
      { label: 'Ativ./cliente', valor: (x) => x.MEDIA_ATIVIDADES_POR_CLIENTE },
      { label: 'Ligações', valor: (x) => x.LIGACOES },
      { label: 'Reuniões', valor: (x) => x.REUNIOES },
      { label: 'WhatsApp', valor: (x) => x.WHATSAPP },
    ],
    (r.diario || []).filter((x) => x.ATIVIDADES > 0),
  );
  const conv = tabelaModelo(
    [
      { label: 'Conversão', valor: (x) => x.METRICA },
      {
        label: 'Semana',
        valor: (x) => `${x.TAXA_SEMANA}% (${x.NUMERADOR_SEMANA}/${x.DENOMINADOR_SEMANA})`,
      },
      { label: 'Mês', valor: (x) => `${x.TAXA_MES}% (${x.NUMERADOR_MES}/${x.DENOMINADOR_MES})` },
    ],
    r.conversoes || [],
  );
  const backlog = tabelaModelo(
    [
      { label: 'Cliente', valor: (x) => x.CLIENTE },
      { label: 'Status', valor: (x) => x.STATUS },
      { label: 'Criado em', valor: (x) => formatarDataBR(x.DATE_CREATE) },
      { label: 'Última atividade', valor: (x) => formatarDataHoraBR(x.LAST_ACTIVITY_TIME) },
      { label: 'Dias sem atividade', valor: (x) => x.DIAS_SEM_ATIVIDADE },
      { label: 'Crítico', valor: (x) => x.CRITICO },
    ],
    (r.backlog || []).slice(0, 120),
  );
  const rotas = tabelaModelo(
    [
      { label: 'Rota', valor: (x) => x.ROTA },
      { label: 'Leads', valor: (x) => x.LEADS },
    ],
    (r.rotas || []).slice(0, 50),
  );
  const marca = marcaAtiva();
  return (
    `<!doctype html><html lang="pt-BR"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Análise SDR — ${escapeHtmlRelatorio(r.meta.sdr_nome)} · ${escapeHtmlRelatorio(marca.nome)}</title><style>${modeloExecutivoCssParaMarca(marca)}</style></head><body>` +
...
function abrirRelatorioVisualJoao() {
  const h = gerarHTMLRelatorioJoao();
  if (h) mostrarRelatorioVisualInline(h, 'Análise SDR — João Reis');
}
function baixarHTMLRelatorioJoao() {
  const h = gerarHTMLRelatorioJoao();
  if (h)
    baixarArquivo(
      h,
      `analise_sdr_joao_modelo_atlas_${dataHoje()}.html`,
...
async function extrairDiarioSDR(webhook) {
  ...
  try {
    ...
    const contatosDiarioMap = await buscarEntidadesPorIds(
      webhook,
      'crm.contact.list',
      idsContatosDiario,
      ['ID', 'NAME', 'LAST_NAME', 'COMPANY_ID'],
    );
    const idsEmpresaDeals = [
      ...new Set(
        [
          ...companyIdsDiretos,
    ...
    const empresasDeals = await buscarEntidadesPorIds(
      webhook,
      'crm.company.list',
      idsEmpresaDeals,
      ['ID', 'TITLE'],
    ...
    const [pendentesLeads, pendentesDeals] = await Promise.all([
      buscarAtividadesPorOwner(
        webhook,
        1,
        [...new Set([...leadIdsAtendidos, ...potenciaisLeadsBase.map((l) => String(l.ID))])],
        'N',
        ['ID', 'OWNER_ID', 'TYPE_ID', 'SUBJECT', 'START_TIME', 'DEADLINE'],
        { DEADLINE: 'ASC' },
      ...
      buscarAtividadesPorOwner(
        webhook,
        2,
        [...new Set([...dealIdsAtendidos, ...dealsPotenciaisBrutos.map((d) => String(d.ID))])],
        'N',
        ['ID', 'OWNER_ID', 'TYPE_ID', 'SUBJECT', 'START_TIME', 'DEADLINE'],
        { DEADLINE: 'ASC' },
    ...
    const ultimaAtividadeTexto = (mapaPorId, id) => {
      const acts = (mapaPorId[String(id)] || [])
        .slice()
        .sort((a, b) => String(a.END_TIME).localeCompare(String(b.END_TIME)));
    ...
    const leadsAtendidos = Object.values(leadsAtendidosMap)
      .map((l) => {
        const acts = atividadePorLead[String(l.ID)] || [];
        const prox = proximaPorLead[String(l.ID)];
        return {
    ...
    const nomeSdrPrincipal = String(
      document.getElementById('nomeSdrPrincipal')?.value || 'João Reis',
    ).trim();
    ...
    const contatoPorBindings = (binds) => {
      for (const b of binds) {
        ...
        if (b.OWNER_TYPE_ID === '2') {
          ...
          const nome = deal
            ? nomeContatoObjeto(contatosDiarioMap[idBitrixString(deal.CONTACT_ID)])
            : '';
          if (nome) return nome;
        }
      }
    ...
    const atividadesEnriquecidas = atividades.map((a) => {
      ...
      return {
        ...
        ASSUNTO: a.SUBJECT || '',
        DIRECAO:
          String(a.DIRECTION) === '1' ? 'Entrada' : String(a.DIRECTION) === '2' ? 'Saída' : '',
    ...
    const atividadesSDR = atividadesEnriquecidas.filter(
      (a) => a.EH_RESPONSAVEL_SDR_COMERCIAL === 'S',
    );
    ...
    atividadesSDR.forEach((a) => {
      ...
      binds.forEach((b) => {
        ...
        if (b.startsWith('Negócio:'))
          (dealsPorResp[a.RESPONSAVEL_ID] ||= new Set()).add(b.slice(8));
      });
    });
    ...
    const responsaveis = Object.values(resumoPorResponsavel)
      .sort((a, b) => {
        ...
        return (
          b.ATIVIDADES - a.ATIVIDADES || b.POTENCIAIS_SEM_ATIVIDADE - a.POTENCIAIS_SEM_ATIVIDADE
        );
      })
      .map((r) => ({
        ...r,
    ...
    const potenciaisSemAtividade =
      potenciaisLeads.filter((x) => x.ATENDIDO_NO_PERIODO !== 'S').length +
    ...
    resultadoDiarioSDR = {
      meta: {
        ...
        funis_potenciais: categoriasPotenciais.map((id) =>
          nomeFunilSemCodigo(meta.categorias?.[id] || `Categoria ${id}`),
        ),
    ...
    camposExtraidos = camposDeDados(dadosExtraidos);
    atualizarStatus(
      `Diário SDR concluído: ${atividadesSDR.length} atividade(s) SDR/Comercial; ${leadsAtendidos.length} Lead(s) atendido(s); ${potenciaisSemAtividade} potencial(is) sem atividade no período.`,
    );
  } catch (e) {
...
function renderizarDiarioSDR() {
  ...
  const etapasTexto = r.meta.status_sdr_labels.length
    ? r.meta.status_sdr_labels.join(', ')
    : 'todos os Leads ativos (fallback)';
  ...
  const kpis = [
    ...
    [
      'Potenciais Comercial/SDR',
      r.resumo.POTENCIAIS_NEGOCIOS_COMERCIAL_SDR,
      'diarioSdrPotenciaisNegociosTabela',
    ],
  ...
  document.getElementById('diarioSdrKpis').innerHTML = kpis
  ...
  document.getElementById('diarioSdrResponsaveisTabela').innerHTML = tabelaRelatorio(
    [
      ...
      {
        label: 'Papel',
        valor: (x) => (x.PAPEL === 'SDR' ? '<span class="badge-relatorio ok">SDR</span>' : ''),
        html: true,
      },
      { label: 'Atividades', valor: 'ATIVIDADES' },
      { label: 'Ligações', valor: 'LIGACOES' },
      ...
      { label: 'Potenciais negócios', valor: 'POTENCIAIS_NEGOCIOS' },
      {
        label: 'Sem atividade',
        valor: (x) =>
          x.POTENCIAIS_SEM_ATIVIDADE
            ? `<span class="badge-relatorio alerta">${x.POTENCIAIS_SEM_ATIVIDADE}</span>`
            : '0',
        html: true,
      },
    ],
...
function montarResultadoVisualDiarioSDR() {
  ...
  return {
    ...
    kpis: [
      kpi('Atividades CRM', r.resumo.ATIVIDADES_CRM_REALIZADAS),
      kpi('Atividades SDR/Comercial', r.resumo.ATIVIDADES_SDR_COMERCIAL),
      kpi('Leads atendidos', r.resumo.LEADS_ATENDIDOS),
      kpi('Negócios atendidos', r.resumo.NEGOCIOS_ATENDIDOS),
      kpi('Potenciais Leads SDR', r.resumo.POTENCIAIS_LEADS_SDR),
      kpi('Potenciais Comercial/SDR', r.resumo.POTENCIAIS_NEGOCIOS_COMERCIAL_SDR),
      kpi('Potenciais sem atividade', r.resumo.POTENCIAIS_SEM_ATIVIDADE),
      kpi('Responsáveis', r.resumo.RESPONSAVEIS_MONITORADOS),
    ],
    tabelas: [
      {
        titulo: 'Resumo por SDR / responsável',
        dados: r.responsaveis,
        colunas: [
          { label: 'Responsável', valor: 'RESPONSAVEL' },
          { label: 'Atividades', valor: 'ATIVIDADES' },
          { label: 'Leads atendidos', valor: 'LEADS_ATENDIDOS' },
      ...
      {
        titulo: 'Leads atendidos no dia',
        dados: r.leads_atendidos,
        colunas: [
          { label: 'Lead', valor: 'LEAD_ID' },
          { label: 'Contato', valor: 'CONTATO' },
          { label: 'Empresa', valor: 'EMPRESA' },
          ...
          { label: 'Responsável', valor: 'RESPONSAVEL' },
          { label: 'Atividade realizada', valor: 'ATIVIDADE_REALIZADA' },
          { label: 'Próxima atividade', valor: 'PROXIMA_ATIVIDADE' },
        ],
      },
      {
        titulo: 'Potenciais Leads na esteira SDR',
        dados: r.potenciais_leads,
        colunas: [
          { label: 'Lead', valor: 'LEAD_ID' },
          { label: 'Contato', valor: 'CONTATO' },
          { label: 'Empresa', valor: 'EMPRESA' },
...
function abrirRelatorioVisualDiarioSDR() {
  const h = gerarHTMLRelatorioVisualGenerico(montarResultadoVisualDiarioSDR());
  if (h) mostrarRelatorioVisualInline(h, 'Diário SDR');
}
function baixarHTMLRelatorioVisualDiarioSDR() {
  const h = gerarHTMLRelatorioVisualGenerico(montarResultadoVisualDiarioSDR());
  if (h) baixarArquivo(h, `diario_sdr_modelo_atlas_${dataHoje()}.html`, 'text/html;charset=utf-8;');
}
