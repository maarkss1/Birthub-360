...
function iniciar() {
  ...
  if (selEntidade && selRelatorio) {
    ['negocios', 'leads', 'empresas', 'contatos', 'atividades', 'usuarios', 'tudo'].forEach(
      (chave) => {
  ...
  try {
    ...
    if (salvo) {
      ...
      if (f.inicio && document.getElementById('dataInicio'))
        document.getElementById('dataInicio').value = f.inicio;
      if (f.fim && document.getElementById('dataFim'))
        document.getElementById('dataFim').value = f.fim;
      if (f.preset && document.getElementById('periodoPreset'))
        document.getElementById('periodoPreset').value = f.preset;
      if (f.meta && document.getElementById('cockpitMetaMensal'))
        document.getElementById('cockpitMetaMensal').value = f.meta;
    }
  } catch (e) {}
...
function selecionarCamposProdutos(marcar) {
  document
    .querySelectorAll(
      '#campos-produtos-contexto input[type=checkbox], #campos-produtos-bitrix input[type=checkbox]',
    )
    .forEach((i) => {
      i.checked = marcar;
    });
...
function camposProdutosSelecionados() {
  const marcados = Array.from(
    document.querySelectorAll(
      '#campos-produtos-contexto input[type=checkbox]:checked, #campos-produtos-bitrix input[type=checkbox]:checked',
    ),
  ).map((i) => i.value);
  const extras = document
    .getElementById('camposProdutosExtra')
    .value.split(',')
    .map((s) => s.trim())
    .filter(Boolean);
...
function aoTrocarEntidade(limparRelatorio = false) {
  ...
  document
    .getElementById('bloco-vendedor')
    .classList.toggle('oculto', !['negocios', 'atividades', 'leads'].includes(chave));
  document
    .getElementById('bloco-origem')
    .classList.toggle('oculto', !['negocios', 'leads'].includes(chave));
  document
    .getElementById('bloco-campo-personalizado')
    .classList.toggle('oculto', chave !== 'negocios');

  const labelVendedor = document.querySelector('label[for="vendedor"]');
  if (labelVendedor)
    labelVendedor.textContent =
      chave === 'atividades' ? 'Responsável pela atividade' : 'Vendedor / Responsável';
...
function aoTrocarRelatorio() {
  ...
  if (rel.handler === 'jornada') document.getElementById('nota-jornada').classList.remove('oculto');
  else if (rel.handler === 'forecast_semanal')
    document.getElementById('nota-forecast').classList.remove('oculto');
  else if (rel.handler === 'diario_sdr')
    document.getElementById('nota-diario-sdr').classList.remove('oculto');
  else if (rel.handler === 'analise_sdr')
    document.getElementById('nota-analise-sdr').classList.remove('oculto');
  else {
    ...
    document
      .getElementById('configSlaPrimeiroContato')
      .classList.toggle('oculto', !rel.slaPrimeiroContato);
    document
      .getElementById('configDiasEstagnacaoSDR')
      .classList.toggle('oculto', !rel.diasEstagnacao);
  }
...
async function carregarTodosCampos() {
  ...
  if (!ent.fieldsMethod) {
    mostrarErro(
      'O Bitrix não expõe uma lista dinâmica de campos para esta entidade — use os campos já listados no passo 4 (mais os campos extras, se souber o código).',
    );
    return;
  }
  ...
  try {
    ...
    ent.campos.forEach((c) => {
      rotulosConhecidos[c.code] = c.label;
    });
    ...
    mostrarErro(
      'Não consegui carregar a lista completa de campos.\n\nDetalhe técnico: ' +
        e.message +
        '\n\n' +
        'Se o erro for de permissão, o webhook precisa de acesso de leitura a ' +
        ent.fieldsMethod +
...
function atualizarRelogioTopo() {
  ...
  const d = document.getElementById('dataAtualTopo'),
    h = document.getElementById('horaAtualTopo');
...
function marcarConexaoPendente() {
  const p = document.getElementById('statusConexaoTopo');
  if (!p) return;
  p.classList.remove('ok', 'erro');
  document.getElementById('statusConexaoTexto').textContent = 'Conexão não testada';
}

async function testarConexaoBitrix() {
  const webhook = document.getElementById('webhook').value.trim(),
    err = validarWebhook(webhook),
    pill = document.getElementById('statusConexaoTopo'),
    txt = document.getElementById('statusConexaoTexto');
  if (err) {
    pill?.classList.remove('ok');
    pill?.classList.add('erro');
    if (txt) txt.textContent = 'Webhook inválido';
    mostrarErro(err);
    return false;
  }
  try {
    ...
    pill?.classList.remove('erro');
    pill?.classList.add('ok');
    if (txt) txt.textContent = 'Bitrix conectado';
    atualizarStatus('Conexão com o Bitrix validada com sucesso.');
    return true;
  } catch (e) {
    pill?.classList.remove('ok');
    pill?.classList.add('erro');
    if (txt) txt.textContent = 'Falha na conexão';
    mostrarErro('Não foi possível validar a conexão.\n\n' + e.message);
    return false;
  }
}
...
function abrirAjuda(chave) {
  ...
  document.getElementById('helpTitulo').textContent = a.titulo;
  document.getElementById('helpConteudo').innerHTML = a.html;
  document.getElementById('helpModal').classList.add('aberto');
}
function fecharAjuda() {
  document.getElementById('helpModal')?.classList.remove('aberto');
}
function fecharAjudaPorFundo(e) {
  if (e.target?.id === 'helpModal') fecharAjuda();
}
...
function ativarAcordeoesExtrator() {
  document.querySelectorAll('main .card').forEach((card, idx) => {
    ...
    btn.setAttribute('aria-label', 'Abrir ou fechar seção');
...
function renderizarAtalhosRelatorios() {
  const c = document.getElementById('atalhosRelatoriosGrupos');
  if (!c) return;
  const grupos = {};
  Object.entries(RELATORIOS).forEach(([chave, rel]) => {
    (grupos[rel.grupo] ||= []).push({ chave, ...rel });
  });
...
function filtrarAtalhosRelatorios() {
  const termo = (document.getElementById('buscaAtalhosRelatorios')?.value || '')
    .trim()
    .toLowerCase();
  let visiveis = 0;
  ...
  document
    .getElementById('atalhosRelatoriosSemResultado')
    ?.classList.toggle('oculto', visiveis > 0);
}
...
function selecionarRelatorioRapido(chave) {
  if (!document.getElementById('fluxo-extracao')) {
    ...
    if (bloco) {
      ...
      if (s) {
        s.value = chave;
        if (typeof aoTrocarRelatorio === 'function') aoTrocarRelatorio();
      }
      if (typeof atualizarResumoConfiguracaoV7 === 'function') atualizarResumoConfiguracaoV7();
...
function atualizarResumoConfiguracaoV7() {
  const relKey = document.getElementById('relatorio')?.value,
    entKey = document.getElementById('entidade')?.value;
  const titulo = document.getElementById('resumoSelecaoTitulo'),
    det = document.getElementById('resumoSelecaoDetalhe');
  let nome = 'Extração manual',
    detalhe = ENTIDADES[entKey]?.label || '';
  if (relKey) {
    nome = RELATORIOS[relKey]?.label || relKey;
    detalhe = RELATORIOS[relKey]?.descricao || '';
  }
  if (titulo) titulo.textContent = nome;
  if (det) det.textContent = detalhe;
  const ini = document.getElementById('dataInicio')?.value,
    fim = document.getElementById('dataFim')?.value;
  const chips = [
    `Modo: ${relKey ? 'Relatório' : 'Dados'}`,
    relKey ? nome : detalhe,
    ini || fim
      ? `Período: ${formatarDataBR(ini) || '…'} → ${formatarDataBR(fim) || '…'}`
      : 'Período: todas as datas',
  ];
  const top = document.getElementById('resumoConfiguracaoTopo');
  if (top)
    top.innerHTML = chips
      .map((x) => `<span class="config-chip">${escapeHtmlRelatorio(x)}</span>`)
      .join('');
}

function categorizarCampoV7(code, label) {
  const c = String(code || '').toUpperCase(),
    t = normalizarTextoChave(`${code} ${label}`);
  ...
  if (/company|contact|lead|cliente|empresa|telefone|phone|email|nome|name/.test(t))
    return 'cliente';
  if (
    /stage|status|assigned|respons|opportunity|valor|probability|pipeline|category|currency|closed/.test(
      t,
    )
  )
    return 'comercial';
...
function nomeCategoriaCampoV7(c) {
  return (
    {
      cliente: 'Cliente',
      comercial: 'Comercial',
...
function renderizarListaCampos(lista) {
  const cont = document.getElementById('campos-lista');
  if (!cont) return;
  cont.innerHTML = '';
  lista.forEach((c, idx) => {
    const div = document.createElement('div'),
      cat = categorizarCampoV7(c.code, c.label);
    div.className = 'campo-item';
    div.dataset.categoria = cat;
    div.dataset.padrao = c.padrao ? 'S' : 'N';
    ...
    div.querySelector('input').addEventListener('change', atualizarContadorCampos);
    cont.appendChild(div);
  });
  filtroCamposCategoriaAtual = 'todos';
  aplicarFiltroCampos();
  atualizarContadorCampos();
}
function atualizarContadorCampos() {
  const n = document.querySelectorAll('#campos-lista input[type=checkbox]:checked').length;
  const el = document.getElementById('camposSelecionadosContagem');
  if (el) el.textContent = `${n} campo${n === 1 ? '' : 's'} selecionado${n === 1 ? '' : 's'}`;
}
function filtrarCamposCategoria(cat) {
  filtroCamposCategoriaAtual = cat;
  aplicarFiltroCampos();
}
function selecionarPresetCampos(tipo) {
  document.querySelectorAll('#campos-lista .campo-item').forEach((div) => {
    const cb = div.querySelector('input[type=checkbox]');
    if (tipo === 'essenciais') cb.checked = div.dataset.padrao === 'S';
  });
  atualizarContadorCampos();
}
...
function aplicarFiltroCampos() {
  ...
  document.querySelectorAll('#campos-lista .campo-item').forEach((div) => {
    const texto = div.textContent.toLowerCase(),
      catOk =
        filtroCamposCategoriaAtual === 'todos' ||
        div.dataset.categoria === filtroCamposCategoriaAtual,
      textOk = !filtro || texto.includes(filtro);
...
function selecionarCampos(marcar) {
  document.querySelectorAll('#campos-lista .campo-item').forEach((d) => {
    if (d.style.display !== 'none') {
      const i = d.querySelector('input[type=checkbox]');
      if (i) i.checked = marcar;
    }
  });
  atualizarContadorCampos();
}
...
function prepararCamposSync() {
  ...
  syncRegistroAtual = null;
  syncCamposDisponiveis = {};
  syncAlteracoes = [];
  renderizarPreviewSync();
  const st = document.getElementById('syncRegistroStatus'),
    nm = document.getElementById('syncRegistroNome');
  if (st) st.textContent = 'Nenhum registro carregado';
  if (nm) nm.textContent = 'Informe entidade e ID.';
  atualizarCampoSyncSelecionado();
}

function tipoInputSync(tipo) {
  if (['integer', 'double', 'user', 'crm_company', 'crm_contact', 'crm_lead'].includes(tipo))
    return 'number';
...
function campoSyncEditavel(meta) {
  ...
  return [
    ...
    'crm_contact',
    'crm_lead',
  ].includes(String(meta.type || ''));
}
function rotuloCampoSync(apiKey, meta) {
  return meta?.title || meta?.upperName || apiKey;
}
...
async function bitrixPostJsonComRetentativa(webhook, method, payload) {
  ...
  for (let tentativa = 1; tentativa <= TENTATIVAS_MAX; tentativa++) {
    const controller = new AbortController(),
      timer = setTimeout(() => controller.abort(), TIMEOUT_REQUISICAO_MS);
    try {
      ...
      if (resp.status === 429 || resp.status >= 500 || body.error === 'QUERY_LIMIT_EXCEEDED')
        throw new Error(body.error_description || body.error || `HTTP ${resp.status}`);
      if (body.error || !resp.ok) {
        const e = new Error(
          `${body.error || resp.status} — ${body.error_description || resp.statusText || 'erro'}`,
        );
        e.definitivo = resp.status >= 400 && resp.status < 500 && resp.status !== 429;
        throw e;
      }
      return body;
    } catch (e) {
      ...
      if (tentativa < TENTATIVAS_MAX) await aguardar(Math.min(2 ** (tentativa - 1), 8) * 1000);
    }
  }
...
async function carregarRegistroSync() {
  ...
  const tipo = document.getElementById('syncEntidade').value,
    id = Number(document.getElementById('syncId').value),
    cfg = CAMPOS_SYNC[tipo];
  if (!id) {
    mostrarErro('Informe o ID do registro que deseja carregar.');
    return;
  }
  try {
    document.getElementById('syncRegistroStatus').textContent = 'Carregando...';
    const [itemBody, fieldsBody] = await Promise.all([
      bitrixPostJsonComRetentativa(webhook, 'crm.item.get', {
        entityTypeId: cfg.entityTypeId,
        id,
        useOriginalUfNames: 'N',
      }),
      bitrixPostJsonComRetentativa(webhook, 'crm.item.fields', {
        entityTypeId: cfg.entityTypeId,
        useOriginalUfNames: 'N',
      }),
    ]);
    syncRegistroAtual = itemBody.result?.item || null;
    if (!syncRegistroAtual) throw new Error('Registro não retornado pelo crm.item.get.');
    const todos = fieldsBody.result?.fields || fieldsBody.result || {};
    syncCamposDisponiveis = {};
    Object.entries(todos).forEach(([apiKey, meta]) => {
      if (campoSyncEditavel(meta)) syncCamposDisponiveis[apiKey] = meta;
    });
    const sel = document.getElementById('syncCampo');
    sel.innerHTML = Object.entries(syncCamposDisponiveis)
      .sort((a, b) =>
        rotuloCampoSync(a[0], a[1]).localeCompare(rotuloCampoSync(b[0], b[1]), 'pt-BR'),
      )
      .map(
        ([key, meta]) =>
          `<option value="${escapeHtmlRelatorio(key)}">${escapeHtmlRelatorio(rotuloCampoSync(key, meta))} · ${escapeHtmlRelatorio(meta.upperName || key)}</option>`,
      )
      .join('');
    syncAlteracoes = [];
    renderizarPreviewSync();
    const nome =
      syncRegistroAtual.title ||
      [syncRegistroAtual.name, syncRegistroAtual.lastName].filter(Boolean).join(' ') ||
      `${cfg.label} #${id}`;
    document.getElementById('syncRegistroStatus').textContent = `${cfg.label} #${id} carregado`;
    document.getElementById('syncRegistroNome').textContent =
      `${nome} · ${Object.keys(syncCamposDisponiveis).length} campo(s) editável(is)`;
    atualizarCampoSyncSelecionado();
    document.getElementById('syncLog').textContent =
      `Registro e metadados carregados em ${new Date().toLocaleString('pt-BR')}. Nenhuma alteração enviada.`;
  } catch (e) {
    document.getElementById('syncRegistroStatus').textContent = 'Falha ao carregar';
    document.getElementById('syncRegistroNome').textContent = e.message;
    mostrarErro('Não foi possível carregar o registro para sincronização.\n\n' + e.message);
  }
}

function atualizarCampoSyncSelecionado() {
  const key = document.getElementById('syncCampo')?.value,
    meta = syncCamposDisponiveis[key],
    inp = document.getElementById('syncNovoValor');
  if (!inp) return;
  inp.type = tipoInputSync(meta?.type);
  inp.value = '';
  const atual = key && syncRegistroAtual ? syncRegistroAtual[key] : '';
  inp.placeholder = key
    ? meta?.type === 'boolean'
      ? `Atual: ${atual ?? ''} · use Y ou N`
      : `Atual: ${String(atual ?? '(vazio)').slice(0, 90)}`
    : 'Carregue o registro primeiro';
}
function converterValorSync(valor, meta) {
  const tipo = String(meta?.type || '');
  if (['integer', 'user', 'crm_company', 'crm_contact', 'crm_lead'].includes(tipo))
    return valor === '' ? null : Number.parseInt(valor, 10);
  if (tipo === 'double') return valor === '' ? null : Number(valor);
  if (tipo === 'boolean') {
    const n = String(valor).trim().toUpperCase();
    return ['Y', 'SIM', '1', 'TRUE'].includes(n) ? 'Y' : 'N';
  }
  return valor;
}
function adicionarAlteracaoSync() {
  if (!syncRegistroAtual) {
    mostrarErro('Carregue um registro antes de preparar alterações.');
    return;
  }
  const campo = document.getElementById('syncCampo').value,
    meta = syncCamposDisponiveis[campo];
  if (!campo || !meta) {
    mostrarErro('Selecione um campo editável.');
    return;
  }
  const bruto = document.getElementById('syncNovoValor').value,
    valor = converterValorSync(bruto, meta),
    item = {
      campo,
      label: rotuloCampoSync(campo, meta),
      codigo: meta.upperName || campo,
      atual: syncRegistroAtual[campo] ?? '',
      novo: valor,
    };
  const idx = syncAlteracoes.findIndex((x) => x.campo === campo);
  if (idx >= 0) syncAlteracoes[idx] = item;
  else syncAlteracoes.push(item);
  renderizarPreviewSync();
  atualizarBotaoSync();
}
function removerAlteracaoSync(campo) {
  syncAlteracoes = syncAlteracoes.filter((x) => x.campo !== campo);
  renderizarPreviewSync();
  atualizarBotaoSync();
}
function limparAlteracoesSync() {
  syncAlteracoes = [];
  renderizarPreviewSync();
  atualizarBotaoSync();
}
function renderizarPreviewSync() {
  ...
  if (!el) return;
  if (!syncAlteracoes.length) {
    el.innerHTML =
      '<div class="rodape-nota" style="padding:12px;">Nenhuma alteração preparada.</div>';
    return;
  }
...
function atualizarBotaoSync() {
  const ok =
    document.getElementById('syncHabilitarEscrita')?.checked &&
    document.getElementById('syncConfirmacao')?.value.trim().toUpperCase() === 'SINCRONIZAR' &&
    syncAlteracoes.length > 0 &&
    syncRegistroAtual;
...
function carregarAuditoriaSync() {
  try {
    return JSON.parse(localStorage.getItem(CHAVE_AUDITORIA_SYNC_LOCAL) || '[]');
  } catch (e) {
    return [];
  }
}
function registrarAuditoriaSync(entrada) {
  try {
    ...
    /* localStorage indisponível — segue sem persistir a auditoria */
  }
  renderizarAuditoriaSync();
}
function limparAuditoriaSync() {
  try {
    localStorage.removeItem(CHAVE_AUDITORIA_SYNC_LOCAL);
  } catch (e) {}
  renderizarAuditoriaSync();
}
function renderizarAuditoriaSync() {
  const el = document.getElementById('syncAuditoriaLista');
  if (!el) return;
  const lista = carregarAuditoriaSync();
  if (!lista.length) {
    el.innerHTML =
      '<div class="rodape-nota" style="padding:12px;">Nenhuma sincronização registrada neste navegador ainda.</div>';
    return;
  }
...
async function executarSyncBitrix() {
  atualizarBotaoSync();
  if (document.getElementById('btnExecutarSync').disabled) return;
  const webhook = document.getElementById('webhook').value.trim(),
    err = validarWebhook(webhook);
  if (err) {
    mostrarErro(err);
  ...
  const tipo = document.getElementById('syncEntidade').value,
    ...
    cfg = CAMPOS_SYNC[tipo],
    fields = {};
  syncAlteracoes.forEach((x) => (fields[x.campo] = x.novo));
  const camposResumo = syncAlteracoes
    .map((x) => `${x.codigo}: ${x.atual ?? ''} → ${x.novo ?? ''}`)
    .join('; ');
  try {
    document.getElementById('btnExecutarSync').disabled = true;
    document.getElementById('syncLog').textContent =
      'Enviando alterações ao Bitrix via crm.item.update...';
    const body = await bitrixPostJsonComRetentativa(webhook, 'crm.item.update', {
      entityTypeId: cfg.entityTypeId,
      id,
      fields,
    });
    ...
    registrarAuditoriaSync({
      quando: new Date().toLocaleString('pt-BR'),
      registro: `${cfg.label} #${id}`,
      campos: camposResumo,
      ok: true,
    });
    document.getElementById('syncHabilitarEscrita').checked = false;
    document.getElementById('syncConfirmacao').value = '';
    await carregarRegistroSync();
    atualizarStatus(`${cfg.label} #${id} atualizado no Bitrix.`);
  } catch (e) {
...
function iniciarExperienciaV7() {
  ...
  atualizarResumoConfiguracaoV7();
  ['entidade', 'relatorio', 'periodoPreset', 'dataInicio', 'dataFim'].forEach((id) =>
    document
      .getElementById(id)
      ?.addEventListener('change', () => setTimeout(atualizarResumoConfiguracaoV7, 0)),
  );
  const fc = document.getElementById('filtroCampos');
  fc?.addEventListener('input', atualizarContadorCampos);
}
...
function abrirIAParaRelatorio(tituloRelatorio) {
  document
    .getElementById('central-inteligencia-v10')
...
function renderizarRadarPrioridades() {
  ...
  const parados = abertos
    ...
    .map((r) => ({
      ...r,
      __DIAS_PARADO: Math.floor((agora - new Date(r.MOVED_TIME).getTime()) / 86400000),
    }))
  ...
  parados.slice(0, 12).forEach((r) => {
    const sev =
      r.__DIAS_PARADO >= slaDias * 3
        ? 'critico'
        : r.__DIAS_PARADO >= slaDias * 2
          ? 'atencao'
          : 'info';
    itens.push({
      sev,
      ...
      sub: [
        r.FUNIL,
        r.ESTAGIO_LABEL || r.STAGE_ID,
        r.RESPONSAVEL_ATUAL_NOME,
        r.OPPORTUNITY ? moedaRelatorio(r.OPPORTUNITY) : '',
      ]
        .filter(Boolean)
        .join(' · '),
    });
  ...
  v10RadarItensAtual = parados.map((r) => ({
    ID: r.ID || '',
    Cliente: r.CLIENTE_NOME || r.TITLE || '',
    Funil: r.FUNIL || '',
    Estagio: r.ESTAGIO_LABEL || r.STAGE_ID || '',
    Responsavel: r.RESPONSAVEL_ATUAL_NOME || '',
    Dias_parado: r.__DIAS_PARADO,
    Valor: Number(r.OPPORTUNITY) || 0,
  }));
...
function baixarCSVRadarV10() {
  ...
  baixarArquivo(
    '\uFEFF' + linhasCSVDe(campos, v10RadarItensAtual),
    `radar_prioridades_atlasgr_${dataHoje()}.csv`,
...
function popularSeletorFunilV10() {
  ...
  sel.innerHTML = [...idsVistos.entries()]
    .map(
      ([id, label]) =>
        `<option value="${escapeHtmlRelatorio(id)}">${escapeHtmlRelatorio(nomeFunilSemCodigo(label))}</option>`,
    )
    .join('');
...
function renderizarFunilVisualSankey() {
  ...
  const nodes = colunas.map((c, i) => {
    ...
    return {
      label: c.label,
      x: 30 + i * larguraCol,
      y: margemTopo + (alturaMaxNode - h) / 2,
      h,
      pop: populacao[c.label] || 0,
    };
  });
  ...
  const TOP_FLUXOS = 24;
  let cortados = 0;
  if (fluxos.length > TOP_FLUXOS) {
    cortados = fluxos.length - TOP_FLUXOS;
    fluxos = fluxos.slice(0, TOP_FLUXOS);
  }

  const fluxoMax = Math.max(1, ...fluxos.map((f) => f.valor));
  const espessuraMin = 2,
    espessuraMax = 26;
  ...
  let svgRibbons = '';
  fluxos.forEach((f) => {
    const nA = nodeByLabel.get(f.a),
      nB = nodeByLabel.get(f.b);
  ...
  if (cortados > 0)
    html += `<p class="rodape-nota">Mostrando as ${TOP_FLUXOS} transições mais frequentes (${cortados} fluxo(s) menor(es) omitido(s) para manter o desenho legível).</p>`;
  const semHistorico = registrosSemHistoricoV10(catId);
  if (semHistorico > 0)
    html += `<p class="rodape-nota">${semHistorico} negócio(s) deste funil não tinham histórico de estágio retornado pelo Bitrix — entram na contagem do estágio atual, mas não geram faixas de transição.</p>`;

  alvo.innerHTML = html;
}

function registrosSemHistoricoV10(catId) {
  return (dadosJornadaNormalizada || []).filter(
    (r) => String(r.CATEGORY_ID ?? '') === String(catId) && !v10ParseHistorico(r).length,
  ).length;
}
...
function popularFontesPivotV10() {
  ...
  if (dadosExtraidos && dadosExtraidos.length) {
    v10FontesPivotDisponiveis['extracao_atual'] = {
      registros: dadosExtraidos,
      campos: camposExtraidos.length ? camposExtraidos : camposDeDados(dadosExtraidos),
    ...
    Object.entries(resultadoCompleto).forEach(([chave, r]) => {
      if (r?.dados?.length) {
        ...
        v10FontesPivotDisponiveis['tudo_' + chave] = {
          registros: r.dados,
          campos: r.campos && r.campos.length ? r.campos : camposDeDados(r.dados),
  ...
  if (dadosProdutos && dadosProdutos.length) {
    v10FontesPivotDisponiveis['produtos'] = {
      registros: dadosProdutos,
      campos:
        camposProdutosAtual && camposProdutosAtual.length
          ? camposProdutosAtual
          : camposDeDados(dadosProdutos),
  ...
  if (!chaves.length) {
    sel.innerHTML = '';
    if (resultado)
      resultado.innerHTML = `<div class="v10-vazio"><strong>Nada extraído ainda.</strong>Rode qualquer extração ou relatório lá em cima — este construtor funciona sobre esses dados.</div>`;
    ['v10PivotLinha', 'v10PivotLinha2', 'v10PivotCampoMetrica'].forEach((id) => {
  ...
  sel.innerHTML = chaves
    .map(
      (k) =>
        `<option value="${k}">${escapeHtmlRelatorio(v10FontesPivotDisponiveis[k].label)}</option>`,
    )
    .join('');
...
function aoTrocarFontePivotV10() {
  ...
  const opcoesLinha = campos
    .map((c) => `<option value="${escapeHtmlRelatorio(c)}">${escapeHtmlRelatorio(c)}</option>`)
    .join('');
  ...
  selMetrica.innerHTML = (numericos.length ? numericos : campos)
    .map((c) => `<option value="${escapeHtmlRelatorio(c)}">${escapeHtmlRelatorio(c)}</option>`)
    .join('');
...
function gerarRelatorioConstrutorV10() {
  ...
  const rotuloMetrica =
    metrica === 'contagem'
      ? 'Registros'
      : metrica === 'soma'
        ? `Soma de ${campoMetrica}`
        : `Média de ${campoMetrica}`;
  ...
  linhas.slice(0, 500).forEach((l) => {
    ...
    const valorFmt =
      metrica !== 'contagem' && ehMoeda
        ? moedaRelatorio(l.metrica)
        : (Math.round(l.metrica * 100) / 100).toLocaleString('pt-BR');
  ...
  if (linhas.length > 500)
    html += `<p class="rodape-nota">Mostrando 500 de ${linhas.length} grupos. O CSV contém tudo.</p>`;
...
function baixarCSVConstrutorV10() {
  ...
  baixarArquivo(
    '\uFEFF' + linhasCSVDe(campos, v10PivotResultadoAtual),
    `construtor_relatorio_atlasgr_${dataHoje()}.csv`,
    'text/csv;charset=utf-8;',
  );
}
...
function obterChaveIASalvaV10() {
  try {
    return String(localStorage.getItem(CHAVE_IA_LOCAL) || '').trim();
  } catch (e) {
    return '';
  }
}
...
function salvarChaveIANoNavegadorV10() {
  ...
  try {
    ...
    mostrarErro(
      'Não foi possível salvar a chave. O modo privado do navegador pode estar bloqueando o armazenamento local.',
    );
  }
}
...
function esquecerChaveIASalvaV10() {
  ...
  const confirmar = window.confirm(
    'Esquecer a chave de IA salva neste navegador? O campo atual também será limpo.',
  );
  if (!confirmar) return;
  try {
    localStorage.removeItem(CHAVE_IA_LOCAL);
  } catch (e) {}
  const campo = document.getElementById('v10ChaveIA');
...
function renderizarChatIAV10() {
  ...
  janela.innerHTML =
    v10ChatHistorico
      .map((m) => {
        if (m.role === 'user')
          return `<div class="v10-msg v10-msg-usuario">${escapeHtmlRelatorio(m.content)}</div>`;
        if (m.role === 'erro')
          return `<div class="v10-msg v10-msg-erro">${escapeHtmlRelatorio(m.content)}</div>`;
        return `<div class="v10-msg v10-msg-ia">${escapeHtmlRelatorio(m.content)}</div>`;
      })
      .join('') +
    (v10ChatEnviando
      ? `<div class="v10-msg v10-msg-ia v10-chat-digitando"><span></span><span></span><span></span></div>`
      : '');
...
async function enviarMensagemChatIAV10() {
  ...
  const chave = String(
    document.getElementById('v10ChaveIA')?.value || obterChaveIASalvaV10() || '',
  ).trim();
  if (!chave) {
    mostrarErro('Informe sua chave da API Anthropic nesta aba antes de perguntar.');
    return;
  }

  const pacote = coletarDadosParaPrompt();
  if (!pacote) {
    mostrarErro('Extraia algum dado primeiro (passo 5) antes de conversar com a IA.');
    return;
  }
  esconderErro();
  ...
  const modeloEscolhido =
    document.querySelector('input[name="v10modelo"]:checked')?.value || 'claude-sonnet-5';
  ...
  try {
    ...
    v10ChatHistorico.push({
      role: 'erro',
      content: `Não foi possível obter resposta da IA: ${e.message}`,
    });
  } finally {
...
function ferramentaSincronizar() {
  document
    .getElementById('card-sincronizacao')
    ?.scrollIntoView({ behavior: 'smooth', block: 'start' });
}
...
document.addEventListener('focusin', (e) => {
  ...
  if (
    ...
    (alvo.tagName === 'TEXTAREA' ||
      (alvo.tagName === 'INPUT' && ['text', 'search', 'number', 'password'].includes(alvo.type)))
  ) {
...
function alternarDitado() {
  ...
  if (!reconhecimentoDeVozDisponivel()) {
    document.getElementById('ferramentasVozStatus').textContent =
      'Ditado por voz não é suportado neste navegador (funciona no Chrome/Edge).';
    return;
  }
  ...
  if (!campo) {
    document.getElementById('ferramentasVozStatus').textContent =
      'Clique em um campo de texto antes de ditar.';
    return;
  }
  reconhecimentoDitado = criarReconhecimentoDeVoz(false);
  btn?.classList.add('ativo');
  document.getElementById('ferramentasVozStatus').textContent =
    `Ouvindo... fale para ditar em "${campo.id || campo.name || 'campo de texto'}".`;
...
const COMANDOS_DE_VOZ = [
  {
    ...
    acao: () => extrair(),
    fala: 'Extraindo dados.',
  },
  {
    padroes: ['sincronizar', 'ir para sincronizar', 'sincronização'],
    acao: () => ferramentaSincronizar(),
    fala: 'Abrindo sincronização.',
  },
  { padroes: ['imprimir'], acao: () => ferramentaImprimir(), fala: 'Imprimindo.' },
  {
    padroes: ['baixar', 'baixar csv', 'baixar arquivo'],
    acao: () => ferramentaBaixar(),
    fala: 'Baixando.',
  },
  {
    padroes: ['topo', 'início', 'inicio', 'voltar ao topo'],
    acao: () => document.getElementById('inicio')?.scrollIntoView({ behavior: 'smooth' }),
    fala: 'Voltando ao topo.',
  },
  {
    padroes: ['jornada', 'jornada do cliente'],
    acao: () => selecionarRelatorioRapido('jornada'),
    fala: 'Abrindo Jornada do Cliente.',
  },
  {
    padroes: ['forecast semanal'],
    acao: () => selecionarRelatorioRapido('forecast_semanal'),
    fala: 'Abrindo Forecast semanal.',
  },
  {
    padroes: ['forecast mensal'],
    acao: () => selecionarRelatorioRapido('forecast_mensal'),
    fala: 'Abrindo Forecast mensal.',
  },
  {
    padroes: ['central de inteligência', 'central de inteligencia'],
    acao: () =>
      document.getElementById('central-inteligencia-v10')?.scrollIntoView({ behavior: 'smooth' }),
    fala: 'Abrindo Central de Inteligência.',
  },
  {
    padroes: ['parar', 'parar comando', 'cancelar'],
    acao: () => alternarComandoDeVoz(),
    fala: 'Parando comando de voz.',
  },
];
...
function interpretarComandoDeVoz(transcript) {
  ...
  const comando = COMANDOS_DE_VOZ.find((c) =>
    c.padroes.some((p) => t.includes(normalizarTextoChave(p))),
  );
  if (comando) {
...
function alternarComandoDeVoz() {
  ...
  if (!reconhecimentoDeVozDisponivel()) {
    document.getElementById('ferramentasVozStatus').textContent =
      'Comando de voz não é suportado neste navegador (funciona no Chrome/Edge).';
    return;
  }
  ...
  btn?.classList.add('ativo');
  document.getElementById('ferramentasVozStatus').textContent =
    'Comando de voz ativo — diga "extrair", "sincronizar", "imprimir", "jornada", "forecast semanal"...';
  ...
  reconhecimentoComandoVoz.onerror = (ev) => {
    document.getElementById('ferramentasVozStatus').textContent =
      `Erro no comando de voz: ${ev.error}.`;
  };
