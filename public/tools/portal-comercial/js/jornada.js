...
function limparNomeClienteParaChave(valor) {
  ...
  s = s.replace(
    /\s*[-–—]\s*\((comercial|financeiro|p[oó]s[\s-]*vendas?|implant[aã]ç[aã]o|sucesso do cliente|perfil securit[aá]rio|reembolso|rh|t\.?i\.?)\)\s*[-–—]?\s*$/i,
    '',
  );
  s = s.replace(
    /\s*\((comercial|financeiro|p[oó]s[\s-]*vendas?|implant[aã]ç[aã]o|sucesso do cliente|perfil securit[aá]rio|reembolso|rh|t\.?i\.?)\)\s*$/i,
    '',
  );
  return s.trim();
}
...
async function buscarMetadadosFunisEEstagios(webhook) {
  ...
  if (!dinamico) {
    ENTIDADES.negocios.categorias.forEach((c) => {
      if (c.code !== '') categorias[String(c.code)] = c.label;
    });
    Object.entries(ENTIDADES.negocios.estagiosPorCategoria || {}).forEach(([cat, lista]) => {
      estagios[String(cat)] = {};
      lista.forEach((st) => {
        estagios[String(cat)][String(st.code)] = { label: st.label, semantics: '' };
      });
    });
  }
...
async function buscarHistoricoEstagios(webhook, idsNegocio) {
  ...
  for (const lote of dividirEmLotes(ids, 100)) {
    ...
    while (true) {
      ...
      ['ID', 'OWNER_ID', 'STAGE_ID', 'CATEGORY_ID', 'STAGE_SEMANTIC_ID', 'CREATED_TIME'].forEach(
        (c) => params.append('select[]', c),
      );
...
function construirSinaisDuplicidadeEmpresas(empresasPorId) {
  ...
  Object.values(empresasPorId).forEach((e) => {
    ...
    valoresMulticampo(e, 'EMAIL')
      .map((x) => String(x).trim().toLowerCase())
      .filter(Boolean)
      .forEach((x) => (porEmail[x] ||= new Set()).add(id));
    valoresMulticampo(e, 'PHONE')
...
function renderizarAuditoriaJornada() {
  ...
  document.getElementById('auditoriaJornadaGrid').innerHTML = itens
    .map(
      ([rotulo, valor]) =>
  ...
  document.getElementById('auditoriaJornadaNotas').innerHTML = notas
    .map((n) => `<li>${n}</li>`)
    .join('');
...
function montarResultadoVisualJornada() {
  ...
  const tabelaDe = (dados, limite = 10) =>
    (dados || []).length
      ? camposDeDados(dados)
          .slice(0, limite)
          .map((c) => ({ label: c, valor: c }))
      : [];
  return {
    chave: 'jornada',
    ...
    kpis: [
      kpi('Clientes elegíveis', a.clientesUnicos || 0),
      kpi('Negócios brutos', a.negociosBrutos || 0),
      kpi('Eventos normalizados', a.eventosNormalizados || 0),
      kpi('Duplicatas no pipeline', a.repeticoesMesmoPipeline || 0),
      kpi('Cadastros possiv. duplicados', a.empresasPossivelmenteDuplicadas || 0),
      kpi('Handoffs da jornada', a.handoffsResponsavelCliente || 0),
      kpi('Reentradas em estágio', a.reentradasEstagioHistorico || 0),
      kpi('Mudanças de funil', a.mudancasFunilHistorico || 0),
    ],
    tabelas: [
      {
        titulo: 'Jornada normalizada (amostra de colunas)',
        dados: dadosJornadaNormalizada,
        colunas: tabelaDe(dadosJornadaNormalizada),
      },
      {
        titulo: 'Possíveis duplicidades',
        dados: dadosDuplicidadesJornada,
        colunas: tabelaDe(dadosDuplicidadesJornada),
      },
      {
        titulo: 'Handoffs de responsável',
        dados: dadosHandoffsCliente,
        colunas: tabelaDe(dadosHandoffsCliente),
      },
    ],
    nota: 'Amostra limitada às 10 primeiras colunas de cada dataset; baixe o CSV/JSON para o detalhamento completo.',
  };
}
function abrirRelatorioVisualJornada() {
  const h = gerarHTMLRelatorioVisualGenerico(montarResultadoVisualJornada());
  if (h) mostrarRelatorioVisualInline(h, 'Jornada do Cliente');
}
function baixarHTMLRelatorioVisualJornada() {
  const h = gerarHTMLRelatorioVisualGenerico(montarResultadoVisualJornada());
  if (h)
    baixarArquivo(h, `jornada_cliente_modelo_atlas_${dataHoje()}.html`, 'text/html;charset=utf-8;');
}

function baixarCSVJornadaNormalizada() {
  if (!dadosJornadaNormalizada.length) return;
  baixarArquivo(
    '﻿' + linhasCSVDe(camposExtraidos, dadosJornadaNormalizada),
    `bitrix_jornada_normalizada_${dataHoje()}.csv`,
...
function baixarCSVDuplicidadesJornada() {
  ...
  baixarArquivo(
    '﻿' + linhasCSVDe(camposExtraidos, dadosDuplicidadesJornada),
    `bitrix_jornada_duplicidades_${dataHoje()}.csv`,
    'text/csv;charset=utf-8;',
  );
}
...
function baixarCSVHistoricoEstagios() {
  ...
  baixarArquivo(
    '﻿' + linhasCSVDe(campos, dadosHistoricoEstagios),
    `bitrix_historico_estagios_${dataHoje()}.csv`,
    'text/csv;charset=utf-8;',
  );
}
...
function baixarCSVHandoffsCliente() {
  ...
  baixarArquivo(
    '﻿' + linhasCSVDe(camposExtraidos, dadosHandoffsCliente),
    `bitrix_handoffs_jornada_cliente_${dataHoje()}.csv`,
    'text/csv;charset=utf-8;',
  );
}
...
function diferencaDiasAteReferencia(valor, referenciaISO) {
  const d = parteDataISO(valor),
    ref = parteDataISO(referenciaISO);
  if (!d || !ref) return '';
  return Math.max(
    0,
    Math.floor((new Date(`${ref}T12:00:00`) - new Date(`${d}T12:00:00`)) / 86400000),
  );
}
...
async function listarCompletoRelatorio(
  webhook,
  method,
  campos,
  filtro = {},
...
function tabelaRelatorio(campos, dados, limite = 250) {
  if (!dados?.length)
    return "<p class='rodape-nota' style='padding:10px;'>Nenhum registro encontrado.</p>";
  const lista = dados.slice(0, limite);
  let h = '<table><thead><tr>';
  campos.forEach((c) => {
    h += `<th>${escapeHtmlRelatorio(c.label)}</th>`;
  });
  h += '</tr></thead><tbody>';
  lista.forEach((r) => {
    h += '<tr>';
...
function fecharDataDeal(d) {
  return parteDataISO(d.UF_CRM_1770928318695 || d.CLOSEDATE || '');
}
function valorDeal(d) {
  return Number(d.OPPORTUNITY) || 0;
}
function cicloDealDias(d) {
  const a = parteDataISO(d.DATE_CREATE),
    b = fecharDataDeal(d);
  if (!a || !b) return '';
  return Math.max(
    0,
...
function dentroPeriodoCatalogo(v, p) {
  return !p.inicio && !p.fim ? true : dataDentroFaixa(v, p.inicio, p.fim);
}
...
function chaveHistoricoForecastAtual() {
  return (
    CHAVE_HISTORICO_FORECAST_LOCAL +
    (typeof marcaAtiva === 'function' ? marcaAtiva().sufixoStorage : '')
  );
}
function carregarHistoricoForecastLocal() {
  try {
    return JSON.parse(localStorage.getItem(chaveHistoricoForecastAtual()) || '[]');
  } catch (e) {
    return [];
  }
}
function salvarHistoricoForecastLocal(snapshot) {
  ...
  try {
    ...
    /* localStorage indisponível (modo privado, quota) — segue sem histórico */
  }
}
...
function sparklineHistoricoForecast(historico) {
  ...
  const max = Math.max(
    1,
    ...pts.map((p) => p.metaMensal || 0),
    ...pts.map((p) => p.fechadoMes || 0),
    ...pts.map((p) => p.projecaoMes || 0),
  );
  const stepX = (w - 2 * pad) / (pts.length - 1);
  const coordY = (v) => (h - pad - ((Number(v) || 0) / max) * (h - 2 * pad)).toFixed(1);
  const linha = (campo) =>
    pts.map((p, i) => `${(pad + i * stepX).toFixed(1)},${coordY(p[campo])}`).join(' ');
...
function mesclarHistoricosForecast(compartilhado, local) {
  const porData = {};
  (local || []).forEach((x) => {
    porData[x.data] = { ...x, fonte: 'local' };
  });
  (compartilhado || []).forEach((x) => {
    porData[x.data] = { ...x, fonte: 'automatico' };
  });
...
function graficoEvolucaoForecast(pontos) {
  ...
  const max =
    Math.max(
      1,
      ...pontos.map((p) => p.metaMensal || 0),
      ...pontos.map((p) => p.fechadoMes || 0),
      ...pontos.map((p) => p.projecaoMes || 0),
    ) * 1.08;
  ...
  const linha = (campo) =>
    pontos.map((p, i) => `${coordX(i).toFixed(1)},${coordY(p[campo]).toFixed(1)}`).join(' ');
  const gridN = 4;
  const grid = Array.from({ length: gridN + 1 }, (_, i) => {
    const valor = (max / gridN) * i,
      y = coordY(valor);
  ...
  const rotulosX = pontos
    .map((p, i) =>
      i % cadaN === 0 || i === pontos.length - 1
        ? `<text x="${coordX(i).toFixed(1)}" y="${h - padB + 18}" class="evo-grid-label" text-anchor="middle">${formatarDataBR(p.data).slice(0, 5)}</text>`
        : '',
    )
    .join('');
  const pontosFechado = pontos
    .map(
      (p, i) =>
        `<circle cx="${coordX(i).toFixed(1)}" cy="${coordY(p.fechadoMes).toFixed(1)}" r="3.4" class="evo-point evo-point-fechado"><title>${formatarDataBR(p.data)} (${p.fonte === 'automatico' ? 'automático' : 'local'})\nFechado: ${moedaRelatorio(p.fechadoMes)}\nProjeção: ${moedaRelatorio(p.projecaoMes)}\nMeta: ${moedaRelatorio(p.metaMensal)}</title></circle>`,
    )
    .join('');
...
function tabelaEvolucaoForecast(pontos) {
  ...
  const linhas = [...pontos]
    ...
    .map((p) => {
      const pct =
        p.metaMensal > 0 ? `${Math.round((p.fechadoMes / p.metaMensal) * 1000) / 10}%` : '—';
...
function pontosAtencaoEvolucaoHtml(pontos) {
  const comDados = [...pontos]
    .reverse()
    .find((p) => p.vencidoCount != null || p.semCloseDateCount != null);
  if (!comDados) {
...
async function extrairJornada(webhook) {
  ...
  try {
    ...
    while (true) {
      ...
      atualizarStatus(
        `Buscando negócios em ordem estável de ID... ${acumulado.length}${total !== null ? ' / ' + total : ''} registros`,
      );
      const body = await bitrixFetchComRetentativa(url);
      const chunk = Array.isArray(body.result) ? body.result : Object.values(body.result || {});
      const merge = mesclarSemDuplicarPorId(acumulado, chunk);
    ...
    atualizarStatus(
      `Enriquecendo ${idsEmpresa.length} empresa(s), ${idsContatoSemEmpresa.length} contato(s) e ${idsLead.length} lead(s) em lotes...`,
    );
    const [empresasPorId, contatosPorId, leadsPorId] = await Promise.all([
      buscarEntidadesPorIds(webhook, 'crm.company.list', idsEmpresa, [
        'ID',
        'TITLE',
        'PHONE',
        'EMAIL',
      ...
      buscarEntidadesPorIds(webhook, 'crm.contact.list', idsContatoSemEmpresa, [
        'ID',
        'NAME',
        'LAST_NAME',
        'PHONE',
      ...
      buscarEntidadesPorIds(webhook, 'crm.lead.list', idsLead, [
        'ID',
        'TITLE',
        'ASSIGNED_BY_ID',
        'CREATED_BY_ID',
    ...
    acumulado.forEach((d) => {
      ...
      const registroOperacional =
        tipo === 'DEAL_ID_ISOLADO' && nomePareceOperacionalJornada(d.TITLE || '');
    ...
    if (document.getElementById('incluirHistoricoEstagios')?.checked && !extracaoCancelada) {
      ...
      try {
        dadosHistoricoEstagios = await buscarHistoricoEstagios(
          webhook,
          acumulado.map((d) => d.ID),
        );
      } catch (e) {
        dadosHistoricoEstagios = [];
      }
    ...
    Object.values(historicoPorDeal).forEach((arr) =>
      arr.sort((a, b) => new Date(a.CREATED_TIME || 0) - new Date(b.CREATED_TIME || 0)),
    );
    ...
    Object.entries(grupos).forEach(([clienteKey, deals]) => {
      ...
      deals.forEach((d, idx) => {
        ...
        const empresa = idBitrixValido(d.COMPANY_ID)
          ? empresasPorId[idBitrixString(d.COMPANY_ID)]
          : null;
        const contato =
          !idBitrixValido(d.COMPANY_ID) && idBitrixValido(d.CONTACT_ID)
        ...
        if (!clienteNome && contato)
          clienteNome = `${contato.NAME || ''} ${contato.LAST_NAME || ''}`.trim();
        if (!clienteNome && lead?.TITLE) clienteNome = limparNomeClienteParaChave(lead.TITLE);
        if (!clienteNome)
          clienteNome = limparNomeClienteParaChave(d.TITLE || '') || `Negócio ${d.ID}`;

        const sinalDup = idBitrixValido(d.COMPANY_ID)
          ? sinaisDuplicidade[idBitrixString(d.COMPANY_ID)] || {
              duplicado: false,
              motivos: [],
              ids: [],
            }
        ...
        const responsavelAtual = idBitrixValido(d.ASSIGNED_BY_ID)
          ? idBitrixString(d.ASSIGNED_BY_ID)
          : '';
        const mudouNaJornada = !!(
          responsavelAnterior &&
          responsavelAtual &&
          responsavelAnterior !== responsavelAtual
        );
        const mudouMesmoFunil = !!(
          mudouNaJornada &&
          categoriaAnterior &&
          categoriaAnterior === cat
        );
        const handoffEntreFunis = !!(
          mudouNaJornada &&
          categoriaAnterior &&
          categoriaAnterior !== cat
        );
        ...
        const respLead = idBitrixValido(lead?.ASSIGNED_BY_ID)
          ? idBitrixString(lead.ASSIGNED_BY_ID)
          : '';
        const mudouLeadDeal = !!(respLead && responsavelAtual && respLead !== responsavelAtual);
        if (mudouLeadDeal) trocasResponsavelLeadDeal++;

        if (d.__CLIENTE_KEY_TIPO === 'DEAL_ID_ISOLADO' && !d.__REGISTRO_OPERACIONAL)
          negociosSemVinculo++;
        ...
        linha.DIAS_DESDE_PRIMEIRA_ENTRADA =
          primeiraData && d.DATE_CREATE
        ...
        linha.DUPLICADO_CLIENTE_NO_FUNIL =
          d.__JORNADA_ELEGIVEL && infoContagem.ordem > 1 ? 'S' : 'N';
        linha.CONTA_CLIENTE_UNICO_NO_FUNIL =
          d.__JORNADA_ELEGIVEL && infoContagem.ordem === 1 ? 'S' : 'N';
        ...
        linha.FUNIL_ANTERIOR_JORNADA = categoriaAnterior
          ? meta.categorias[categoriaAnterior] || `Categoria ${categoriaAnterior}`
          : '';
        ...
        linha.FUNIL_ANTERIOR_ELEGIVEL = categoriaAnteriorElegivel
          ? meta.categorias[categoriaAnteriorElegivel] || `Categoria ${categoriaAnteriorElegivel}`
          : '';
        linha.TIPO_TROCA_RESPONSAVEL = mudouMesmoFunil
          ? 'TROCA_MESMO_FUNIL'
          : handoffEntreFunis
            ? 'HANDOFF_ENTRE_FUNIS'
            : '';
        ...
        linha.HISTORICO_ESTAGIOS_SEQUENCIA = histEnriquecido
          .map(
            (h) =>
              `${h.FUNIL_LABEL || ''}::${h.STAGE_LABEL || h.STAGE_ID || ''}@${h.CREATED_TIME || ''}`,
          )
          .join(' > ');
        linha.HISTORICO_ESTAGIOS_JSON = histEnriquecido.length
          ? JSON.stringify(histEnriquecido)
          : '';
        ...
        categoriasHistoricas.forEach((c) => {
          ...
          if (
            !funisHistoricosComprimidos.length ||
            funisHistoricosComprimidos[funisHistoricosComprimidos.length - 1] !== label
          ) {
    ...
    const camposCalculados = [
      'CLIENTE_KEY',
      'CLIENTE_KEY_TIPO',
      'CLIENTE_KEY_CONFIANCA',
      'CLIENTE_CONTAGEM_KEY',
      'CLIENTE_NOME',
      'CLIENTE_NOME_NORMALIZADO',
      'FUNIL_CLASSIFICACAO',
      'REGISTRO_OPERACIONAL',
      'JORNADA_CLIENTE_ELEGIVEL',
      'ORDEM_NA_JORNADA',
      'TOTAL_REGISTROS_CLIENTE',
      'ORDEM_CLIENTE_NO_FUNIL',
      'TOTAL_NEGOCIOS_CLIENTE_NO_FUNIL',
      'CLIENTE_REPETIDO_NO_FUNIL',
      'DUPLICADO_CLIENTE_NO_FUNIL',
      'CONTA_CLIENTE_UNICO_NO_FUNIL',
      'CONTA_COMO_EVENTO_JORNADA',
      'ORDEM_EVENTO_JORNADA',
      'TOTAL_EVENTOS_JORNADA',
      'FUNIL',
      'ESTAGIO_LABEL',
      'STAGE_SEMANTIC_NORMALIZED',
      'DIAS_DESDE_PRIMEIRA_ENTRADA',
      'POSSIVEL_DUPLICIDADE_CADASTRAL',
      'MOTIVO_DUPLICIDADE_CADASTRAL',
      'COMPANY_IDS_RELACIONADOS',
      'RESPONSAVEL_ATUAL_ID',
      'RESPONSAVEL_ATUAL_NOME',
      'RESPONSAVEL_ANTERIOR_JORNADA_ID',
      'RESPONSAVEL_ANTERIOR_JORNADA_NOME',
      'FUNIL_ANTERIOR_JORNADA',
      'TROCA_RESPONSAVEL_DESDE_REGISTRO_ANTERIOR',
      'TROCA_RESPONSAVEL_MESMO_FUNIL',
      'HANDOFF_RESPONSAVEL_ENTRE_FUNIS',
      'HANDOFF_ENVOLVE_FUNIL_INTERNO',
      'HANDOFF_JORNADA_CLIENTE',
      'TROCA_RESPONSAVEL_MESMO_FUNIL_JORNADA',
      'RESPONSAVEL_ANTERIOR_ELEGIVEL_ID',
      'RESPONSAVEL_ANTERIOR_ELEGIVEL_NOME',
      'FUNIL_ANTERIOR_ELEGIVEL',
      'TIPO_TROCA_RESPONSAVEL',
      'QTD_TROCAS_RESPONSAVEL_CLIENTE',
      'QTD_TROCAS_RESPONSAVEL_MESMO_FUNIL_CLIENTE',
      'QTD_HANDOFFS_RESPONSAVEL_CLIENTE',
      'RESPONSAVEL_LEAD_ORIGEM_ID',
      'RESPONSAVEL_LEAD_ORIGEM_NOME',
      'TROCA_RESPONSAVEL_LEAD_PARA_NEGOCIO',
      'CRIADO_POR_NOME',
      'MODIFICADO_POR_NOME',
      'MOVEU_ESTAGIO_POR_NOME',
      'QTD_MOVIMENTOS_ESTAGIO',
      'PRIMEIRO_ESTAGIO_HISTORICO',
      'PRIMEIRO_MOVIMENTO_ESTAGIO_EM',
      'ULTIMO_ESTAGIO_HISTORICO',
      'ULTIMO_MOVIMENTO_ESTAGIO_EM',
      'HISTORICO_ESTAGIOS_SEQUENCIA',
      'HISTORICO_ESTAGIOS_JSON',
      'HISTORICO_FUNIS_SEQUENCIA',
      'QTD_MUDANCAS_FUNIL_HISTORICO',
      'MUDOU_DE_FUNIL_NO_HISTORICO',
      'QTD_REENTRADAS_ESTAGIO_HISTORICO',
    ];
    ...
    dadosDuplicidadesJornada = linhas.filter(
      (r) =>
    ...
    const empresasPossivelmenteDuplicadas = Object.values(sinaisDuplicidade).filter(
      (x) => x.duplicado,
    ).length;
    ...
    auditoriaJornada = {
      ...
      negociosComMudancaFunilHistorico: linhas.filter((r) => r.MUDOU_DE_FUNIL_NO_HISTORICO === 'S')
        .length,
      mudancasFunilHistorico: linhas.reduce(
        (acc, r) => acc + (Number(r.QTD_MUDANCAS_FUNIL_HISTORICO) || 0),
        0,
      ),
      reentradasEstagioHistorico: linhas.reduce(
        (acc, r) => acc + (Number(r.QTD_REENTRADAS_ESTAGIO_HISTORICO) || 0),
        0,
      ),
      negociosSemVinculo,
    ...
    mostrarErro(
      'A extração da jornada do cliente parou por causa de um erro' +
