...
const MARCAS = {
  atlasgr: {
    ...
    corPrimaria: '#FF5618',
    corSecundaria1: '#FF8008',
    corSecundaria2: '#FF6B10',
    senhaHash: '971b5af4a5fda505e27419910527bf48b52b754ca55cc34592a3ea6c4f466d7a',
    sufixoStorage: '',
    prefixoArquivo: '',
    get logoSvg() {
      return typeof MODELO_EXECUTIVO_LOGO !== 'undefined' ? MODELO_EXECUTIVO_LOGO : '';
    },
    get webhookPadrao() {
      return typeof WEBHOOK_FIXO_PADRAO !== 'undefined' ? WEBHOOK_FIXO_PADRAO : '';
    },
  },
  totaltrac: {
    nome: 'Total Trac',
    ...
    corPrimaria: '#008FCE',
    corSecundaria1: '#374898',
    corSecundaria2: '#93DBF2',
    ...
    get logoSvg() {
      return typeof MODELO_EXECUTIVO_LOGO_TOTALTRAC !== 'undefined'
        ? MODELO_EXECUTIVO_LOGO_TOTALTRAC
        : '';
    },
...
const ENTIDADES = {
  negocios: {
    ...
    campos: [
      ...
      {
        code: 'MOVED_TIME',
        label: 'Data última mudança de estágio (usada para calcular dias parado)',
        padrao: true,
      },
  ...
  atividades: {
    ...
    campos: [
      ...
      {
        code: 'TYPE_ID',
        label: 'Tipo (1=Reunião, 2=Ligação, 3=Tarefa, 4=E-mail, 5=Ação, 6=Ação do usuário)',
        padrao: true,
      },
      ...
      {
        code: 'OWNER_TYPE_ID',
        label: 'Tipo da entidade vinculada (1=Lead, 2=Negócio, 3=Contato, 4=Empresa)',
        padrao: true,
      },
  ...
  tudo: {
    label:
      '🔵 Extração completa — tudo do Bitrix (negócios, leads, empresas, contatos, atividades, usuários)',
    especial: true,
    hasCategoria: false,
  },
};
...
const RELATORIOS = {
  jornada: {
    grupo: 'Jornada & Cliente',
    label: '🟣 Jornada do Cliente — completa',
    descricao:
      'Cliente único por pipeline, histórico de estágios, mudança de funil, reentrada, duplicidade, aging e responsáveis.',
    handler: 'jornada',
    periodo: 'todas',
  },
  handoffs: {
    grupo: 'Jornada & Cliente',
    label: '🤝 Handoffs e trocas de responsável',
    descricao:
      'Mudanças observáveis de responsável entre Leads, negócios e passagens entre pipelines.',
    handler: 'catalogo',
    periodo: 'todas',
  },
  reentradas: {
    grupo: 'Jornada & Cliente',
    label: '🔁 Reentradas, retrabalho e mudanças de pipeline',
    descricao: 'Reentradas históricas em estágios e mudanças reais de pipeline.',
    handler: 'catalogo',
    periodo: 'todas',
  },
  duplicidades: {
    grupo: 'Jornada & Cliente',
    label: '🧬 Duplicidades e identidade do cliente',
    descricao:
      'Clientes repetidos no pipeline e possíveis empresas duplicadas por nome, e-mail ou telefone.',
    handler: 'catalogo',
    periodo: 'todas',
  },
  implantacao_posvenda: {
    grupo: 'Jornada & Cliente',
    label: '🚀 Implantação, Onboarding e Pós-Venda',
    descricao: 'Backlog e aging dos pipelines posteriores ao Comercial.',
    handler: 'catalogo',
    periodo: 'todas',
  },

  forecast_semanal: {
    grupo: 'Comercial & Receita',
    label: '📈 Forecast semanal — Comercial',
    descricao:
      'Fechado, Commit, Best Case, pipeline ponderado, previsão por vendedor e higiene de CLOSEDATE.',
    handler: 'forecast_semanal',
    periodo: 'semana_atual',
  },
  forecast_mensal: {
    grupo: 'Comercial & Receita',
    label: '🗓️ Forecast mensal — Comercial',
    descricao: 'Forecast do mês, buckets de confiança e gap para a meta opcional.',
    handler: 'catalogo',
    periodo: 'mensal',
    meta: true,
  },
  pipeline_coverage: {
    grupo: 'Comercial & Receita',
    label: '🎯 Pipeline & Coverage — 30/60/90 dias',
    descricao: 'Pipeline aberto, ponderado, horizontes de fechamento e cobertura da meta.',
    handler: 'catalogo',
    periodo: 'mensal',
    meta: true,
  },
  conversao_comercial: {
    grupo: 'Comercial & Receita',
    label: '🧭 Conversão Comercial — funil e Win Rate',
    descricao: 'Coorte de oportunidades, ganhos/perdas e conversão histórica por estágio.',
    handler: 'catalogo',
    periodo: 'mensal',
  },
  aging_sla: {
    grupo: 'Comercial & Receita',
    label: '⏱️ Aging & SLA Comercial',
    descricao: 'Backlog por tempo no estágio atual e SLA configurável.',
    handler: 'catalogo',
    periodo: 'todas',
    slaAging: true,
  },
  performance_vendedores: {
    ...
    descricao: 'Pipeline, ganhos, perdas, receita, ticket, Win Rate e ciclo por responsável atual.',
    handler: 'catalogo',
    periodo: 'mensal',
  },
  ganhos_perdas_ciclo: {
    grupo: 'Comercial & Receita',
    label: '🏁 Ganhos, perdas e ciclo de vendas',
    descricao: 'Fechamentos, receita ganha, valor perdido, ticket e ciclo de venda.',
    handler: 'catalogo',
    periodo: 'mensal',
  },
  origens_canais: {
    grupo: 'Comercial & Receita',
    label: '🛰️ Origens, canais e conversão',
    descricao: 'Leads, oportunidades, ganhos e receita por SOURCE_ID ou UTM_SOURCE.',
    handler: 'catalogo',
    periodo: 'mensal',
  },
  produtos_receita: {
    grupo: 'Comercial & Receita',
    label: '📦 Produtos e receita',
    descricao: 'Produtos presentes em negócios ganhos, unidades, negócios e valor das linhas.',
    handler: 'catalogo',
    periodo: 'mensal',
  },
  clientes_receita: {
    grupo: 'Comercial & Receita',
    label: '🏢 Clientes, receita e concentração',
    descricao: 'Receita por cliente, recorrência, ticket médio e concentração Top 10.',
    handler: 'catalogo',
    periodo: 'mensal',
  },
  ...
  diario_sdr: {
    grupo: 'SDR & Leads',
    label: '📅 Diário SDR — atividades, Leads atendidos e potenciais',
    descricao: 'Atividades concluídas, Leads atendidos e potenciais ainda sem atividade.',
    handler: 'diario_sdr',
    periodo: 'diario',
  },
  analise_sdr: {
    grupo: 'SDR & Leads',
    label: '📊 João Reis — análise semanal e mensal',
    descricao: 'Produção diária do João, mix de atividades, jornada, backlog e conversões.',
    handler: 'analise_sdr',
    periodo: 'mensal',
  },
  funil_leads: {
    grupo: 'SDR & Leads',
    label: '🪜 Funil de Leads & conversão SDR',
    descricao: 'Leads criados, status, desqualificação, oportunidades e ganhos.',
    handler: 'catalogo',
    periodo: 'mensal',
  },
  produtividade_atividades: {
    grupo: 'SDR & Leads',
    label: '⚡ Produtividade de atividades por responsável',
    descricao: 'Atividades concluídas por usuário/canal e entidades únicas tocadas.',
    handler: 'catalogo',
    periodo: 'mensal',
  },
  sla_primeiro_contato: {
    grupo: 'SDR & Leads',
    label: '☎️ SLA de primeiro contato',
    descricao: 'Tempo da criação do Lead até a primeira atividade concluída vinculada.',
    handler: 'catalogo',
    periodo: 'mensal',
    slaPrimeiroContato: true,
  },
  auditoria_sdr: {
    grupo: 'SDR & Leads',
    label: '🧪 Auditoria SDR — validar dados e plano',
    descricao:
      'Leads sem atividade, atividades sem resultado registrado e completude de campos-chave do trabalho de SDR.',
    handler: 'catalogo',
    periodo: 'mensal',
  },
  decisao_final_sdr: {
    grupo: 'SDR & Leads',
    label: '🧭 Decisão Final SDR — saneamento seguro',
    descricao:
      'Leads estagnados classificados em ação recomendada (recontatar, desqualificar, escalar ou nutrir), sem escrever no Bitrix.',
    handler: 'catalogo',
    periodo: 'todas',
    diasEstagnacao: true,
  },
  ...
  atividades_pendentes: {
    grupo: 'Operação & Qualidade',
    label: '📌 Atividades pendentes e atrasadas',
    descricao: 'Backlog de atividades abertas, atrasadas, sem prazo e por responsável.',
    handler: 'catalogo',
    periodo: 'todas',
  },
  qualidade_crm: {
    grupo: 'Operação & Qualidade',
    label: '🧹 Qualidade do CRM & campos faltantes',
    descricao: 'Completude de Negócios e Leads nos campos operacionais já mapeados.',
    handler: 'catalogo',
    periodo: 'todas',
  },
};
...
const SUBENTIDADES_TUDO = [
  {
    chave: 'negocios',
    label: 'Negócios (todos os funis/pipelines)',
    method: 'crm.deal.list',
    fieldsMethod: 'crm.deal.fields',
  ...
  {
    chave: 'atividades',
    label: 'Atividades (ligações, reuniões e tarefas de CRM)',
    method: 'crm.activity.list',
    fieldsMethod: 'crm.activity.fields',
    campoData: 'CREATED',
  },
  {
    ...
    method: 'user.get',
    fieldsMethod: null,
    camposFixos: CAMPOS_USUARIO_COMPLETO,
    semFiltroData: true,
  },
];
...
function baixarCSV() {
  ...
  baixarArquivo(
    '﻿' + linhas.join('\r\n'),
    `bitrix_${chaveEnt}_${dataHoje()}.csv`,
...
function baixarJSON() {
  ...
  baixarArquivo(
    ...
    `bitrix_${chaveEnt}_${dataHoje()}.json`,
