...
function baixarRelatorioVisualAtual() {
  ...
  const nomeArquivo =
    normalizarTextoChave(relatorioVisualAtualNome || 'relatorio').replace(/ /g, '_') || 'relatorio';
  baixarArquivo(
    relatorioVisualAtualHtml,
    `${nomeArquivo}_${dataHoje()}.html`,
...
function baixarRelatorioVisualPDF() {
  ...
  try {
    ...
    mostrarErro(
      'Não foi possível abrir a impressão automaticamente. Use "Baixar" e abra o arquivo .html para imprimir/salvar como PDF.',
    );
  }
}
...
function carregarRelatoriosSalvos() {
  try {
    return JSON.parse(localStorage.getItem(CHAVE_RELATORIOS_SALVOS_LOCAL) || '[]');
  } catch (e) {
    return [];
  }
}
function salvarRelatorioVisualAtual() {
  ...
  try {
    ...
    lista.unshift({
      id: String(Date.now()),
      nome: relatorioVisualAtualNome,
      quando: new Date().toLocaleString('pt-BR'),
      html: relatorioVisualAtualHtml,
    });
    localStorage.setItem(CHAVE_RELATORIOS_SALVOS_LOCAL, JSON.stringify(lista.slice(0, 15)));
  } catch (e) {
    mostrarErro(
      'Não foi possível salvar — armazenamento local cheio ou indisponível neste navegador.',
    );
    return;
  }
...
function excluirRelatorioSalvo(id) {
  ...
  try {
    localStorage.setItem(CHAVE_RELATORIOS_SALVOS_LOCAL, JSON.stringify(lista));
  } catch (e) {}
  renderizarRelatoriosSalvos();
}
...
function renderizarRelatoriosSalvos() {
  ...
  const lista = carregarRelatoriosSalvos();
  if (!lista.length) {
    el.innerHTML = '<p class="small-note">Nenhum relatório salvo neste navegador ainda.</p>';
    return;
  }
  el.innerHTML = lista
    .map(
...
function somarModelo(rows, campo = 'VALOR') {
  return (rows || []).reduce((a, x) => a + (Number(x[campo]) || 0), 0);
}
function agruparPorModelo(rows, fn) {
  const o = {};
  (rows || []).forEach((x) => (o[fn(x)] ||= []).push(x));
  return o;
}
function origemLabelModelo(id, mapa) {
  const s = String(id || '').trim();
  return s ? mapa?.[s] || s : 'origem não informada';
}
...
async function construirDadosModeloForecast(webhook, meta, inicio, fim, dealsComercial) {
  ...
  if (catsFin.length) {
    ...
    fin = (
      await listarCompletoRelatorio(
        webhook,
        'crm.deal.list',
        campos,
        filtro,
        { ID: 'ASC' },
        'Forecast visual: Financeiro...',
      )
    ).dados;
  }
  ...
  const mapRow = (d) => {
    const cat = String(d.CATEGORY_ID ?? ''),
      sm = meta.estagios?.[cat]?.[String(d.STAGE_ID)] || {};
    const data = parteDataISO(d.MOVED_TIME) || parteDataISO(d.DATE_CREATE);
    const criacao = parteDataISO(d.DATE_CREATE);
    return {
      DEAL_ID: d.ID,
      CLIENTE_KEY: chaveClienteDealModelo(d),
      CLIENTE: idBitrixValido(d.COMPANY_ID)
        ? empresas[idBitrixString(d.COMPANY_ID)]?.TITLE || d.TITLE || ''
        : d.TITLE || '',
      ESTAGIO: sm.label || d.STAGE_ID || '',
      SEMANTICA: semanticaDeal(d, sm),
      RESPONSAVEL:
        nomeUsuario(d.ASSIGNED_BY_ID) ||
        (d.ASSIGNED_BY_ID ? `ID ${d.ASSIGNED_BY_ID}` : 'Sem responsável'),
      ORIGEM: origemLabelModelo(d.SOURCE_ID, origens),
      VALOR: Number(d.OPPORTUNITY) || 0,
      DATA_MOVIMENTO: data,
      DATA_MOVIMENTO_BR: formatarDataBR(data),
      MES_CHAVE: chaveMesISO(data),
      MES_LABEL: mesAnoBR(data),
      ...
      DATA_CRIACAO_BR: formatarDataBR(criacao),
      CICLO_DIAS: criacao && data ? diferencaDiasAteReferencia(criacao, data) : '',
    };
  };
  const comercial = (dealsComercial || []).map(mapRow),
    financeiro = fin.map(mapRow);
  const ehFechado = (x) => {
    const n = normalizarTextoChave(x.ESTAGIO);
    return n.includes('contrato assinado') || (x.SEMANTICA === 'success' && n.includes('assin'));
  };
  const fechados = financeiro.filter(
    (x) => ehFechado(x) && dataDentroFaixa(x.DATA_MOVIMENTO, inicio, fim),
  );
  ...
  const pendentes = financeiro.filter((x) => {
    ...
    return (
      x.SEMANTICA === 'process' &&
      (n.includes('aguardando assinatura') ||
        n.includes('analise de documentos') ||
        n.includes('assinatura de contrato')) &&
      !n.includes('contrato assinado') &&
      !ehEstagioPiloto(null, x.ESTAGIO) &&
      dentroJanela60d(x)
    );
  });
  const pipeline = comercial.filter(
    (x) => x.SEMANTICA === 'process' && !ehEstagioPiloto(null, x.ESTAGIO) && dentroJanela60d(x),
  );
  const porVend = (rows) => {
    const m = {};
    rows.forEach((x) => {
      const k = x.RESPONSAVEL;
      if (!m[k]) m[k] = { RESPONSAVEL: k, VALOR: 0, NEGOCIOS: 0, CLIENTES: new Set() };
      m[k].VALOR += x.VALOR;
      m[k].NEGOCIOS++;
      m[k].CLIENTES.add(x.CLIENTE_KEY);
    });
  ...
  const media = (rows, campo) => {
    const v = rows.map((x) => Number(x[campo])).filter((n) => Number.isFinite(n));
    return v.length ? Math.round((v.reduce((a, b) => a + b, 0) / v.length) * 10) / 10 : 0;
  };
  const porOrigem = (rows) => {
    const m = {};
    rows.forEach((x) => {
      m[x.ORIGEM] = (m[x.ORIGEM] || 0) + x.VALOR;
    });
  ...
  const porEstagio = (rows) => {
    const m = {};
    rows.forEach((x) => {
      if (!m[x.ESTAGIO]) m[x.ESTAGIO] = { ESTAGIO: x.ESTAGIO, VALOR: 0, NEGOCIOS: 0 };
      m[x.ESTAGIO].VALOR += x.VALOR;
      m[x.ESTAGIO].NEGOCIOS++;
    });
  ...
  const inicioAnoPassado = anoAtras(inicio),
    fimAnoPassado = anoAtras(fim);
  const fechadosAnoPassado = financeiro.filter(
    (x) => ehFechado(x) && dataDentroFaixa(x.DATA_MOVIMENTO, inicioAnoPassado, fimAnoPassado),
  );
  const valorAnoPassado = somarModelo(fechadosAnoPassado),
    valorAtual = somarModelo(fechados);
  const comparativoAno = {
    anoAtualLabel: inicio ? inicio.slice(0, 4) : '',
    anoPassadoLabel: inicioAnoPassado ? inicioAnoPassado.slice(0, 4) : '',
    valorAtual,
    valorAnoPassado,
    negociosAtual: fechados.length,
    negociosAnoPassado: fechadosAnoPassado.length,
    deltaPct:
      valorAnoPassado > 0
        ? Math.round(((valorAtual - valorAnoPassado) / valorAnoPassado) * 1000) / 10
        : null,
  };
  return {
    ...
    vendedores_fechados: porVend(fechados),
    vendedores_pipeline: porVend(pipeline),
    pipeline_por_estagio: porEstagio(pipeline),
    comparativo_ano: comparativoAno,
    resumo: {
      FECHADOS_VALOR: somarModelo(fechados),
      FECHADOS_NEGOCIOS: fechados.length,
      FECHADOS_CLIENTES: contarClientesUnicosModelo(fechados),
      PENDENTES_VALOR: somarModelo(pendentes),
      PENDENTES_NEGOCIOS: pendentes.length,
      PENDENTES_CLIENTES: contarClientesUnicosModelo(pendentes),
      PIPELINE_VALOR: somarModelo(pipeline),
      PIPELINE_NEGOCIOS: pipeline.length,
      PIPELINE_CLIENTES: contarClientesUnicosModelo(pipeline),
      TICKET_MEDIO_FECHADOS: fechados.length ? somarModelo(fechados) / fechados.length : 0,
      MAIOR_FECHADO_VALOR: maiorFechado ? maiorFechado.VALOR : 0,
      MAIOR_FECHADO_CLIENTE: maiorFechado ? maiorFechado.CLIENTE : '',
      CICLO_MEDIO_FECHADOS_DIAS: media(fechados, 'CICLO_DIAS'),
      DIAS_MEDIO_PENDENTES: media(pendentes, 'DIAS_NO_ESTAGIO'),
      DIAS_MEDIO_PIPELINE: media(pipeline, 'DIAS_NO_ESTAGIO'),
      TOP_ORIGEM_LABEL: topOrigem ? topOrigem[0] : '',
      TOP_ORIGEM_VALOR: topOrigem ? topOrigem[1] : 0,
    },
  };
}

function renderBarModelo(
  rows,
  titulo = 'Visão geral por vendedor(a)',
  rotuloCampo = 'RESPONSAVEL',
) {
  if (!rows?.length) return `<p class="small-note">Sem dados no período.</p>`;
  const max = Math.max(1, ...rows.map((x) => x.VALOR || 0)),
    cores = ['#2a78d6', '#eb6834', '#1baf7a', '#eda100', '#e87ba4', '#7254c7'];
  return (
    `<div class="mini-chart"><div class="mini-chart-title">${escapeHtmlRelatorio(titulo)}</div>` +
    rows
      .map(
        (x, i) =>
          `<div class="barrow"><div class="barlabel">${escapeHtmlRelatorio(x[rotuloCampo])}</div><div class="bartrack"><div class="barfill valor-pisca" style="width:${Math.max(2, (x.VALOR / max) * 100).toFixed(1)}%;background:${cores[i % cores.length]}"></div></div><div class="barvalue valor-pisca">${moedaRelatorio(x.VALOR)}</div></div>`,
      )
      .join('') +
    `</div>`
  );
...
function donutComposicaoModelo(fatias) {
  ...
  const legenda = fatias
    .map(
      (f) =>
        `<span><i class="trend-dot" style="background:${f.cor}"></i>${escapeHtmlRelatorio(f.rotulo)} · ${moedaRelatorio(f.valor)} (${Math.round((f.valor / total) * 1000) / 10}%)</span>`,
    )
    .join('');
...
function cardForecastModelo(x, tipo, dominio) {
  ...
  if (tipo === 'pendente')
    txt = `Aguardando assinatura ${x.DIAS_NO_ESTAGIO === 0 ? 'hoje' : `há ${x.DIAS_NO_ESTAGIO} dia(s)`} (desde ${x.DATA_MOVIMENTO_BR || '—'})`;
  // v20 — link direto pro negócio no Bitrix (drill-down): usa só o domínio do
  // webhook (sem token) + ID do negócio, nunca a credencial.
  const linkBitrix =
    dominio && x.DEAL_ID
      ? `<a class="ccard-abrir" href="https://${dominio}/crm/deal/details/${encodeURIComponent(x.DEAL_ID)}/" target="_blank" rel="noopener noreferrer">Abrir no Bitrix ↗</a>`
      : '';
  ...
  const entrada = x.DATA_CRIACAO_BR
    ? `<span>📥 Entrou em ${escapeHtmlRelatorio(x.DATA_CRIACAO_BR)}</span>`
    : '';
  const ciclo =
    x.CICLO_DIAS !== '' && x.CICLO_DIAS != null
      ? `<span>⏱ Ciclo: ${x.CICLO_DIAS} dia(s)</span>`
      : '';
...
function mesesForecastModelo(rows, tipo, porEtapa = false, dominio) {
  const grupos = agruparPorModelo(rows, (x) => x.MES_CHAVE || 'sem-data'),
    keys = Object.keys(grupos).sort().reverse();
  return (
    keys
      .map((k) => {
        const a = grupos[k],
          label = a[0]?.MES_LABEL || 'Sem data',
          stats = `${a.length} negócio(s) · ${contarClientesUnicosModelo(a)} cliente(s) · ${moedaRelatorio(somarModelo(a))}`;
        let body = '';
        if (porEtapa) {
...
function gerarHTMLForecastModelo(resultado, tipo = 'semanal') {
  ...
  const periodo =
    tipo === 'mensal'
      ? mesAnoBR(r.periodo_fim)
      : `${formatarDataBR(r.periodo_inicio)} a ${formatarDataBR(r.periodo_fim)}`;
  const metaMensal =
    tipo === 'semanal'
      ? Number(resultado?.meta?.meta_mensal) || 0
      : Number(resultado?.meta_visual) || 0;
  ...
  const realizadoMesBase =
    tipo === 'semanal'
      ? Number(resultado?.resumo?.FECHADO_MES) || 0
      : Number(resultado?.resumo?.FECHADO) || 0;
  const projecaoMesBase =
    tipo === 'semanal'
      ? Number(resultado?.resumo?.FORECAST_MES_TOTAL) || 0
      : Number(resultado?.resumo?.FORECAST_TOTAL) || 0;
  ...
  const alertaHigiene =
    semCloseDate + closeDateVencida <= 0
      ? ''
      : `<div class="alert-banner warn"><span class="icon">🧹</span><span>${semCloseDate ? `${semCloseDate} negócio(s) sem CLOSEDATE` : ''}${semCloseDate && closeDateVencida ? ' e ' : ''}${closeDateVencida ? `${closeDateVencida} com CLOSEDATE vencida` : ''} — vale revisar antes de fechar o mês.</span></div>`;
  const badgePendentes =
    r.resumo.PENDENTES_NEGOCIOS > 0
      ? `<span class="badge-ping" title="${r.resumo.PENDENTES_NEGOCIOS} negócio(s) aguardando assinatura">${r.resumo.PENDENTES_NEGOCIOS}</span>`
      : '';
  ...
  const vendedoresUnicos = [
    ...new Set(
      [...r.fechados, ...r.pendentes, ...r.pipeline].map((x) => x.RESPONSAVEL).filter(Boolean),
    ),
  ].sort((a, b) => a.localeCompare(b, 'pt-BR'));
  const filtroVendedorHtml = vendedoresUnicos.length
  ...
  const compDeltaTxt =
    comp.deltaPct == null
      ? 'sem negócios fechados no mesmo período do ano passado para comparar'
      : `${comp.deltaPct >= 0 ? '+' : ''}${comp.deltaPct}% em relação a ${escapeHtmlRelatorio(comp.anoPassadoLabel || 'ano passado')}`;
  const comparativoAnoHtml =
    comp.valorAtual || comp.valorAnoPassado
  ...
  const pipelineEstagioHtml = renderBarModelo(
    r.pipeline_por_estagio || [],
    'Valor do pipeline aberto, por estágio',
    'ESTAGIO',
  );
  const statsAdicionaisHtml =
    ...
    `</div>`;
  const secaoAnaliseHtml =
    `<h2 class="section">📊 Análise</h2><p class="section-sub">Composição do forecast, comparativo com o ano anterior, pipeline por estágio e estatísticas adicionais.</p>` +
    ...
    `</div>`;
  return (
    `<!doctype html><html lang="pt-BR"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Forecast Comercial — ${escapeHtmlRelatorio(periodo)} · ${escapeHtmlRelatorio(marca.nome)}</title><style>${modeloExecutivoCssParaMarca(marca)}</style></head><body>` +
...
function abrirRelatorioVisualForecast() {
  const h = gerarHTMLForecastModelo(resultadoForecastSemanal, 'semanal');
  if (h) mostrarRelatorioVisualInline(h, 'Forecast Semanal — Comercial');
}
function baixarHTMLForecastModelo() {
  const h = gerarHTMLForecastModelo(resultadoForecastSemanal, 'semanal');
  if (h) baixarArquivo(h, `forecast_modelo_atlas_${dataHoje()}.html`, 'text/html;charset=utf-8;');
}
...
async function extrairForecastSemanal(webhook) {
  ...
  try {
    ...
    const palavraForecast = normalizarTextoChave(
      document.getElementById('palavraFunilForecast').value || 'Comercial',
    );
    ...
    if (!categorias.length)
      throw new Error(
        'Não encontrei o pipeline Comercial. Ajuste o nome no campo de configuração do Forecast.',
      );

    const categoria = categorias[0];
    const nomeCategoria = nomeFunilSemCodigo(
      meta.categorias[categoria] || `Categoria ${categoria}`,
    );
    ...
    const empresas = await buscarEntidadesPorIds(webhook, 'crm.company.list', idsEmpresa, [
      'ID',
      'TITLE',
    ]);
    ...
    for (const d of deals) {
      ...
      const usaProbBitrix =
        Number.isFinite(probInformada) && probInformada > 0 && probInformada <= 100;
      const prob = usaProbBitrix
        ? probInformada
      ...
      const vendedorNome =
        nomeUsuario(vendedorId) || (vendedorId ? `ID ${vendedorId}` : 'Sem responsável');
    ...
    const metaSemanalDerivada =
      metaMensal > 0 ? Math.round((metaMensal / semanasNoMes) * 100) / 100 : 0;
    ...
    for (const d of deals) {
      ...
      if (semanticaMes === 'success' && dataDentroFaixa(d.CLOSEDATE, mesInicio, mesFim)) {
        ...
        semanticaMes === 'process' &&
        !ehEstagioPiloto(d.STAGE_ID, stageLabelMes) &&
        dataDentroFaixa(d.CLOSEDATE, mesInicio, mesFim)
      ) {
        const probInformadaMes = Number(d.PROBABILITY);
        const probMes =
          Number.isFinite(probInformadaMes) && probInformadaMes > 0 && probInformadaMes <= 100
            ? probInformadaMes
    ...
    const modeloVisualForecast = await construirDadosModeloForecast(
      webhook,
      meta,
      inicio,
      fim,
    ...
    if (metaMensal > 0) {
      ...
      salvarHistoricoForecastLocal({
        ...
        projecaoMes: fechadoConsistente + pipelinePonderadoDelta,
      });
    }
    ...
    resultadoForecastSemanal = {
      meta: {
        ...
        meta_semanal_derivada: metaSemanalDerivada,
        meta_semanal_origem: metaSemanalOrigem,
      ...
      resumo: {
        ...
        META_SEMANAL: metaSemanal,
        GAP_META: gap,
        ATINGIMENTO_SEMANAL_PCT:
          metaSemanal > 0 ? Math.round((fechado / metaSemanal) * 1000) / 10 : null,
        FECHADO_MES: fechadoMes,
        PIPELINE_PONDERADO_MES: pipelinePonderadoMes,
        FORECAST_MES_TOTAL: forecastMesTotal,
        META_MENSAL: metaMensal,
        GAP_MENSAL: gapMensal,
        ATINGIMENTO_MENSAL_PCT:
          metaMensal > 0 ? Math.round((fechadoMes / metaMensal) * 1000) / 10 : null,
      ...
      higiene: linhas.filter(
        (r) => r.SITUACAO_SEMANA === 'Sem CLOSEDATE' || r.SITUACAO_SEMANA === 'CLOSEDATE vencida',
      ),
    ...
    atualizarStatus(
      `Forecast concluído: ${abertosSemanaQtd} negócio(s) aberto(s) previstos para a semana; forecast total ${moedaRelatorio(forecastTotal)}.`,
    );
  } catch (e) {
...
function renderizarForecastSemanal() {
  ...
  document.getElementById('forecastPeriodoTexto').innerHTML =
    ...
    (r.meta.meta_mensal
      ? ` Meta mensal (${escapeHtmlRelatorio(mesAnoBR(r.meta.mes_inicio))}): <strong>${moedaRelatorio(r.meta.meta_mensal)}</strong>.`
      : ' Meta mensal não informada.');
  ...
  document.getElementById('forecastKpis').innerHTML = kpis
    .map(([rotulo, valor, alvo]) => kpiCardHtml(rotulo, valor, alvo))
    .join('');
  ...
  document.getElementById('forecastVendedoresTabela').innerHTML = tabelaRelatorio(
    [
      ...
      { label: 'Ponderado', valor: (x) => moedaRelatorio(x.FORECAST_PONDERADO_ABERTO), html: true },
      {
        ...
        valor: (x) => `<strong>${moedaRelatorio(x.FORECAST_TOTAL)}</strong>`,
        html: true,
      },
    ],
    r.vendedores,
  ...
  document.getElementById('forecastNegociosTabela').innerHTML = tabelaRelatorio(
    [
      ...
      { label: 'Prob.', valor: (x) => `${x.PROBABILIDADE_PCT}%` },
      {
        label: 'Bucket',
        valor: (x) =>
          `<span class="badge-relatorio">${escapeHtmlRelatorio(x.BUCKET_FORECAST)}</span>`,
        html: true,
      },
  ...
  document.getElementById('forecastHigieneTabela').innerHTML = tabelaRelatorio(
    [
      ...
      { label: 'Valor', valor: (x) => moedaRelatorio(x.OPPORTUNITY), html: true },
      {
        label: 'Problema',
        valor: (x) =>
          `<span class="badge-relatorio alerta">${escapeHtmlRelatorio(x.SITUACAO_SEMANA)}</span>`,
        html: true,
      },
    ],
    r.higiene,
