...
Valide suporte multi-organização (Birth Hub 360, e portais adicionais que possam existir — tenants, não marcas: a plataforma tem identidade visual única, Birth Hub 360º). Não presuma que separação visual significa separação de dados.
...
Identidade Birth Hub 360º: Antique Gold `#D4AF37`, Obsidian `#0B132B`, Snow White `#F8FAFC` (ver identidade-visual/birthhub360/). Cores de origem por tenant (Birth Hub 360 laranja `#FF5618`/Birth Hub 360 azul `#374898`) só valem como metadado da extração Bitrix, nunca como UI do produto. Respeitar modo escuro, usar BirthHubLogo (não os logos antigos), usar PT-BR, ser responsivo e acessível.
