...
- `WelcomeScreen.tsx` **antes do Piloto 001** (ver `.Codex/PILOTS.md`): hero centralizada com
  ...
  exceção justificada (ver `AGENTS.md` seção 5), não um erro. Use o antes/depois documentado em
  `.Codex/PILOTS.md` como referência do que "genérico" parece na prática neste projeto — não como
...
## Usando uma exceção justificada (seção 5 do `AGENTS.md`)

As regras desta skill (e as regras visuais #2/#4/#5/#6 do `AGENTS.md`) descrevem o padrão default,
...
Analytics/Comercial Inteligente, 010 Chatbook — ver `.Codex/PILOTS.md`): a fonte de "inovação" mais
...
- [ ] Nenhum texto, crédito, link, controle ou funcionalidade existente foi removido sem antes
      ...
      decorativo) — ver `AGENTS.md` seção 6.
