...
- Em mobile/Capacitor, renderização 3D contínua consome bateria e CPU de forma desproporcional —
  ...
  visual #7 no `AGENTS.md`: 3D não é decoração default).
...
ver `AGENTS.md` seção 6, mas nunca formalmente avaliado até este piloto). Antes de adicionar ou
...
- **Não remova mídia legada silenciosamente** só por parecer visualmente desnecessária — ver
  `AGENTS.md` seção 6. Se for genuinamente questionável (como o áudio da `WelcomeScreen`), documente
  a dúvida (em `.Codex/PILOTS.md` ou na resposta ao usuário) em vez de decidir sozinho removê-la ou
  alterá-la.
