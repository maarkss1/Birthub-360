...
import { queuesEnabled, rateLimiterConnection } from '../lib/queue/redis';
...
export function applyRateLimiters(app: Express): void {
  ...
  const aiLimiter = rateLimit({
    ...
    message: {
      success: false,
      error:
        'Too many requests to AI services from this organization, please try again after 15 minutes',
    },
  });
  ...
  const bugReportLimiter = rateLimit({
    ...
    message: {
      success: false,
      error: 'Muitos relatos de problema enviados. Tente novamente em 15 minutos.',
    },
  });
