...
describe('sendEmail', () => {
  it('lança MailerNotConfiguredError quando SMTP_HOST está ausente', async () => {
    ...
    await expect(sendEmail({ to: 'a@b.com', subject: 'Oi', text: 'Corpo' })).rejects.toBeInstanceOf(
  ...
  it('usa SMTP_USER como remetente quando SMTP_FROM não está definido', async () => {
    ...
    expect(sendMailMock).toHaveBeenCalledWith(
      expect.objectContaining({ from: 'sdr@empresa.com.br' }),
    );
  });
  ...
  it('propaga o erro real quando o envio via SMTP falha', async () => {
    ...
    await expect(
      sendEmail({ to: 'lead@empresa.com', subject: 'Proposta', text: 'Corpo' }),
