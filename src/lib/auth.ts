...
export const auth = betterAuth({
  ...
  trustedOrigins: [
    ...
    ...(process.env.NODE_ENV === 'production' ? [] : ['https://dev-server.loca.lt']),
  ...
  emailAndPassword: {
    ...
    // era a ÚNICA barreira do cadastro: bastava digitar qualquer endereço corporativo
    // — mesmo pertencente a outra pessoa — para ganhar
      ...
      create: {
        before: async (user) => {
          ...
          if (!user.organizationId) {
            ...
            // de ser restrito a domínios específicos (ver access-policy.ts),
            // rotular toda organização nova com nome fixo ficaria simplesmente
