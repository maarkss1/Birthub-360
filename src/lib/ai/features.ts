...
const callModel = async (
  systemPrompt: string,
  userPrompt: string,
  model: string = 'local-llama3',
  piiValues: PiiValue[] = [],
) => {
  const aiModel = getAiModel(model);
  const minimized =
    piiValues.length > 0
      ? minimizePii(userPrompt, piiValues)
      : { text: userPrompt, applied: [] as PiiToken[] };
  const result = await aiModel.invoke([
    new SystemMessage(systemPrompt),
    new HumanMessage(minimized.text),
  ]);
  return minimized.applied.length > 0
    ? rehydratePii(result.content, minimized.applied)
    : result.content;
};
...
function parseAndValidate<T>(raw: string, schema: z.ZodType<T>): ParseResult<T> {
  ...
  if (!result.success) {
    return {
      success: false,
      error: `JSON não corresponde ao formato esperado: ${result.error.message}`,
    };
  }
...
const callModelJson = async <T>(
  ...
  const rehydrate = (raw: string) =>
    minimized.applied.length > 0 ? rehydratePii(raw, minimized.applied) : raw;
  ...
  const repaired = await aiModel.invoke([
    ...
    new HumanMessage(
      `Sua resposta anterior não é um JSON válido nesse formato (${firstAttempt.error}). Responda de novo, SOMENTE com o JSON corrigido, sem markdown.`,
    ),
  ]);
...
export const draftFollowUp = async (previousInteraction: string, piiValues: PiiValue[] = []) =>
  callModel(
...
export const scoreLeadQuality = async (leadData: string, piiValues: PiiValue[] = []) =>
  callModel(
...
export const generateObjectionHandling = async (objection: string, piiValues: PiiValue[] = []) =>
  callModel(
...
export const analyzeCompetitors = async (companyDescription: string, piiValues: PiiValue[] = []) =>
  callModel(
...
export const generateElevatorPitch = async (productDetails: string, piiValues: PiiValue[] = []) =>
  callModel(
...
export const identifyPainPoints = async (customerFeedback: string, piiValues: PiiValue[] = []) =>
  callModel(
...
export const createColdCallScript = async (
...
export const summarizeMeetingNotes = async (transcript: string, piiValues: PiiValue[] = []) =>
  callModel(
...
export const evaluateDealRisk = async (dealDetails: string, piiValues: PiiValue[] = []) =>
  callModel(
...
export const categorizeLead = async (
  leadDescription: string,
  piiValues: PiiValue[] = [],
): Promise<CategorizationResult> =>
  callModelJson<CategorizationResult>(
...
export const extractActionItems = async (
  ...
  callModelJson<ActionItemsResult>(
