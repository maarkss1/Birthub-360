...
export const fromPrismaLeadStatus = (v: string): LeadStatus =>
  (LEAD_STATUS_FROM_PRISMA[v] ?? v) as LeadStatus;
...
export const fromPrismaCompanyStatus = (v: string): CompanyStatus =>
  (COMPANY_STATUS_FROM_PRISMA[v] ?? v) as CompanyStatus;

export const toPrismaActivityType = (v: ActivityType): string => ACTIVITY_TYPE_TO_PRISMA[v];
export const fromPrismaActivityType = (v: string): ActivityType =>
  (ACTIVITY_TYPE_FROM_PRISMA[v] ?? v) as ActivityType;

export const toPrismaActivityStatus = (v: ActivityStatus): string => ACTIVITY_STATUS_TO_PRISMA[v];
export const fromPrismaActivityStatus = (v: string): ActivityStatus =>
  (ACTIVITY_STATUS_FROM_PRISMA[v] ?? v) as ActivityStatus;

export const toPrismaAutomationTrigger = (v: AutomationTriggerLabel): string =>
  AUTOMATION_TRIGGER_TO_PRISMA[v] ?? v;
export const fromPrismaAutomationTrigger = (v: string): AutomationTriggerLabel =>
  (AUTOMATION_TRIGGER_FROM_PRISMA[v] ?? v) as AutomationTriggerLabel;

export const toPrismaAutomationAction = (v: AutomationActionLabel): string =>
  AUTOMATION_ACTION_TO_PRISMA[v] ?? v;
export const fromPrismaAutomationAction = (v: string): AutomationActionLabel =>
  (AUTOMATION_ACTION_FROM_PRISMA[v] ?? v) as AutomationActionLabel;
