...
// caller's own `organizationId`, so this can never cross a tenant boundary. Two plain equality queries
...
export function deleteMetricsForUser(organizationId: string, userId: string) {
  return prisma.metric.deleteMany({ where: { organizationId, userId } });
}
