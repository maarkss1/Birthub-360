...
export async function listMetricsHandler(req: Request, res: Response) {
  const metrics = await listMetrics(req.organizationId!, req.voiceHubUser?.id);
...
export async function createMetricHandler(req: Request, res: Response) {
  ...
  const metric = await createMetric(req.organizationId!, req.voiceHubUser?.id, parsed.data);
...
export async function clearMetricsHandler(req: Request, res: Response) {
  await clearMetrics(req.organizationId!, req.voiceHubUser?.id);
