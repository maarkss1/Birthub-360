...
import { analyzeConversation } from '../../features/integrations/whatsapp/conversation-intelligence.service';
...
export const whatsappSignalQueue = queuesEnabled
  ...
  : null;
whatsappSignalQueue?.on('error', (err) =>
  logger.warn({ message: err.message }, 'whatsappSignalQueue offline'),
);
 ...
 * rodada normalmente.
 */
export async function scheduleConversationAnalysis(
  leadId: string,
  organizationId: string,
): Promise<void> {
  ...
  try {
    ...
    logger.warn(
      { err: error, leadId },
      'Não foi possível agendar a análise de conversa do WhatsApp',
    );
  }
}
...
export function createWhatsAppSignalWorker() {
  ...
  worker.on('failed', (job, err) => {
    logger.error(
      { err, jobId: job?.id, leadId: job?.data.leadId },
      'Falha ao analisar conversa de WhatsApp',
    );
