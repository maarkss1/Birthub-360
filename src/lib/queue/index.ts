...
import { aiService } from '../../features/intelligence/services/ai.service';
...
leadsQueueEvents?.on('error', (err) =>
  logger.warn({ message: err.message }, 'leadsQueueEvents offline'),
);
    ...
    async (job: Job) => {
      ...
      return requestContext.run({ tenantId: organizationId }, async () => {
        try {
          ...
          const temperature =
            score != null ? (score >= 75 ? 'Quente' : score >= 45 ? 'Morno' : 'Frio') : undefined;
          ...
          logger.info(
            { leadId, status: qualificationResult.status, score },
