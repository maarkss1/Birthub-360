...
function baseConfirmation(
  overrides: Partial<AvailabilityConfirmation> = {},
...
describe('isVerifiableConfirmation', () => {
  ...
  it('rejeita inferência de modelo disfarçada de evidência (evidenceRef genérico)', () => {
    expect(isVerifiableConfirmation(baseConfirmation({ evidenceRef: 'ai-inferred' }), NOW)).toBe(
      false,
    );
    expect(isVerifiableConfirmation(baseConfirmation({ evidenceRef: 'model-judgment' }), NOW)).toBe(
      false,
    );
  });

  it('rejeita tipo de evidência fora da lista fechada (ex.: um agente tentando inventar um tipo novo)', () => {
    expect(isVerifiableConfirmation(baseConfirmation({ evidenceType: 'ai-guess' }), NOW)).toBe(
      false,
    );
  });
  ...
  it('rejeita horário proposto no passado', () => {
    ...
    expect(
      isVerifiableConfirmation(
        baseConfirmation({ proposedStart: past, proposedEnd: new Date('2026-08-01T10:30:00Z') }),
        NOW,
      ),
    ).toBe(false);
  });
  ...
  it('rejeita fim antes ou igual ao início', () => {
    expect(isVerifiableConfirmation(baseConfirmation({ proposedEnd: FUTURE_START }), NOW)).toBe(
      false,
    );
  });
});
...
describe('scheduleMeetingIfConfirmed', () => {
  const context = {
    ...
    ownerEmail: 'sdr@birthhub360.com.br',
    ownerUserId: 'user-1',
  };
