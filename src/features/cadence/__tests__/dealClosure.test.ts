...
describe('isDeterministicCloseEvent', () => {
    ...
    (type) => {
      expect(
        isDeterministicCloseEvent(
          baseEvent({
            type,
            triggeredBy: type === 'manual_crm_confirmation' ? 'user-9' : 'webhook:provider',
          }),
        ),
      ).toBe(true);
    },
  ...
  it('aceita gatilho humano (userId) e gatilho de webhook de provedor real', () => {
    expect(
      isDeterministicCloseEvent(
        baseEvent({ triggeredBy: 'user-42', type: 'manual_crm_confirmation' }),
      ),
    ).toBe(true);
...
describe('evaluateDealClosure', () => {
  ...
  it('rejeita SEM criar evento quando o gatilho é um agente de IA — nunca deixa o modelo declarar ganho', () => {
    const result = evaluateDealClosure(
      baseEvent({ triggeredBy: 'closer:auto' }),
      () => 'event-2',
      NOW,
    );
