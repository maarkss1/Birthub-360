...
Retorne SEMPRE e APENAS um JSON válido no formato:
{
  ...
  "citedSnippetIndexes": [1, 3]
    ...
    try {
      const response = await model.invoke([
        ...
        new HumanMessage(
          `Trechos Numerados da Base de Conhecimento:\n${contextText}\n\nDúvida do Consultor:\n${input.question}`,
        ),
      ]);
      ...
      return {
        directAnswer:
          'Não foi possível sintetizar a resposta com os dados atuais. Por favor, consulte o suporte de engenharia.',
        technicalSpecifications: [],
        confidenceScore: 40,
        sourceReferences: [],
      };
  ...
  private resolveCitations(citedSnippetIndexes: unknown, hits: SearchHit[]): CopilotCitation[] {
    ...
    for (const raw of citedSnippetIndexes) {
      ...
      // index-1 sempre é um índice válido de hits: o guard acima já garante 1 <= index <= hits.length.
