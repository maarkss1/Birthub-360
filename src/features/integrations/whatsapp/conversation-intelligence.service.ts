...
function parseModelOutput(raw: string): ConversationSignalResult {
  ...
  return {
    intent:
      typeof parsed.intent === 'string' && ALLOWED_INTENTS.has(parsed.intent)
        ? parsed.intent
        : null,
    urgency:
      typeof parsed.urgency === 'string' && ALLOWED_URGENCY.has(parsed.urgency)
        ? parsed.urgency
        : null,
    ...
    nextStep:
      typeof parsed.nextStep === 'string' && parsed.nextStep.trim() ? parsed.nextStep.trim() : null,
    summary:
