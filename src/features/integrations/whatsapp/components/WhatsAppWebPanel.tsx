...
function ChatArea({
  ...
  const { messages, loading, sending, error, sendMessage } = useWhatsAppMessages(
    ...
    <div className="flex-1 flex flex-col min-w-0">
      <div className="flex items-center gap-2 px-4 py-3 border-b border-line bg-emerald-600/10 shrink-0">
        ...
        <div className="min-w-0">
          ...
          <p className="text-[11px] text-ink-2 truncate">
            {formatPhoneDisplay(conversation.phoneE164)}
          </p>
        </div>
      </div>
      ...
      <div className="p-3 border-t border-line shrink-0">
        ...
        <div className="flex items-center gap-2">
          <label htmlFor="whatsapp-web-message-input" className="sr-only">
            Mensagem
          </label>
          <input
...
export function WhatsAppWebPanel({ connected }: { connected: boolean }) {
  ...
  return (
    ...
    <div className="flex h-[440px] sm:h-[600px] max-h-[75vh] rounded-2xl border border-line overflow-hidden bg-surface shadow-sm">
      <div
        className={`w-full md:w-80 shrink-0 border-r border-line flex-col ${selectedPhone ? 'hidden md:flex' : 'flex'}`}
      >
          ...
          {!loading && !error && conversations.length === 0 && (
            <div className="h-full flex flex-col items-center justify-center text-center p-6 gap-2">
              ...
              <p className="text-xs text-ink-2">
                O histórico do celular não é importado automaticamente — as conversas aparecem aqui
                a partir da próxima mensagem enviada ou recebida pelo WhatsApp da organização.
              </p>
            </div>
          )}
