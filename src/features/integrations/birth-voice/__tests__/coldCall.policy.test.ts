...
describe('evaluateLead', () => {
  it('libera lead nunca contatado', () => {
    expect(
      evaluateLead({ id: 'l1', lastInteraction: null, attempts: 0 }, policy, SEGUNDA_9H_SP),
    ).toEqual({ eligible: true });
  });

  it('para de insistir depois do limite de tentativas', () => {
    expect(
      evaluateLead({ id: 'l1', lastInteraction: null, attempts: 3 }, policy, SEGUNDA_9H_SP),
  ...
  it('respeita o intervalo mínimo entre tentativas', () => {
    ...
    expect(
      evaluateLead({ id: 'l1', lastInteraction: ontem, attempts: 1 }, policy, SEGUNDA_9H_SP),
  ...
  it('libera de novo depois de cumprido o intervalo', () => {
    ...
    expect(
      evaluateLead(
        { id: 'l1', lastInteraction: tresDiasAtras, attempts: 1 },
        policy,
        SEGUNDA_9H_SP,
      ),
    ).toEqual({ eligible: true });
  ...
  it('prioriza o limite de tentativas sobre o intervalo', () => {
    ...
    expect(
      evaluateLead(
        { id: 'l1', lastInteraction: tresDiasAtras, attempts: 5 },
        policy,
        SEGUNDA_9H_SP,
      ),
    ).toEqual({ eligible: false, reason: 'max-attempts' });
