...
import { assertSafeExternalUrl } from '../../../../shared/security/urlGuard';
import { getConnectionWebhookUrl, hostnameOf, normalizeWebhookUrl, testWebhook } from './client';
...
export async function connectBitrix(
  organizationId: string,
  rawWebhookUrl: unknown,
  rawLabel: unknown,
): Promise<{ id: string; portalDomain: string }> {
  ...
  await AuditService.log({
    ...
    entity: 'BitrixConnection',
    entityId: connection.id,
    tenantId: organizationId,
    afterState: { portalDomain, label },
  });
  logger.info(
    { organizationId, connectionId: connection.id, portalDomain },
    '[bitrix] Webhook conectado',
  );
  return { id: connection.id, portalDomain };
}

export async function disconnectBitrix(
  organizationId: string,
  connectionId: string,
): Promise<void> {
  const { count } = await prisma.bitrixConnection.deleteMany({
...
export async function regenerateWebhookSecret(
  organizationId: string,
  connectionId: string,
): Promise<{ webhookSecret: string; webhookReceiverUrl: string }> {
  ...
  await AuditService.log({
    ...
    entityId: connectionId,
    tenantId: organizationId,
    afterState: { webhookSecretRegenerated: true },
  });
  return { webhookSecret, webhookReceiverUrl: buildWebhookReceiverUrl(connectionId) };
}

export async function setInboundEventsEnabled(
  organizationId: string,
  connectionId: string,
  enabled: boolean,
): Promise<void> {
  ...
  if (enabled && !connection.webhookSecret) {
    throw new AppError(
      'Gere o segredo do webhook de entrada antes de ativar o recebimento de eventos.',
      400,
    );
  }
  await prisma.bitrixConnection.update({
    where: { id: connectionId },
    data: { inboundEventsEnabled: enabled },
  });
  await AuditService.log({
    action: 'UPDATE',
    entity: 'BitrixConnection',
    entityId: connectionId,
    tenantId: organizationId,
    afterState: { inboundEventsEnabled: enabled },
  });
}
...
export async function testBitrixConnection(
  organizationId: string,
  connectionId: string,
): Promise<{ success: boolean; portalDomain: string }> {
