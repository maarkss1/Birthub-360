...
describe('createExtractionRun — validação server-side (06A, seções 9/18)', () => {
  it('rejeita sem connectionId', async () => {
    ...
    await expect(
      createExtractionRun('org-1', 'user-1', {
        connectionId: '',
        entities: ['lead'],
        filters: { period: 'all' },
      }),
  ...
  it('propaga 404 quando a conexão não pertence a esta organização (isolamento de tenant)', async () => {
    clientMock.getConnectionWebhookUrl.mockRejectedValue(
      new AppError('Conexão Bitrix24 não encontrada para esta organização.', 404),
    );
    const { createExtractionRun } = await import('../extraction.js');
    await expect(
      createExtractionRun('org-1', 'user-1', {
  ...
  it('cria a linha como "queued", grava auditoria e dispara a execução em segundo plano', async () => {
    ...
    expect(prismaMock.bitrixExtractionRun.create).toHaveBeenCalledWith(
...
describe('executeExtractionRun — paginação, progresso, cancelamento e resiliência (06A, seção 10)', () => {
  ...
  it('pagina até esgotar (next === null), soma o total e marca "completed" com os 3 formatos de arquivo', async () => {
    ...
    expect(clientMock.callBitrix).toHaveBeenNthCalledWith(
      ...
      WEBHOOK_URL,
      'crm.lead.list',
      expect.objectContaining({ start: 2 }),
      { correlationId: 'corr-1' },
    );
    ...
    expect(filesMock.writeExtractionFile).toHaveBeenCalledWith(
      'org-1',
      'run-1',
      'lead.csv',
      'csv-content',
  ...
  it('entidade que falha marca status "failed", grava errorMessage e incrementa a métrica de falha por entidade', async () => {
    ...
    clientMock.callBitrix.mockRejectedValue(
      new Error('Bitrix24 aplicou limite de chamadas (HTTP 429).'),
    );
    ...
    expect(prismaMock.bitrixExtractionRun.updateMany).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        data: expect.objectContaining({
          status: 'failed',
          errorMessage: 'Bitrix24 aplicou limite de chamadas (HTTP 429).',
        }),
      }),
    );
    expect(filesMock.writeExtractionFile).not.toHaveBeenCalled();
    expect(await bitrixExtractionFailuresTotal.get()).toMatchObject({
      values: expect.arrayContaining([
        expect.objectContaining({ labels: { tenant: 'org-1', entity: 'deal' }, value: 1 }),
      ]),
    });
  });
  ...
  it('conexão sem webhook associado falha o run inteiro com entity="run" (não processa nenhuma entidade)', async () => {
    ...
    expect(await bitrixExtractionFailuresTotal.get()).toMatchObject({
      values: expect.arrayContaining([
        expect.objectContaining({ labels: { tenant: 'org-1', entity: 'run' } }),
      ]),
    });
  });
  ...
  it('período "custom" inválido persistido na linha (corrompido/editado fora do fluxo normal) falha o run com mensagem clara', async () => {
    ...
    expect(prismaMock.bitrixExtractionRun.updateMany).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({
          status: 'failed',
          errorMessage: expect.stringContaining('data inicial'),
        }),
      }),
    );
  });
  ...
  it('não aplica ASSIGNED_BY_ID a entidades sem esse campo (empresa) — evita filtro que o Bitrix rejeitaria', async () => {
    ...
    expect((params as { filter: Record<string, unknown> }).filter).not.toHaveProperty(
      'ASSIGNED_BY_ID',
    );
  });
  ...
  it('user.get nunca recebe "select" (a API não aceita) mesmo com campos pedidos', async () => {
    prismaMock.bitrixExtractionRun.findFirst.mockResolvedValue(
      baseRun({ entities: ['user'], fields: { user: ['EMAIL'] } }),
    );
    clientMock.callBitrix.mockResolvedValueOnce({ result: [], next: null });
...
describe('checkpoint incremental (Onda 41) — retomada real entre execuções, sem reprocessar o período inteiro', () => {
  it('2ª execução da mesma conexão+entidade+período usa o checkpoint gravado pela 1ª — não refaz o full-scan', async () => {
    ...
    prismaMock.bitrixExtractionRun.findFirst.mockResolvedValue(
      baseRun({ id: 'run-1', entities: ['lead'], connectionId: 'conn-1', filters }),
    );
    ...
    prismaMock.bitrixExtractionRun.update.mockImplementation(
      async (args: { data: { progress: unknown } }) => {
    ...
    expect(
      (firstParams as { filter: Record<string, unknown> }).filter['>=DATE_CREATE'],
    ).toBeUndefined();
    ...
    prismaMock.bitrixExtractionRun.findFirst.mockResolvedValue(
      baseRun({ id: 'run-2', entities: ['lead'], connectionId: 'conn-1', filters }),
    );
    prismaMock.bitrixExtractionRun.findMany.mockResolvedValue([
    ...
    expect(prismaMock.bitrixExtractionRun.findMany).toHaveBeenCalledWith(
      expect.objectContaining({
    ...
    expect((secondParams as { filter: Record<string, unknown> }).filter['>=DATE_CREATE']).toBe(
      firstCheckpoint,
    );
  });
  ...
  it('período "custom" explícito nunca usa (nem consulta) o checkpoint de uma execução anterior', async () => {
          ...
          entities: [
            {
              ...
              status: 'done',
              pagesExhausted: true,
              checkpointTo: '2020-01-01T00:00:00.000Z',
            },
          ],
        },
      },
    ...
    const [, , params] = clientMock.callBitrix.mock.calls[0];
    expect((params as { filter: Record<string, unknown> }).filter['>=DATE_CREATE']).toBe(
      expected.from.toISOString(),
    );
    expect((params as { filter: Record<string, unknown> }).filter['<=DATE_CREATE']).toBe(
      expected.to.toISOString(),
    );
  });

  it('entidade sem campo de data (user) nunca participa de checkpoint — nem consulta o histórico', async () => {
    prismaMock.bitrixExtractionRun.findFirst.mockResolvedValue(
      baseRun({ entities: ['user'], connectionId: 'conn-1', filters: { period: 'all' } }),
    );
    clientMock.callBitrix.mockResolvedValueOnce({ result: [], next: null });
  ...
  it('entidade que só esgotou por bater no teto de segurança (pagesExhausted: false) NÃO grava checkpoint — evita pular registros nunca lidos numa retomada futura', async () => {
    prismaMock.bitrixExtractionRun.findFirst.mockResolvedValue(
      baseRun({ entities: ['lead'], connectionId: 'conn-1', filters: { period: 'all' } }),
    );
    prismaMock.bitrixExtractionRun.findMany.mockResolvedValue([]);
    let call = 0;
    clientMock.callBitrix.mockImplementation(async () => {
    ...
    const updateCall = prismaMock.bitrixExtractionRun.update.mock.calls[0][0] as {
      data: { progress: { entities: Array<Record<string, unknown>> } };
    };
...
describe('cancelExtractionRun', () => {
  ...
  it('404 quando a extração não existe (ou é de outra organização)', async () => {
    ...
    const { cancelExtractionRun } = await import('../extraction.js');
    await expect(cancelExtractionRun('org-1', 'run-inexistente')).rejects.toMatchObject({
      statusCode: 404,
    });
  });
...
describe('getExtractionRun / listExtractionRuns — isolamento de tenant', () => {
  it('getExtractionRun busca sempre com organizationId no WHERE (nunca só pelo id)', async () => {
    ...
    expect(prismaMock.bitrixExtractionRun.findFirst).toHaveBeenCalledWith({
  ...
  it('getExtractionRun 404 para execução de outra organização', async () => {
    prismaMock.bitrixExtractionRun.findFirst.mockResolvedValue(null);
    const { getExtractionRun } = await import('../extraction.js');
    await expect(getExtractionRun('org-1', 'run-de-outra-org')).rejects.toMatchObject({
      statusCode: 404,
    });
  });
...
describe('deleteExtractionRun — exclusão precisa remover o arquivo, não só a linha (LGPD)', () => {
  ...
  it('404 sem deletar nada quando a extração não é desta organização', async () => {
    prismaMock.bitrixExtractionRun.findFirst.mockResolvedValue(null);
    const { deleteExtractionRun } = await import('../extraction.js');
    await expect(deleteExtractionRun('org-1', 'run-de-outra-org')).rejects.toMatchObject({
      statusCode: 404,
    });
...
describe('downloadExtractionFile — nunca serve arquivo sem checar tenant/status/existência real em disco', () => {
  it('404 quando a extração não é desta organização', async () => {
    ...
    await expect(downloadExtractionFile('org-1', 'run-x', 'csv', 'lead')).rejects.toMatchObject({
      statusCode: 404,
    });
  });
  ...
  it('permite download de uma extração completed_partial (gerou arquivo utilizável, só não esgotou o portal)', async () => {
    prismaMock.bitrixExtractionRun.findFirst.mockResolvedValue({
      status: 'completed_partial',
      files: [{ format: 'csv', entity: 'lead', filename: 'lead.csv' }],
    });
  ...
  it('CSV sem "entity" informado devolve 400 pedindo a entidade', async () => {
    ...
    const { downloadExtractionFile } = await import('../extraction.js');
    await expect(downloadExtractionFile('org-1', 'run-x', 'csv')).rejects.toMatchObject({
      statusCode: 400,
    });
  });
  ...
  it('410 quando o histórico existe mas o arquivo não está mais em disco (armazenamento efêmero)', async () => {
    prismaMock.bitrixExtractionRun.findFirst.mockResolvedValue({
      status: 'completed',
      files: [{ format: 'json', entity: null, filename: 'extraction.json' }],
    });
    filesMock.readExtractionFile.mockResolvedValue(null);
    const { downloadExtractionFile } = await import('../extraction.js');
    await expect(downloadExtractionFile('org-1', 'run-x', 'json')).rejects.toMatchObject({
      statusCode: 410,
    });
  });
