...
describe('POST /webhook/:connectionId', () => {
  ...
  it('rejeita com 404 quando inboundEventsEnabled está desligado, mesmo com a conexão existindo', async () => {
    ...
    const res = await request(app)
      .post('/webhook/conn-1')
      .send('auth[application_token]=segredo-correto');
    expect(res.status).toBe(404);
  });
  ...
  it('rejeita com 401 quando o application_token não bate com o segredo salvo', async () => {
    ...
    const res = await request(app)
      .post('/webhook/conn-1')
      .send('auth[application_token]=segredo-errado');
  ...
  it('escopo estreito: quando o bitrixRecordId não corresponde a nenhum Lead JÁ IMPORTADO, não cria nada — só registra "skipped"', async () => {
    ...
    expect(prismaMock.bitrixSyncLog.create).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({ status: 'skipped', leadId: null }),
      }),
    );
  });
  ...
  it('atualiza o Lead local já importado com os dados atuais do Bitrix, dentro do contexto de tenant correto (RLS)', async () => {
    ...
    expect(res.status).toBe(200);
    expect(clientMock.callBitrix).toHaveBeenCalledWith(CONNECTION.webhookUrl, 'crm.lead.get', {
      id: '123',
    });
    ...
    expect(prismaMock.bitrixSyncLog.create).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({ status: 'success', direction: 'inbound' }),
      }),
    );
  ...
  it('incrementa bitrix_sync_failures_total quando o processamento do evento falha (achado da Onda 7)', async () => {
    ...
    clientMock.callBitrix.mockRejectedValue(
      new Error('Bitrix24 respondeu com erro de servidor (HTTP 502).'),
    );
    ...
    expect(prismaMock.bitrixSyncLog.create).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({ status: 'failed', direction: 'inbound' }),
      }),
    );
