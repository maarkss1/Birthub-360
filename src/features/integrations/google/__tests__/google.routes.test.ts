...
describe('GET /api/google/auth-url', () => {
  ...
  it('devolve 503 quando o serviço não está configurado (GOOGLE_CLIENT_ID/SECRET ausentes)', async () => {
    ...
    expect(res.body).toEqual({
      success: false,
      error: 'GOOGLE_CLIENT_ID/GOOGLE_CLIENT_SECRET ausentes.',
    });
  });
...
describe('GET /api/google/calendar/upcoming', () => {
  ...
  it('devolve 409 quando a organização ainda não conectou o Google', async () => {
    getUpcomingCalendarEventsMock.mockRejectedValue(
      new GoogleNotConnectedError('Esta organização ainda não conectou uma conta Google.'),
    );
    const app = buildApp();
    ...
    expect(res.body).toEqual({
      success: false,
      error: 'Esta organização ainda não conectou uma conta Google.',
    });
  });
});
