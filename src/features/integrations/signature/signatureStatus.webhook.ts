...
import { applySignatureStatusUpdate } from '../../cadence/application/documentSignature';
...
const VALID_STATUSES = new Set<SignatureStatus>([
  'created',
  'sent',
  'viewed',
  'signed',
...
async function handleSignatureStatus(req: Request, res: Response): Promise<void> {
  ...
  if (!secret) {
    ...
    logger.error(
      'Webhook de status de assinatura recebido, mas SIGNATURE_INBOUND_WEBHOOK_SECRET não está configurado.',
    );
  ...
  if (!Buffer.isBuffer(rawBody)) {
    logger.error(
      'Webhook de status de assinatura sem corpo bruto — a rota precisa ser montada antes do express.json().',
    );
  ...
  if (
    ...
    !providerRequestId ||
    !statusRaw ||
    !VALID_STATUSES.has(statusRaw as SignatureStatus)
  ) {
    res.status(400).json({
      success: false,
  ...
  try {
    ...
    if (!result.applied) {
      ...
      logger.warn(
        { provider, providerRequestId, status: statusRaw, reason: result.reason },
        'Atualização de status de assinatura ignorada.',
      );
    ...
    logger.error(
      { err: error, provider, providerRequestId },
      'Falha ao processar webhook de status de assinatura.',
    );
    res
      .status(500)
      .json({ success: false, error: 'Falha ao processar atualização de status de assinatura.' });
  }
}
