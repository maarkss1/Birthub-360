...
export async function get3CXConnectionsForOrg(
  organizationId: string,
): Promise<ThreeCXConnection[]> {
...
export async function save3CXConnectionForOrg(
  organizationId: string,
  conn: {
    id: string;
    label: string;
...
export async function delete3CXConnectionForOrg(
  organizationId: string,
  connectionId: string,
): Promise<void> {
...
export async function list3CXConnections(
  organizationId: string,
): Promise<ThreeCXConnectionSummary[]> {
...
export async function connect3CX(
  organizationId: string,
  input: ThreeCXConnectionInput,
): Promise<ThreeCXConnectionSummary> {
...
export async function test3CXConnection(
  organizationId: string,
  connectionId: string,
): Promise<{ success: boolean; message: string; pbxUrl: string }> {
  ...
  try {
    ...
    return {
      success: res.ok,
      message: res.ok
        ? 'PABX 3CX respondendo normalmente.'
...
export async function make3CXCall(
  ...
  // valia antes.
  if (
    await isSuppressed(organizationId, destinationNumber, {
      leadId: leadId ?? null,
      email: email ?? null,
    })
  ) {
  ...
  try {
    ...
    failureReason = controller.signal.aborted
      ...
      : 'falha de rede ao comunicar com o PABX';
    logger.warn(
      { err, organizationId, connectionId: conn.id },
      '[3cx] Falha ao disparar chamada real',
    );
  } finally {
    clearTimeout(timeout);
  }
  ...
  if (!dialSucceeded) {
    logger.warn(
      ...
      '[3cx] Chamada NÃO foi disparada',
    );
    throw new AppError(
      `Não foi possível disparar a chamada pelo PABX 3CX (${failureReason}).`,
      502,
    );
  }
 ...
 */
export async function process3CXWebhook(
  payload: Record<string, unknown>,
): Promise<ThreeCXWebhookResult> {
  const eventType =
    asString(payload.event) ?? asString(payload.eventType) ?? asString(payload.type) ?? 'unknown';
  const extension =
    asString(payload.extension) ??
    asString(payload.Extension) ??
    asString(payload.dn) ??
    asString(payload.DN);
  const callId =
  ...
  if (!extension) {
    ...
    logger.warn(
      { eventType, callId },
      '[3cx] Webhook sem ramal (extension) — organização não pôde ser resolvida. Descartado.',
    );
  ...
  if (resolved === 'not-found') {
    logger.warn(
      ...
      '[3cx] Nenhuma conexão 3CX cadastrada para este ramal — evento descartado.',
    );
  ...
  return requestContext.run({ tenantId: organizationId }, async () => {
    ...
    if (NON_TERMINAL_EVENT_TYPES.has(eventType.toLowerCase())) {
      ...
      logger.info(
        { eventType, callId, organizationId, connectionId: connection.id },
        '[3cx] Evento intermediário de chamada recebido — sem resultado final ainda.',
      );
    ...
    const durationSeconds =
      ...
      asNumber(payload.call_duration) ??
      asNumber(payload.talkTime);
    const machineDetected =
      asBoolean(payload.machineDetected) ??
      asBoolean(payload.amd) ??
      asBoolean(payload.voicemailDetected);
    const toNumber =
      asString(payload.to) ??
      asString(payload.destination) ??
      asString(payload.callee) ??
      asString(payload.dialedNumber) ??
      asString(payload.external_number);
    const fromNumber =
      asString(payload.from) ?? asString(payload.caller) ?? asString(payload.source);
    ...
    if (!lead) {
      logger.info(
        { callId, organizationId, connectionId: connection.id },
        '[3cx] Evento de chamada sem lead correspondente nesta organização.',
      );
    ...
    if (callId) {
      ...
      if (existingActivity) {
        logger.info(
          { callId, organizationId, leadId: lead.id },
          '[3cx] Atividade já registrada para esta chamada — reentrega ignorada.',
        );
