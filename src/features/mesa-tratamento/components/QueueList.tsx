...
import type { QueueLeadSummary } from '../mesaTratamento.api.js';
        ...
        {queue.map((lead, index) => (
          ...
          >
            <div className="flex items-center justify-between gap-2">
              <span className="flex items-center gap-1.5 font-semibold text-ink truncate">
                {index === 0 ? (
                  ...
                  <Lock size={12} className="text-ink-2 shrink-0" />
                )}
                {lead.title}
              </span>
              <span
            ...
            <div className="flex items-center gap-1.5 mt-1.5 flex-wrap">
              ...
              {lead.temperature && (
                <Badge variant={TEMPERATURE_VARIANT[lead.temperature] ?? 'default'}>
                  {lead.temperature}
                </Badge>
              )}
