...
export function CurrentLeadCard({ lead, leadStatuses, onRegistered }: CurrentLeadCardProps) {
  ...
  async function handleSubmit() {
    ...
    if (disqualifying && !lossReasonId)
      return toast.error('Selecione o motivo de desqualificação.');
    ...
    <Card accentBar>
      ...
      <CardContent className="space-y-4">
        <div className="grid grid-cols-2 gap-3 text-sm">
          <div>
            ...
            <span className="text-ink">
              {lead.contactName || 'Não informado'}
        ...
        <div className="border-t border-line pt-4 space-y-3">
          ...
          <div>
            <label className={labelClass} htmlFor="mesa-outcome">
              Resultado *
            </label>
            <select
              id="mesa-outcome"
          ...
          <div>
            <label className={labelClass} htmlFor="mesa-stage">
              Mover etapa no Bitrix24 (opcional)
            </label>
            <select
              id="mesa-stage"
              className={selectClass}
          ...
          {disqualifying && (
            <div>
              <label className={labelClass} htmlFor="mesa-loss-reason">
                Motivo de desqualificação * (obrigatório no Bitrix)
              </label>
              <select
                id="mesa-loss-reason"
                className={selectClass}
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className={labelClass} htmlFor="mesa-next-title">
            ...
            <div>
              <label className={labelClass} htmlFor="mesa-next-when">
                Data/hora
              </label>
              <input
                id="mesa-next-when"
                type="datetime-local"
                className={selectClass}
                value={nextActionWhen}
                onChange={(e) => setNextActionWhen(e.target.value)}
              />
            </div>
          </div>
        </div>
