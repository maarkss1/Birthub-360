...
> = {
  ...
  // mesmo problema já corrigido na Sidebar/Configurações (PR #247). text-brand-ink resolve.
  likely: { barClass: 'bg-brand', textClass: 'text-brand-ink dark:text-brand' },
      ...
      <div className="flex flex-wrap items-start justify-between gap-3 mb-3">
        <div>
          ...
          <p className="text-[11px] text-ink-2 max-w-md">
            Conservador (já garantido) → Provável (Forecast oficial) → Otimista (se tudo em aberto
            fechar).
          </p>
        </div>
        {momentum &&
          (() => {
            ...
            return (
              <span
                className={`inline-flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-[11px] font-bold shrink-0 ${style.className}`}
              >
          ...
          return (
            <div key={s.key}>
              <div className="flex items-baseline justify-between gap-2 mb-1">
                ...
                <span className="text-sm font-black text-ink [font-variant-numeric:tabular-nums]">
                  {formatCurrency(s.amount, range.currency)}
                </span>
              </div>
              ...
              {s.gapToGoal != null && (
                <p className="text-[10px] text-ink-2 mt-0.5">
                  {s.gapToGoal > 0
                    ? `Falta ${formatCurrency(s.gapToGoal, range.currency)} para a meta neste cenário`
                    : 'Meta batida neste cenário'}
                </p>
              )}
            </div>
          );
