    ...
    <Card padding="sm" className="border-brand/20 bg-brand/[0.03]">
      ...
      {!error && recommendations && recommendations.length === 0 && (
        <p className="text-xs text-ink-2">
          Nenhuma prioridade identificada no momento — sem alertas ativos nem negócios com valor
          relevante em risco.
        </p>
      )}
              ...
              return (
                <li key={i} className="rounded-xl border border-line bg-surface p-3">
                  ...
                  <p className="text-xs font-semibold text-ink flex items-start gap-1.5">
                    <ArrowRight
                      className="w-3.5 h-3.5 mt-0.5 text-brand shrink-0"
                      aria-hidden="true"
                    />
                    {rec.suggestedAction}
                  </p>
                  {rec.relatedDealIds.length > 0 && (
                    <p className="text-[10px] text-ink-2 mt-1.5">
                      {rec.relatedDealIds.length} negócio(s) relacionado(s) — ver no Centro de
                      Decisão abaixo.
                    </p>
                  )}
                </li>
      ...
      {!error && !recommendations && (
        <p className="text-xs text-ink-2">
          Prioriza as próximas ações do mês em ordem de impacto — usa os mesmos números já
          calculados nesta tela, nunca inventa negócio ou valor.
        </p>
      )}
    </Card>
  );
