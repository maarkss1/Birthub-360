    ...
    <div className="space-y-6">
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <KpiTile
          ...
          hint={`${data.count} negócio(s)`}
          metricKey="pipeline_criado"
        />
        <KpiTile
          label="Ticket Médio Criado"
          value={formatCurrency(data.averageTicket)}
          metricKey="ticket_medio"
        />
        <KpiTile
          label="Pipeline Necessário"
          value={formatCurrency(data.pipelineNeeded)}
          hint="Meta futura / Win Rate esperado"
        />
        <KpiTile
          label="Creation Coverage"
          value={formatMultiple(data.creationCoverage)}
          tone={
            data.creationCoverage != null
              ? data.creationCoverage >= 1
                ? 'good'
                : 'critical'
              : undefined
          }
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
        <KpiTile
          ...
          tone={
            data.pacePercent != null ? (data.pacePercent >= 100 ? 'good' : 'critical') : undefined
          }
        ...
        <KpiTile
          ...
          tone={
            data.paceGapAmount != null ? (data.paceGapAmount <= 0 ? 'good' : 'critical') : undefined
          }
          hint={
            data.paceGapAmount != null && data.paceGapAmount < 0
              ? 'À frente do ritmo'
              : 'Positivo = atrás do ritmo'
          }
        />
        <KpiTile
