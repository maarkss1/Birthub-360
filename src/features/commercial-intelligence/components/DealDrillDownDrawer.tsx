...
export function DealDrillDownDrawer({ filter, query, onClose }: DealDrillDownDrawerProps) {
  ...
  useEffect(() => {
    ...
    commercialIntelligenceApi
      .deals(filter, {
        ...
        agingCritical: query.agingCritical,
        missingNextAction: query.missingNextAction,
        ids: query.ids?.join(','),
        limit: 100,
      })
      .then((result) => {
  ...
  return (
    <Drawer
      isOpen={!!query}
      onClose={onClose}
      title={query?.title ?? 'Negócios'}
      subtitle={total > 0 ? `${total} negócio(s)` : undefined}
    >
      {loading && (
      ...
      {!loading && !error && rows.length === 0 && (
        <EmptyState
          ...
          description="Não há negócios que atendam a este filtro no momento."
        />
      )}
      {!loading && !error && rows.length > 0 && (
          ...
          {rows.map((row) => (
            <div key={row.id} className="rounded-xl border border-line p-3 space-y-1.5">
              <div className="flex items-start justify-between gap-2">
                ...
                <p className="font-black text-sm text-ink [font-variant-numeric:tabular-nums]">
              ...
              <div className="flex flex-wrap gap-1.5 text-[11px]">
                ...
                {row.tier && (
                  <Badge
                    variant={
                      row.tier === 'Commit' ? 'success' : row.tier === 'Upside' ? 'default' : 'info'
                    }
                  >
                    {TIER_LABEL[row.tier]}
                  </Badge>
              ...
              <dl className="grid grid-cols-2 gap-x-3 gap-y-0.5 text-[11px] text-ink-2 pt-1">
                <div>
                  ...
                  <dd className="inline">{row.owner || 'Não disponível'}</dd>
                </div>
                <div>
                  <dt className="inline font-semibold">Probabilidade: </dt>
                  <dd className="inline">
                    {row.weightedProbability != null
                      ? `${row.weightedProbability}%`
                      : 'Não disponível'}
                  </dd>
                </div>
                <div>
                  <dt className="inline font-semibold">Prev. fechamento: </dt>
                  <dd className="inline">{formatDate(row.expectedCloseAt)}</dd>
                </div>
                <div>
                  <dt className="inline font-semibold">Última atividade: </dt>
                  <dd className="inline">{formatDate(row.lastInteraction)}</dd>
                </div>
              </dl>
                ...
                <div className="pt-1">
                  ...
                  {composerFor === row.id && (
                    <div className="mt-2 rounded-lg border border-line bg-surface-2 p-2 space-y-2">
                      <label htmlFor={`bitrix-note-${row.id}`} className="sr-only">
                        Nota para o Bitrix24
                      </label>
                      <textarea
