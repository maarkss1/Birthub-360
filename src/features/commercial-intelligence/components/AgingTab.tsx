  ...
  if (totalOpen === 0) {
    return (
      <EmptyState
        ...
        description="Aging é calculado sobre negócios abertos do funil Negócio."
      />
    );
  }
    ...
    <div className="space-y-6">
      <Card padding="sm">
        <h3 className="text-sm font-bold text-ink mb-3">
          Aging de oportunidades (desde a criação)
        </h3>
            ...
            <div key={bucket.label} className="flex items-center gap-3">
              ...
              <div className="flex-1 h-6 rounded-md bg-surface-2 overflow-hidden">
                <div
                  ...
                  style={{
                    width: `${totalOpen > 0 ? Math.max(bucket.count > 0 ? 4 : 0, (bucket.count / totalOpen) * 100) : 0}%`,
                  }}
                />
              </div>
              <div className="w-12 shrink-0 text-right text-xs [font-variant-numeric:tabular-nums] text-ink">
                {bucket.count}
              </div>
      ...
      <Card padding="sm">
        ...
        <p className="text-[11px] text-ink-2 mb-3">
          ...
          {data.trackingSince
            ? ` Rastreado desde ${new Date(data.trackingSince).toLocaleDateString('pt-BR')}.`
            : ' Ainda sem histórico de transição registrado — dados abaixo são estimados.'}
        </p>
              ...
              {data.byStage.map((stage) => (
                <tr key={stage.stageId} className="border-b border-line last:border-0">
                  ...
                  <td className="py-1.5 text-right">
                    {stage.amountOverThreshold > 0 ? (
                      <button
                        ...
                        onClick={() =>
                          setDrillDown({
                            title: `Acima do aging crítico — ${stage.stageName}`,
                  ...
                  <td className="py-1.5 text-right">
                    <Badge
                      variant={
                        stage.dataQuality === 'measured'
                          ? 'success'
                          : stage.dataQuality === 'estimated'
                            ? 'warning'
                            : 'default'
                      }
                    >
