...
import { type FunnelStageConversion, formatPercent } from '../commercialIntelligence.api.js';
    ...
    <Card padding="sm">
      ...
      {funnel.length === 0 ? (
        <p className="text-xs text-ink-2">
          Nenhuma etapa configurada no pipeline de Negócios ainda.
        </p>
      ) : (
        <div className="space-y-2">
          {funnel.map((stage) => {
            const primaryCount = hasHistoricalFunnel ? stage.historicalReachedCount : stage.count;
            const primaryConversion = hasHistoricalFunnel
              ? stage.historicalConversionFromPrevious
              : stage.conversionFromPrevious;
            return (
              <div key={stage.stageId} className="flex items-center gap-3">
                <div
                  className="w-40 shrink-0 text-xs font-semibold text-ink truncate"
                  title={stage.label}
                >
                  {stage.label}
                </div>
                <div className="flex-1 h-6 rounded-md bg-surface-2 overflow-hidden">
                  <div
                    ...
                    style={{
                      width: `${Math.max(4, Math.min(100, firstStageBase && firstStageBase > 0 ? (primaryCount / firstStageBase) * 100 : 0))}%`,
                    }}
                  />
                </div>
                <div className="w-16 shrink-0 text-right text-xs [font-variant-numeric:tabular-nums] text-ink">
                  {primaryCount}
                </div>
                <div className="w-20 shrink-0 text-right text-[11px] text-ink-2">
                  {primaryConversion != null ? formatPercent(primaryConversion) : '—'}
                </div>
                <div
                  className="w-24 shrink-0 text-right text-[11px] text-ink-2"
                  title="Dias médios na etapa"
                >
                  {stage.averageDaysInStage != null
                    ? `${stage.averageDaysInStage}d na etapa`
                    : 'Sem histórico'}
                </div>
              </div>
            );
