...
import { FORECAST_RULES_VERSION } from '../application/forecastEngine.js';
import { buildForecastSnapshot } from '../application/forecastSnapshot.js';
import type { ExecutiveOverview } from '../domain/CommercialIntelligence.js';
...
function minimalOverview(overrides: Partial<ExecutiveOverview> = {}): ExecutiveOverview {
  return {
    ...
    coverageMonth: {
      coverage: 2,
      coverageRecommended: 3,
      pipelineEligible: 90_000,
      remainingGoal: 90_000,
    },
    coverage30: {
      coverage: null,
...
describe('InMemoryForecastSnapshotStore — protótipo append-only (persistência real pendente de handoff de schema)', () => {
  it('save nunca sobrescreve — dois snapshots do mesmo período convivem', async () => {
    ...
    const first = buildForecastSnapshot(
      ORG,
      minimalOverview({ forecastAmount: 70_000 }),
      new Date('2026-08-01T00:00:00Z'),
    );
    const second = buildForecastSnapshot(
      ORG,
      minimalOverview({ forecastAmount: 75_000 }),
