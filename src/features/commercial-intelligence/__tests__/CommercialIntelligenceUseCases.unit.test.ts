...
class FakeRepository implements CommercialIntelligenceRepository {
  ...
  async hasBitrixConnection(): Promise<boolean> {
    return this.bitrixConnected;
  }

  syncActivity: { lastSyncAt: Date | null; syncedCount: number; failedCount: number } = {
    lastSyncAt: null,
    syncedCount: 0,
    failedCount: 0,
  };
  ...
  filterOptionsResult: {
    ...
    icps: string[];
    companies: string[];
  } = { owners: [], products: [], sources: [], icps: [], companies: [] };
...
describe('CommercialIntelligenceUseCases', () => {
  ...
  it('Pipeline Criado: só conta negócios com createdAt dentro do período (não os apenas movimentados)', async () => {
    const createdThisMonth = deal({
      id: 'novo-1',
      amount: 25_000,
      createdAt: new Date('2026-08-03T00:00:00Z'),
    });
    const createdLastMonth = deal({
      id: 'antigo-1',
      amount: 999_000,
  ...
  it('Sales Cycle: média e mediana em dias sobre negócios fechados no período', async () => {
    const closed1 = deal({
      ...
      closedAt: new Date('2026-08-01T03:00:00Z'),
    }); // 31 dias; meia-noite BRT
    const closed2 = deal({
      id: 'c2',
      amount: 1,
      stageIsLost: true,
      createdAt: new Date('2026-07-11T03:00:00Z'),
      closedAt: new Date('2026-08-01T03:00:00Z'),
    }); // 21 dias; meia-noite BRT
  ...
  it('Aging: respeita o filtro de owner (não mistura negócios de outro vendedor)', async () => {
    ...
    const useCases = new CommercialIntelligenceUseCases(repo);
    const aging = await useCases.aging(
      ORG,
      { month: PERIOD, owner: 'ana@birthhub360.com.br' },
      NOW,
    );
    const bucket90plus = aging.buckets.find((b) => b.label === '90+ dias');
    expect(bucket90plus?.count).toBe(1);
  ...
  it('Qualidade do CRM: respeita o filtro de owner (não mistura negócios de outro vendedor)', async () => {
    const ana = deal({ id: 'ana-1', amount: 1000, owner: 'ana@birthhub360.com.br' });
    const bruno = deal({ id: 'bruno-1', amount: 2000, owner: 'bruno@birthhub360.com.br' });
    const repo = new FakeRepository([ana, bruno]);
    const useCases = new CommercialIntelligenceUseCases(repo);
  ...
  it('Motivos de perda: classifica lossReason numa taxonomia fixa e preserva a observação original', async () => {
    const lost = deal({
      ...
      stageIsLost: true,
      closedAt: new Date('2026-08-05'),
      lossReason: 'Preço muito alto para o orçamento deles',
    });
  ...
  it('Qualidade do CRM: com conexão Bitrix24, contabiliza vínculo, falhas e lista as falhas com o motivo', async () => {
    ...
    const failed = deal({
      id: 'f-1',
      amount: 3_000,
      bitrixLeadId: 'bx-lead-2',
      bitrixSyncStatus: 'failed',
      bitrixSyncError: 'Token expirado',
    });
    ...
    expect(quality.bitrixSync.failures[0]).toMatchObject({
      leadId: 'f-1',
  ...
  it('Leading Indicators: weeklySeries expõe as 4 semanas usadas no cálculo de movingAverage4w, mais antiga → mais recente', async () => {
    ...
    expect(pipelineCriado?.movingAverage4w).toBeCloseTo(
  ...
  it('Pipeline Creation Pace: dias úteis decorridos/total do mês e ritmo (%) batem com o esperado proporcional', async () => {
    ...
    const lost = deal({
      ...
      closedAt: new Date('2026-08-06T00:00:00Z'),
      lossReason: 'Preço',
    });
    ...
    expect(result.paceExpectedAmount).toBeCloseTo(
      Math.round((paceExpectedAmount + Number.EPSILON) * 100) / 100,
      2,
    );
    expect(result.pacePercent).toBeCloseTo(
      (result.amount / (result.paceExpectedAmount as number)) * 100,
      1,
    );
  });
  ...
  it('Proteção 90 dias: 4 entradas em meses de calendário sequenciais, sem meta = sem_dados', async () => {
    ...
    expect(overview.coverageProtection.map((e) => e.period)).toEqual([
      '2026-08',
      '2026-09',
      '2026-10',
      '2026-11',
    ]);
    expect(overview.coverageProtection.map((e) => e.label)).toEqual([
      'AGO/2026',
      'SET/2026',
      'OUT/2026',
      'NOV/2026',
  ...
  it('Proteção 90 dias: classifica cada mês com base na meta e no pipeline elegível daquele mês', async () => {
    ...
    const eligibleAug = deal({
      id: 'e-aug',
      amount: 20_000,
      expectedCloseAt: new Date('2026-08-20T00:00:00Z'),
    });
    const eligibleSep = deal({
      id: 'e-sep',
      amount: 200_000,
      expectedCloseAt: new Date('2026-09-10T00:00:00Z'),
    });
  ...
  it('Proteção 90 dias: busca as metas dos 4 meses numa única chamada ao repositório (sem N+1)', async () => {
    ...
    expect(overview.coverageProtection.map((e) => e.goalAmount)).toEqual([
  ...
  it('Comparação com o mês anterior: Fechado e Win Rate do mês anterior quando há negócios fechados', async () => {
    ...
    const useCases = new CommercialIntelligenceUseCases(repo);
    const overview = await useCases.executiveOverview(ORG, { month: PERIOD }, NOW);
    expect(overview.previousPeriod).toEqual({
      period: '2026-07',
      closedAmount: 40_000,
      closedCount: 1,
      winRate: 50,
  ...
  it('Forecast Confidence: cobertura de histórico de etapa eleva o score quando os negócios têm LeadStageHistory', async () => {
    ...
    const history = deals.map((d) => ({
      ...
      enteredAt: new Date('2026-08-01T00:00:00Z'),
      exitedAt: null,
    }));
  ...
  it('Pipeline Creation Pace: ritmo esperado proporcional a dias úteis decorridos no mês', async () => {
    ...
    expect(result.paceExpectedAmount).toBeCloseTo(
      200_000 * (result.elapsedBusinessDays / result.totalBusinessDays),
      2,
    );
    expect(result.pacePercent).toBeCloseTo(
      (result.amount / (result.paceExpectedAmount as number)) * 100,
  ...
  it('Alertas: "Forecast acima da meta" (positivo) quando o forecast já cobre a meta', async () => {
    ...
    expect(alerts.some((a) => a.id === 'forecast-acima-meta' && a.severity === 'positive')).toBe(
      true,
    );
  });

  it('Alertas: crítico quando a confiabilidade do forecast está baixa', async () => {
    const missingFields = deal({
      id: 'm1',
      amount: 0,
      owner: null,
      expectedCloseAt: null,
  ...
  it('Alertas: "Propostas sem interação" quando um negócio Commit/Best Case está sem interação recente', async () => {
    ...
    const useCases = new CommercialIntelligenceUseCases(repo);
    const alerts = await useCases.alerts(ORG, { month: PERIOD }, NOW);
    expect(alerts.some((a) => a.id === 'propostas-sem-interacao' && a.severity === 'warning')).toBe(
      true,
    );
  });
  ...
  it('Funil histórico: negócio perdido cedo não conta como tendo alcançado uma etapa posterior — a etapa terminal nunca vira proxy de progresso', async () => {
    ...
    const useCases = new CommercialIntelligenceUseCases(repo);
    const performance = await useCases.performance(ORG, { month: PERIOD }, NOW);
    expect(performance.funnel.find((f) => f.stageId === 'stage-nova')?.historicalReachedCount).toBe(
      1,
    );
    expect(
      performance.funnel.find((f) => f.stageId === 'stage-proposta')?.historicalReachedCount,
    ).toBe(0);
  });
    ...
    const history = [
      {
        ...
        stageName: 'Nova Oportunidade',
        enteredAt: new Date('2026-08-01T00:00:00Z'),
        exitedAt: new Date('2026-08-02T00:00:00Z'),
      },
      {
        leadId: 'won-1',
        ...
        exitedAt: new Date('2026-08-09T00:00:00Z'),
      },
      {
        leadId: 'won-1',
        stageId: 'stage-ganho',
        stageName: 'Negócios Ganhos',
        enteredAt: new Date('2026-08-09T00:00:00Z'),
        exitedAt: null,
      },
    ];
  ...
  it('Funil histórico: funnelHistoricalTrackingSince é null sem nenhum histórico e a data mais antiga quando há histórico', async () => {
    ...
    const perfSemHistorico = await new CommercialIntelligenceUseCases(repoSemHistorico).performance(
      ORG,
      { month: PERIOD },
      NOW,
    );
    ...
    const repoComHistorico = new FakeRepository(
      [deal({ id: 'd2', amount: 1_000 })],
      STAGES,
      history,
    );
    const perfComHistorico = await new CommercialIntelligenceUseCases(repoComHistorico).performance(
      ORG,
      { month: PERIOD },
      NOW,
    );
  ...
  it('Qualidade do CRM: expõe última sincronização e volume de sucesso/falha dos últimos 30 dias', async () => {
    ...
    repo.syncActivity = {
      lastSyncAt: new Date('2026-08-14T10:00:00Z'),
      syncedCount: 12,
      failedCount: 3,
    };
    const useCases = new CommercialIntelligenceUseCases(repo);
