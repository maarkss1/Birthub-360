...
const OVERVIEW: ExecutiveOverview = {
  ...
  coverageMonth: {
    coverage: 1.5,
    coverageRecommended: 2,
    pipelineEligible: 300_000,
    remainingGoal: 200_000,
  },
  ...
  coverageProtection: [
    ...
    },
    {
      ...
      label: 'SET/2026',
      goalAmount: 300_000,
      pipelineEligible: 250_000,
      remainingGoal: 300_000,
      coverage: 0.83,
      coverageRecommended: 2,
      status: 'critico',
    },
  ...
  forecastConfidence: {
    ...
    sampleSize: 8,
    fieldCompletenessScore: 0.8,
    stageHistoryCoverage: 0.6,
...
const ALERTS: ExecutiveAlert[] = [
  {
    ...
    description: 'Forecast atual cobre apenas 79% da meta.',
    metricValue: 64_000,
  },
];
...
describe('buildExecutiveExportRows — uma linha por indicador (bloco, indicador, valor, unidade)', () => {
  it('inclui os blocos principais e nunca fabrica um valor "0" quando o dado é null', () => {
    ...
    expect(blocks).toEqual(
      new Set(['Resultado do Mês', 'Forecast', 'Pipeline', 'Eficiência', 'Alertas']),
    );
...
describe('rowsToCsv — formato tabular simples (bloco;indicador;valor;unidade), compatível com Excel PT-BR', () => {
  ...
  it('escapa valores com ponto-e-vírgula/aspas/quebra de linha entre aspas duplas', () => {
    const csv = rowsToCsv([
      {
        block: 'Alertas',
        indicator: 'Título; com ponto e vírgula',
        value: 'Valor com "aspas"',
        unit: '',
      },
    ]);
...
describe('buildExecutiveExportHtml — relatório autônomo, sem CSS/JS externo', () => {
  ...
  it('escapa caracteres HTML perigosos vindos de dados (ex.: título de alerta)', () => {
    const maliciousAlert: ExecutiveAlert = {
      id: 'x',
      severity: 'warning',
