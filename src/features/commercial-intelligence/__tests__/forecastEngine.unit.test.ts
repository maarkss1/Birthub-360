...
import { FORECAST_RULES, scoreOpportunity } from '../application/forecastEngine.js';
...
describe('forecastEngine.scoreOpportunity — motor determinístico (não-IA)', () => {
  ...
  it('penaliza estagnação só quando há histórico real (daysInCurrentStage e média ambos não-nulos)', () => {
    ...
    expect(withHistory.negativeFactors).toContain(
      'Parado na etapa há mais que o dobro do tempo médio histórico',
    );
    ...
    expect(withoutHistory.negativeFactors).not.toContain(
      'Parado na etapa há mais que o dobro do tempo médio histórico',
    );
  });
  ...
  it('classifica em Commit/BestCase/Pipeline/Upside conforme os limiares documentados', () => {
    expect(
      scoreOpportunity(
        baseSignals({
          stageProbability: FORECAST_RULES.COMMIT_THRESHOLD,
          nextAction: new Date('2026-08-11'),
          lastInteraction: new Date('2026-08-09'),
