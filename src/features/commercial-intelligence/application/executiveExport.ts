...
export function buildExecutiveExportRows(
  ...
  const add = (block: string, indicator: string, value: string | null, unit: string) => {
    rows.push({
      block,
      indicator,
      value: value == null || value === '' ? 'Não disponível' : value,
      unit,
    });
  };
  ...
  add(
    'Resultado do Mês',
    'Meta New MRR',
    overview.goal ? money(overview.goal.amount) : null,
    currency,
  );
  ...
  add(
    ...
    money(creation.pipelineNeeded),
    currency,
  );
  add(
    'Pipeline',
    'Creation Coverage',
    creation.creationCoverage != null
      ? raw(Math.round(creation.creationCoverage * 1000) / 10)
      : null,
    '%',
  );
  ...
  add(
    ...
    creation.totalBusinessDays > 0
      ? `${creation.elapsedBusinessDays}/${creation.totalBusinessDays}`
      : null,
    'dias úteis',
  );
 ...
 */
export function buildExecutiveExportHtml(
  rows: ExportKpiRow[],
  alerts: ExecutiveAlert[],
  period: string,
...
export function buildExecutiveExport(
  ...
  if (format === 'html') {
    return {
      content: buildExecutiveExportHtml(rows, alerts, overview.period, now.toISOString()),
  ...
  return {
    content: JSON.stringify(
      buildExecutiveExportJson(overview, performance, creation, alerts, now),
      null,
      2,
    ),
