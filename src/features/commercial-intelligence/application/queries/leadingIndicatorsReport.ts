...
export async function buildLeadingIndicators(
  ...
  const countProposals = (from: Date, to: Date) =>
    history.filter(
      (h) => h.stageName === 'Proposta Enviada' && h.enteredAt >= from && h.enteredAt < to,
    ).length;
  const countWon = (from: Date, to: Date) =>
    deals.filter((d) => d.stageIsWon && d.closedAt && d.closedAt >= from && d.closedAt < to).length;

  const buildPoint = async (
    label: string,
    fn: (from: Date, to: Date) => number | Promise<number>,
  ): Promise<LeadingIndicatorPoint> => {
    ...
    const movingAverage4w = roundMoney(
      weeklyValues.reduce((s, v) => s + v, 0) / weeklyValues.length,
    );
    const delta =
      previousWeek === 0 ? (current === 0 ? 0 : 1) : (current - previousWeek) / previousWeek;
    const trend: LeadingIndicatorPoint['trend'] =
