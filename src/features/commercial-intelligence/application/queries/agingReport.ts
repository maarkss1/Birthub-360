...
export async function buildAging(
  ...
  for (const s of open) {
    ...
    const entry = byStageMap.get(s.deal.pipelineStageId) ?? {
  ...
  const byStage: StageAging[] = [...byStageMap.entries()].map(([stageId, entry]) => {
    ...
    const measured = entry.rows
      .filter((s) => s.daysInCurrentStage != null)
      .map((s) => s.daysInCurrentStage as number);
    ...
    return {
      ...
      dataQuality:
        measured.length === entry.rows.length
          ? 'measured'
          : hasHistory
            ? 'estimated'
            : entry.rows.length > 0
