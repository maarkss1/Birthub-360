...
export async function buildCrmQuality(
  ...
  const overallScore =
    withCompleteness.length > 0
      ? roundMoney(
          withCompleteness.reduce((sum, f) => sum + (f.completeness as number), 0) /
            withCompleteness.length,
        )
      : null;

  const suspectedDuplicateGroups =
    await repository.countDuplicateCompanyGroupsAmongOpenDeals(organizationId);
