...
function buildFallbackRecommendations(
  ...
  priorityDeals: DealDrillDownRow[],
): MentorRecommendation[] {
  ...
  const fromDeals: MentorRecommendation[] = priorityDeals.slice(0, 3).map((d) => ({
    priority: d.riskFactors.length >= 2 ? 'alta' : 'media',
    title: `Negócio em risco: ${d.title ?? d.companyName ?? d.id}`,
    rationale:
      d.riskFactors.length > 0
        ? d.riskFactors.join('; ')
        : 'Valor em risco elevado sem fator de risco específico listado.',
    suggestedAction:
      'Contatar o responsável, atualizar a próxima ação e a data prevista de fechamento.',
    relatedDealIds: [d.id],
  }));
...
export class CommercialIntelligenceAiService {
  ...
  async generateExecutiveSummary(
    ...
    if (overview.isEmpty) {
      return {
        summary: 'Ainda não há negócios no funil "Negócio" para resumir.',
    ...
    const contextLines = [
      ...
      `- Proteção 90 dias (M+1/M+2/M+3): ${overview.coverageProtection
        .slice(1)
        .map(
          (e) =>
            `${e.label} ${e.coverage != null ? `${e.coverage.toFixed(1)}x` : 'sem dados'} (${e.status})`,
        )
        .join(', ')}`,
    ...
    const summary = await this.invoke(
      SUMMARY_TOOL_KEY,
      DEFAULT_SUMMARY_MODEL,
      DEFAULT_SUMMARY_TEMPERATURE,
      SUMMARY_SYSTEM_PROMPT,
      userPrompt,
      'Não foi possível gerar o resumo executivo agora.',
    );
   ...
   */
  async draftBitrixRiskNote(
    organizationId: string,
    leadId: string,
  ): Promise<BitrixNoteDraftResult | null> {
    ...
    const contextLines = [
      ...
      deal.positiveFactors.length > 0
        ? `- Fatores positivos: ${deal.positiveFactors.join('; ')}`
        : null,
    ].filter((line): line is string => line !== null);

    const userPrompt = `## Dados reais deste negócio — use somente estes dados:\n${contextLines.join('\n')}`;
    const draft = await this.invoke(
      NOTE_TOOL_KEY,
      DEFAULT_NOTE_MODEL,
      DEFAULT_NOTE_TEMPERATURE,
      NOTE_SYSTEM_PROMPT,
      userPrompt,
      'Não foi possível gerar a nota agora.',
    );
    return { draft };
  }
   ...
   */
  async generateMentorPlaybook(
    organizationId: string,
    filter: CommercialIntelligenceFilter,
  ): Promise<MentorPlaybookResult> {
    const [overview, alerts, aging, creation, trends, priorityDeals] = await Promise.all([
      ...
      this.useCases.dealsDrillDown(organizationId, {
        month: filter.month,
        owner: filter.owner,
        sort: 'riskImpact',
        limit: 5,
    ...
    const dealLines = priorityDeals.rows.map(
      (d) =>
    ...
    const contextLines = [
      ...
      momentum
        ? `- Momentum do Win Rate: ${momentum.direction} (${momentum.latestWinRate.toFixed(1)}% no último mês vs. média anterior de ${momentum.previousAverageWinRate.toFixed(1)}%)`
        : '- Momentum do Win Rate: sem base histórica suficiente',
      `- Gap do forecast para a meta: ${overview.gapForecast != null ? money(overview.gapForecast, currency) : 'não disponível'}`,
      ...
      ...(dealLines.length > 0
        ? dealLines
        : ['- Nenhum negócio aberto elegível para priorização.']),
    ];
    ...
    try {
      const raw = await this.invoke(
        ...
        DEFAULT_MENTOR_TEMPERATURE,
        MENTOR_SYSTEM_PROMPT,
        userPrompt,
        'Não foi possível gerar o playbook agora.',
      );
      const parsed = cleanAndParseJson<MentorRecommendation[]>(raw);
      if (!Array.isArray(parsed) || parsed.length === 0)
        throw new Error('Playbook vazio ou em formato inesperado.');
  ...
  private async invoke(
    ...
    const model = getAiModel(
      customSetting?.model ?? defaultModel,
      customSetting?.temperature ?? defaultTemperature,
      toolKey,
    );
