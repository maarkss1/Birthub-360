...
async function preprocessForOcr(buffer: Buffer): Promise<Buffer> {
  try {
    ...
    const scale =
      longEdge > 0 && longEdge < OCR_MIN_LONG_EDGE_PX ? OCR_MIN_LONG_EDGE_PX / longEdge : 1;
  ...
  try {
    ...
    if (!text) {
      throw new OcrValidationError(
        'Não foi possível ler nenhum texto nessa imagem. Tente uma foto mais nítida.',
      );
    }
    return text;
  } finally {
 ...
 */
export async function structureOcrCandidate(
  rawText: string,
  ...
  if (!result.tradeName) {
    throw new OcrValidationError(
      'A IA não conseguiu identificar o nome de uma empresa nessa imagem.',
    );
  }
  ...
  return {
    ...
    suggestedContact: result.contactName
      ? { name: result.contactName, role: result.contactRole || 'Não informado' }
      : null,
