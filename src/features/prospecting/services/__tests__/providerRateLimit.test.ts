...
describe('getRateLimitPerMinute', () => {
  ...
  it('ignora valor inválido (zero, negativo ou não numérico) e cai no default', () => {
    expect(getRateLimitPerMinute('apollo', { APOLLO_RATE_LIMIT_PER_MINUTE: '0' })).toBe(
      DEFAULT_RATE_LIMIT_PER_MINUTE.apollo,
    );
    expect(getRateLimitPerMinute('apollo', { APOLLO_RATE_LIMIT_PER_MINUTE: '-1' })).toBe(
      DEFAULT_RATE_LIMIT_PER_MINUTE.apollo,
    );
    expect(getRateLimitPerMinute('apollo', { APOLLO_RATE_LIMIT_PER_MINUTE: 'abc' })).toBe(
      DEFAULT_RATE_LIMIT_PER_MINUTE.apollo,
    );
  });
});
