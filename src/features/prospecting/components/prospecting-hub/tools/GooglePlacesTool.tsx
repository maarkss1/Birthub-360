...
export function GooglePlacesTool({ configured }: { configured: boolean }) {
  ...
  const handleSearch = async () => {
    ...
    setError(null);
    try {
      const localizacao = criteria.cidade
        ? `${criteria.cidade}, ${criteria.estado}`
        : criteria.estado;
      const result = await api.post<{ candidates: ProspectCandidate[] }>(
        '/api/prospecting/tools/google-places',
        {
    ...
    <div className="grid grid-cols-1 xl:grid-cols-12 gap-8">
      <div className="xl:col-span-4 bg-surface p-6 rounded-2xl border border-line shadow-sm space-y-4">
        ...
        <p className="text-xs text-ink-2">
          Busca só no Google Places (New) Text Search — sem Apollo, sem OpenStreetMap.
        </p>
        ...
        >
          {isSearching ? (
            ...
            <>
              <Cpu size={18} /> Buscar no Google Places
            </>
          )}
        </button>
