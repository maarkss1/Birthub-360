...
export function HunterTool({ configured }: { configured: boolean }) {
  ...
  const [verifyResult, setVerifyResult] = useState<{ email: string | null; score?: number } | null>(
    null,
  );
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      const result = await api.post<{ contacts: HunterPersonContact[]; error?: string }>(
        '/api/prospecting/tools/hunter',
        { domain },
        { timeoutMs: 20_000 },
      );
  ...
  const handleVerifyEmail = async () => {
    ...
    try {
      const result = await api.post<{ email: string | null; score?: number }>(
        '/api/prospecting/tools/hunter/verify-email',
        { domain, fullName: verifyName },
        { timeoutMs: 15_000 },
      );
    ...
    <div className="grid grid-cols-1 xl:grid-cols-12 gap-8">
      <div className="xl:col-span-4 bg-surface p-6 rounded-2xl border border-line shadow-sm space-y-4">
        ...
        <p className="text-xs text-ink-2">
          Busca só no Hunter.io Domain Search — pessoas reais a partir de e-mails publicados num
          domínio.
        </p>
        ...
        <div>
          <label
            ...
            className="block text-[10px] tracking-wider font-bold uppercase mb-1.5 text-ink-2"
          >
            Nome da empresa (para salvar no CRM)
          </label>
          <input
            id="hu-empresa"
            type="text"
        ...
        >
          {isSearching ? (
            ...
            <>
              <Search size={18} /> Buscar Pessoas
            </>
          )}
        </button>

        <div className="pt-3 mt-1 border-t border-line space-y-3">
          <p className="text-[10px] tracking-wider font-bold uppercase text-ink-2">
            Verificar 1 e-mail específico
          </p>
          <input
            type="text"
          ...
          >
            {isVerifying ? (
              ...
              <>
                <ShieldCheck size={14} /> Verificar E-mail
              </>
            )}
          </button>
          {verifyResult && (
            <p className="text-xs text-ink-2">
              {verifyResult.email ? (
                <>
                  E-mail encontrado:{' '}
                  <a
                    href={`mailto:${verifyResult.email}`}
                    className="text-success-active dark:text-success hover:underline"
                  >
                    {verifyResult.email}
                  </a>
                  {verifyResult.score != null && ` (confiança ${verifyResult.score}%)`}
                </>
              ) : (
      ...
      <div className="xl:col-span-8 space-y-3">
        {(!contacts || contacts.length === 0) && !isSearching && (
          <div className="bg-surface p-8 rounded-2xl border border-dashed border-line text-center text-ink-2 text-sm">
            {contacts?.length === 0
              ? 'Nenhuma pessoa encontrada neste domínio.'
            ...
            >
              ...
              {contact.email && (
                <a
                  ...
                  className="flex items-center gap-1 text-xs text-success-active dark:text-success hover:underline"
                >
              ...
              {getTelephoneLink(contact.phone) && (
                <a
                  href={getTelephoneLink(contact.phone)}
