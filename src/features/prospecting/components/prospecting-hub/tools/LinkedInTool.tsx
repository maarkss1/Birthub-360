...
export function LinkedInTool({ configured }: { configured: boolean }) {
  ...
  const [manualLink, setManualLink] = useState<{ href: string; isDirectProfile: boolean } | null>(
    null,
  );
  ...
  const searchCompanies = async () => {
    ...
    try {
      const result = await api.post<{ candidates: ProspectCandidate[]; error?: string }>(
        '/api/prospecting/tools/apollo',
        companyCriteria,
        { timeoutMs: 30_000 },
      );
  ...
  const searchPeople = async () => {
    ...
    setPeopleError(null);
    try {
      const result = await api.post<{ decisionMakers: DecisionMaker[]; error?: string }>(
    ...
    <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 xl:grid-cols-12 gap-8">
          <div className="xl:col-span-4 bg-surface p-6 rounded-2xl border border-line shadow-sm space-y-4">
            <p className="text-xs text-ink-2">
              Busca empresas via Apollo e mostra só as que têm página real de LinkedIn confirmada.
            </p>
            <div>
              <label
                htmlFor="li-segmento"
            ...
            {companyError && (
              <p className="text-xs text-danger-active dark:text-danger">{companyError}</p>
            )}
          </div>
          ...
          <div className="xl:col-span-8 space-y-4">
            {companyTotal !== null && (
              <p className="text-xs font-semibold text-ink-2">
                {companiesWithLinkedin.length} de {companyTotal} empresa
                {companyTotal === 1 ? '' : 's'} encontrada{companyTotal === 1 ? '' : 's'} têm página
                confirmada no LinkedIn.
              </p>
            )}
      ...
      {subTab === 'decisores' && (
        <div className="grid grid-cols-1 xl:grid-cols-12 gap-8">
          <div className="xl:col-span-4 bg-surface p-6 rounded-2xl border border-line shadow-sm space-y-4">
            <p className="text-xs text-ink-2">
              Busca decisores via Apollo (com fallback Hunter) e mostra só os que têm perfil real de
              LinkedIn.
            </p>
            <div>
              <label
            ...
            >
              {isSearchingPeople ? (
                ...
                <>
                  <Search size={18} /> Buscar Decisores
                </>
              )}
            </button>
            {peopleError && (
              <p className="text-xs text-danger-active dark:text-danger">{peopleError}</p>
            )}
          </div>
          ...
          <div className="xl:col-span-8 space-y-3">
            {peopleTotal !== null && (
              <p className="text-xs font-semibold text-ink-2">
                {peopleWithLinkedin.length} de {peopleTotal} decisor{peopleTotal === 1 ? '' : 'es'}{' '}
                encontrado{peopleTotal === 1 ? '' : 's'} têm perfil confirmado no LinkedIn.
              </p>
            )}
      ...
      <div className="bg-surface-2 p-5 rounded-2xl border border-line space-y-3">
        <p className="text-[10px] tracking-wider font-bold uppercase text-ink-2 flex items-center gap-1.5">
          <Wand2 size={12} /> Gerador manual (quando a Apollo não encontrou o perfil)
        </p>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <input
            ...
            className="p-2.5 bg-surface rounded-xl border border-line outline-none focus:border-brand text-sm text-ink"
          />
          <input
            type="text"
          ...
          <input
            type="text"
            placeholder="URL do LinkedIn já conhecida (opcional)"
            value={manualKnownUrl}
            onChange={(e) => setManualKnownUrl(e.target.value)}
            className="p-2.5 bg-surface rounded-xl border border-line outline-none focus:border-brand text-sm text-ink"
          />
        </div>
          ...
          {manualLink && (
            <>
              ...
              >
                ...
                {manualLink.isDirectProfile ? 'Abrir perfil no LinkedIn' : 'Buscar no LinkedIn'}
              </a>
              {promoted['li-manual'] ? (
                <span className="flex items-center gap-1.5 text-success-active dark:text-success font-bold text-xs">
                  <CheckCircle2 size={14} /> No CRM
        ...
        <p className="text-[10px] text-ink-2">
          Isto gera um link — não é uma busca automática de dados do LinkedIn (o LinkedIn não
          oferece essa API publicamente).
        </p>
      </div>
    </div>
  );
