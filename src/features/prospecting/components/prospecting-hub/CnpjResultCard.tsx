    ...
    <div className="bg-surface rounded-2xl border border-line shadow-sm p-6 sm:p-8 space-y-6">
      <div className="flex items-start justify-between flex-wrap gap-4">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <span
              className={`flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold uppercase ${isActive ? 'bg-success/15 text-success-active dark:text-success' : 'bg-danger/15 text-danger-active dark:text-danger'}`}
            >
              <ShieldCheck size={10} /> {d.situacaoCadastral}
            </span>
          </div>
          <p className="text-sm text-ink-2">
            {d.legalName} · {result.cnpj}
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <InfoTile
          ...
          value={`${d.size} (${d.employeeCountEstimate}+ estimado)`}
        />
        <InfoTile
          icon={TrendingUp}
          label="Capital Social"
          value={d.capitalSocial.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}
        />
        <InfoTile icon={MapPin} label="Localização" value={`${d.city}, ${d.state}`} />
      </div>

      <div>
        <p className="text-[10px] tracking-wider font-bold uppercase text-ink-2 mb-1">
          ...
          result.marketRisk.transporters != null ? (
            <div className="space-y-1.5">
              ...
              <p className="text-xs text-ink-2">
                Indicador de intensidade do mercado de transporte rodoviário de cargas na UF (fonte
                ANTT, mesmo dado já publicado em Market Intelligence) — não é uma medida de
                sinistralidade (roubo/furto).
              </p>
            </div>
          ) : (
            <p className="text-sm text-ink-2">
              RNTRC territorial NÃO DISPONÍVEL — {result.marketRisk.reason}
            </p>
          )}
        </div>
      )}
      ...
      {d.qsa.length > 0 && (
        <div>
          <p className="text-[10px] tracking-wider font-bold uppercase text-ink-2 mb-2">
            Quadro Societário
          </p>
          <div className="flex flex-wrap gap-2">
            {d.qsa.map((s, i) => (
              <span
                key={i}
                className="bg-surface-2 border border-line rounded-full px-3 py-1 text-xs text-ink-2"
              >
