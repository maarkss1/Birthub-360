...
export function DecisionMakerSearch({
  ...
  const toggleSeniority = (v: string) => {
    setCriteria((c) => ({
      ...c,
      senioridades: c.senioridades?.includes(v)
        ? c.senioridades.filter((x) => x !== v)
        : [...(c.senioridades || []), v],
    }));
  };
  ...
  const isPersonaActive = (persona: PersonaOption) => {
    const currentTitles = (criteria.cargos || '')
      .split(',')
      .map((t) => t.trim())
      .filter(Boolean);
  ...
  const togglePersona = (persona: PersonaOption) => {
    setCriteria((c) => {
      ...
      const nextSeniorities = active
        ? (c.senioridades || []).filter(
            (s) => !(persona.seniorities as readonly string[]).includes(s),
          )
        : Array.from(new Set([...(c.senioridades || []), ...persona.seniorities]));
      return {
        ...c,
        cargos: nextTitles.length ? nextTitles.join(',') : undefined,
        senioridades: nextSeniorities,
      };
    });
  };
  ...
  const toggleDepartment = (v: string) => {
    setCriteria((c) => ({
      ...c,
      departamentos: c.departamentos?.includes(v)
        ? c.departamentos.filter((x) => x !== v)
        : [...(c.departamentos || []), v],
    }));
  };
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      const res = await api.post<{ decisionMakers: DecisionMaker[]; error?: string }>(
        '/api/prospecting/decision-makers',
        {
          domain,
          criteria,
        },
  ...
  const handleGenerateIcebreaker = async (idx: number) => {
    ...
    try {
      ...
      const res = await api.post<{ icebreaker: string }>(
        '/api/prospecting/icebreaker',
        { companyName },
        { timeoutMs: 60_000 },
      );
      setIcebreakers((prev) => ({
    ...
    return (
      <div className="mt-3">
        ...
        >
          <Search size={14} />{' '}
          {alreadyFoundCount
            ? 'Buscar mais decisores (outros filtros)'
            : 'Buscar Decisores Nesta Empresa'}
        </button>
      </div>
    );
  }
  ...
  return (
    <div
      className={`mt-4 p-4 border rounded-2xl ${light ? 'border-indigo-400/40 bg-surface-2 text-ink' : 'border-indigo-500/20 bg-indigo-500/5'}`}
    >
      <div className="flex items-center justify-between mb-4">
        <h4 className="font-bold text-sm text-indigo-300 flex items-center gap-2">
          <Users size={16} /> Pesquisa de Decisores (Apollo)
        </h4>
        <button
          type="button"
              ...
              <div className="flex flex-wrap gap-2">
                {normalizedCompanyEmails.map((email) => (
                  <a
                    key={email}
                    href={`mailto:${email}`}
                    className="text-success-active dark:text-success hover:underline flex items-center gap-1"
                  >
                    <Mail size={12} /> {email}
                  </a>
                ))}
                ...
                {normalizedCompanyPhones.map((phone) => (
                  <span key={phone} className="flex flex-wrap items-center gap-2">
                    <a
                      ...
                      className="text-ink hover:text-ink hover:underline flex items-center gap-1"
                    >
                      <Phone size={12} /> {phone}
                    </a>
                    <a
      ...
      <div className="mb-4">
        ...
        <p className="text-[10px] text-ink-2 mt-1">
          Direto da tabela &quot;Contatos ideais&quot; do Playbook — cada clique já ajusta cargo e
          senioridade.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
        <div>
          <label
            htmlFor="criteria-cargos"
            className="block text-[10px] tracking-wider font-bold uppercase mb-1.5 text-ink-2"
          >
            Cargos Específicos (Vírgula)
          </label>
          <input
            id="criteria-cargos"
            type="text"
        ...
        <div>
          <label
            htmlFor="criteria-keywords"
            className="block text-[10px] tracking-wider font-bold uppercase mb-1.5 text-ink-2"
          >
            Palavras-chave (Perfil LinkedIn)
          </label>
          <input
            ...
            onChange={(e) =>
              setCriteria({ ...criteria, palavrasChavePerfil: e.target.value || undefined })
            }
            className="w-full p-2.5 bg-surface rounded-xl border border-line outline-none focus:border-indigo-400 text-sm"
          />
        </div>
        ...
        <div className="flex items-center pt-5">
          <label className="flex items-center gap-2 cursor-pointer text-sm text-ink-2">
            <input
              ...
              onChange={(e) =>
                setCriteria({ ...criteria, apenasEmailVerificado: e.target.checked })
              }
      ...
      {results && (
        <div className="mt-4 border-t border-indigo-100 pt-4">
          <h5 className="text-[10px] tracking-wider font-bold uppercase text-ink-2 mb-3">
          ...
          {selectedDecisionMaker && selectedLinkedIn && (
            <div className="mb-4 rounded-2xl border border-indigo-400/30 bg-indigo-500/10 p-4">
              <div className="flex items-start justify-between gap-3 mb-3">
                <div>
                  <p className="text-[9px] uppercase tracking-wider font-bold text-indigo-300 mb-1">
                    Perfil consolidado dentro da ferramenta
                  </p>
                  <h6 className="font-black text-lg text-ink">{selectedDecisionMaker.name}</h6>
                  <p className="text-xs text-ink-2">
                    {selectedDecisionMaker.title || 'Cargo não encontrado'} · {companyName}
                  </p>
                </div>
                <button
                  type="button"
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs">
                <div className="rounded-xl bg-surface-2 p-3">
                  ...
                  {selectedDecisionMaker.email ? (
                    <a
                      href={`mailto:${selectedDecisionMaker.email}`}
                      className="text-success-active dark:text-success hover:underline flex items-center gap-1"
                    >
                ...
                <div className="rounded-xl bg-surface-2 p-3">
                  ...
                  <a
                    href={selectedLinkedIn.href}
                    target="_blank"
                    rel="noreferrer"
                    className="text-blue-400 hover:underline flex items-center gap-1"
                  >
                    <Linkedin size={12} />
                    {selectedLinkedIn.isDirectProfile
                      ? 'Abrir perfil completo'
                      : 'Localizar perfil no LinkedIn'}
                  </a>
                </div>
              </div>
              <p className="text-[9px] text-ink-2 mt-2">
                Perfil montado com os dados retornados por Apollo/Hunter. O LinkedIn não permite
                incorporar sua página completa com segurança.
              </p>
            </div>
          )}
          {results.length === 0 ? (
            ...
            <div className="space-y-2">
              {results.map((dm, idx) => {
                ...
                return (
                  <div
                    key={idx}
                    className="flex flex-wrap items-center gap-x-3 gap-y-1 bg-surface border border-line rounded-xl px-3 py-2 text-xs text-ink-2"
                  >
                    <button
                      type="button"
                    ...
                    {dm.title && (
                      <span className="text-ink-2 font-medium bg-surface-2 px-2 py-0.5 rounded-md">
                        {dm.title}
                      </span>
                    )}
                    {dm.email && (
                      <a
                        ...
                        className="flex items-center gap-1 text-success-active dark:text-success font-medium bg-success/10 px-2 py-0.5 rounded-md hover:underline"
                      >
                    ...
                    {getTelephoneLink(dm.phone) && (
                      <a
                        href={getTelephoneLink(dm.phone)}
