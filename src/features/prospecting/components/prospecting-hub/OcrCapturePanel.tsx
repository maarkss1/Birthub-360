...
export function OcrCapturePanel() {
  ...
  const startCamera = async () => {
    ...
    try {
      ...
      setError(
        'Não foi possível acessar a câmera. Verifique a permissão do navegador ou envie uma foto pelo botão de upload.',
      );
    }
  };
  ...
  const handleCnpjEnrich = async () => {
    ...
    setError(null);
    try {
      const res = await api.post<{ company: CnpjEnrichmentResponse }>(
      ...
      if (c) {
        setFormData((prev) => ({
          ...
          phone: c.ddd_telefone_1
            ? c.ddd_telefone_1.includes('(')
              ? c.ddd_telefone_1
              : `(${c.ddd_telefone_1.slice(0, 2)}) ${c.ddd_telefone_1.slice(2)}`
            : prev.phone,
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 p-4 rounded-2xl bg-surface border border-line">
        <div>
          ...
          <p className="text-xs text-ink-2 mt-0.5">
            Tire foto, faça upload ou pressione{' '}
            <kbd className="px-1.5 py-0.5 rounded bg-surface-2 border border-line text-ink font-mono text-[10px]">
              Ctrl+V
            </kbd>{' '}
          ...
          <div className="space-y-4 max-w-md mx-auto">
            ...
            <div>
              <p className="font-bold text-ink text-base">
                Arraste uma foto de cartão, placa ou fachada
              </p>
              <p className="text-xs text-ink-2 mt-1">
                Formatos suportados: JPEG, PNG ou WebP até 8MB
              </p>
            </div>
            ...
            <div className="flex items-center justify-center gap-1.5 text-[11px] text-ink-2 pt-1">
              ...
              <span>
                Ou apenas pressione <strong>Ctrl+V</strong> para colar print/screenshot
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-col items-center justify-center gap-3 p-8 rounded-2xl bg-surface border border-line text-center shadow-sm">
          ...
          <div>
            <p className="font-bold text-sm text-ink">
              Processando OCR e estruturando dados com IA...
            </p>
            <p className="text-xs text-ink-2 mt-0.5">
              Normalizando iluminação, lendo texto e identificando contatos corporativos.
            </p>
          </div>
        </div>
      )}
      ...
      {error && (
        <div
          role="alert"
          className="flex items-start gap-2.5 rounded-2xl border border-rose-500/30 bg-rose-500/10 p-4 text-xs text-rose-200 shadow-sm"
        >
          ...
          <div className="lg:col-span-5 space-y-4">
            ...
            {rawExtractedText && (
              <details className="rounded-xl border border-line bg-surface-2/40 p-3 text-xs">
                <summary className="font-bold text-ink-2 cursor-pointer hover:text-ink">
                  Ver Texto Cru Extraído pelo OCR
                </summary>
              ...
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-ink-2">
                    Revise ou edite os campos antes de cadastrar no CRM
                  </p>
                </div>
              </div>
      ...
      {promoted && (
        <div className="bg-emerald-500/10 border border-emerald-500/30 rounded-3xl p-8 text-center space-y-3 shadow-sm">
          ...
          <div>
            ...
            <p className="text-xs text-ink-2 mt-1">
              A empresa foi inserida no Pipeline CRM e sincronizada com o Bitrix24.
            </p>
          </div>
          <div className="pt-2">
            <button
