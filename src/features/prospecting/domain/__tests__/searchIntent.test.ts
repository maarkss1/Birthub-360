...
describe('buildSearchIntent — localização', () => {
  ...
  it('normaliza localizacaoExcluir em lista (trim, sem vazios)', () => {
    const intent = buildSearchIntent({
      ...baseCriteria,
      localizacaoExcluir: ' São Paulo ,, Minas Gerais ',
    });
...
describe('buildSearchIntent — needsFirmographicFiltering', () => {
  it('true quando porte foi informado', () => {
    expect(buildSearchIntent({ ...baseCriteria, porte: '11,50' }).needsFirmographicFiltering).toBe(
      true,
    );
  });
  ...
  it('true quando faturamento (anual ou mensal, min ou max) foi informado', () => {
    expect(
      buildSearchIntent({ ...baseCriteria, faturamentoMin: 100_000 }).needsFirmographicFiltering,
    ).toBe(true);
    expect(
      buildSearchIntent({ ...baseCriteria, faturamentoMax: 100_000 }).needsFirmographicFiltering,
    ).toBe(true);
    expect(
      buildSearchIntent({ ...baseCriteria, faturamentoMensalMin: 10_000 })
        .needsFirmographicFiltering,
    ).toBe(true);
    expect(
      buildSearchIntent({ ...baseCriteria, faturamentoMensalMax: 10_000 })
        .needsFirmographicFiltering,
    ).toBe(true);
  });
  ...
  it('true quando ano de fundação, tecnologia (incluir/excluir) ou capital aberto foram informados', () => {
    expect(
      buildSearchIntent({ ...baseCriteria, anoFundacaoMin: 2000 }).needsFirmographicFiltering,
    ).toBe(true);
    expect(
      buildSearchIntent({ ...baseCriteria, anoFundacaoMax: 2020 }).needsFirmographicFiltering,
    ).toBe(true);
    expect(
      buildSearchIntent({ ...baseCriteria, tecnologias: 'salesforce' }).needsFirmographicFiltering,
    ).toBe(true);
    expect(
      buildSearchIntent({ ...baseCriteria, tecnologiasExcluir: 'sap' }).needsFirmographicFiltering,
    ).toBe(true);
    expect(
      buildSearchIntent({ ...baseCriteria, apenasCapitalAberto: true }).needsFirmographicFiltering,
    ).toBe(true);
  });
  ...
  it('technologiesInclude/technologiesExclude normalizadas em lista (trim, sem vazios)', () => {
    const intent = buildSearchIntent({
      ...baseCriteria,
      tecnologias: ' salesforce ,aws,, ',
      tecnologiasExcluir: 'sap',
    });
...
describe('buildSearchIntent — decisorCargos / needsDecisionMakerContacts', () => {
  ...
  it('false quando decisorCargos está ausente ou vazio', () => {
    ...
    expect(
      buildSearchIntent({ ...baseCriteria, decisorCargos: [] }).needsDecisionMakerContacts,
    ).toBe(false);
  });
});
...
describe('buildSearchIntent — quantityRequested', () => {
  it(`usa MAX_LEADS_PER_SEARCH (${MAX_LEADS_PER_SEARCH}) como default quando quantidade não foi informada`, () => {
    expect(
      buildSearchIntent({ ...baseCriteria, quantidade: undefined as unknown as number })
        .quantityRequested,
    ).toBe(MAX_LEADS_PER_SEARCH);
  });
  ...
  it('quantidade=0 é falsy — cai no mesmo default de "não informado" (MAX_LEADS_PER_SEARCH), igual ao `||` que já existia em prospecting.service.ts', () => {
    expect(buildSearchIntent({ ...baseCriteria, quantidade: 0 }).quantityRequested).toBe(
  ...
  it(`nunca ultrapassa MAX_LEADS_PER_SEARCH (${MAX_LEADS_PER_SEARCH}), mesmo com quantidade maior`, () => {
    expect(buildSearchIntent({ ...baseCriteria, quantidade: 999 }).quantityRequested).toBe(
      MAX_LEADS_PER_SEARCH,
    );
  });
...
describe('buildSearchIntent — demais campos', () => {
  it('companyName vem de nomeEmpresa (trim) ou undefined', () => {
    expect(buildSearchIntent({ ...baseCriteria, nomeEmpresa: '  Atlas Log  ' }).companyName).toBe(
      'Atlas Log',
    );
  ...
  it('excludeNames é repassado (ou lista vazia por padrão)', () => {
    expect(
      buildSearchIntent({ ...baseCriteria, excludeNames: ['Já Visto Ltda'] }).excludeNames,
    ).toEqual(['Já Visto Ltda']);
