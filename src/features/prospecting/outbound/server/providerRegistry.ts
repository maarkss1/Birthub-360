...
const DECLARED_NOT_MIGRATED: SearchProvider[] = [
  ...
  { name: 'gemini', capabilities: [], configured: () => Boolean(process.env.GEMINI_API_KEY), health: async () => ({ status: 'not_implemented', checkedAt: new Date().toISOString(), message: 'Motor de IA para outreach/dossiê, não um search provider - fora do escopo do registry.' }) },
