    ...
    <div className="flex-1 overflow-y-auto bg-transparent font-sans">
      <div className="bh-page">
        <div className="bh-page-stack">
          <PageHeader
            title="Equipe"
            subtitle="Crie acessos, defina papéis e remova usuários da organização."
            icon={<Users className="w-5 h-5" />}
          />

          {/* Formulário de novo usuário */}
          <form onSubmit={handleCreate} className="bh-surface-elevated p-6 rounded-2xl space-y-4">
            ...
            {revealedCredentials && (
              <div className="bg-success/10 border border-success/30 rounded-xl p-4 text-xs space-y-2">
                <p className="font-bold text-success-active dark:text-success">
                  {revealedCredentials.justCreated ? 'Usuário criado!' : 'Senha redefinida!'} Copie
                  a senha temporária agora — ela não aparece de novo.
                </p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full bg-surface-2 border border-line rounded-xl px-3 py-2.5 text-xs text-ink focus:outline-none focus:ring-1 focus:ring-brand"
                >
                  {(assignableRoles.length ? assignableRoles : Object.keys(ROLE_LABELS)).map(
                    (r) => (
          ...
          <div className="bg-surface/80 rounded-2xl border border-line overflow-hidden">
            <div className="px-6 py-4 border-b border-line space-y-2">
              <h2 className="font-black text-sm text-ink flex items-center gap-2">
                <Shield size={16} className="text-brand" /> Usuários da organização (
                {members.length})
              </h2>
              {resetError && (
