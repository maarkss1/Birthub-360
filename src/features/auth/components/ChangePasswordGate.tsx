...
export function ChangePasswordGate() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (changeResult.error) {
      setError(
        changeResult.error.message || 'Não foi possível trocar a senha. Confira a senha atual.',
      );
    ...
    if (clearResult?.error) {
      setError(
        'Senha trocada, mas não consegui liberar o acesso automaticamente. Recarregue a página.',
      );
      setIsSubmitting(false);
      return;
    }
        ...
        <div className="glass-panel p-8 sm:p-10 rounded-[2.5rem] border border-line bg-surface/95 shadow-2xl">
          <div className="flex flex-col items-center mb-6 text-center">
            ...
            <p className="text-ink-2 text-xs mt-2">
              Sua conta foi criada com uma senha temporária. Defina uma senha nova antes de acessar
              a plataforma.
            </p>
          </div>
