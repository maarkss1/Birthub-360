...
export const searchPlaybookTool = tool(
  async ({ query }: { query: string }) => {
    try {
      ...
      const usable = results.filter(
        (r) => r.similarity == null || r.similarity >= MIN_SEMANTIC_SIMILARITY,
      );
  ...
  },
  {
    ...
    description:
      'Pesquisa na base de conhecimento (Base de Conhecimento / Playbook Comercial da Atlas) por regras de qualificação, matrizes de objeção ou critérios de ICP. Use isso quando não souber como qualificar um tipo específico de cliente. Cada resultado cita o documento de origem — sempre mencione essa fonte ao usar o conteúdo na resposta final.',
    schema: z.object({
      query: z
        .string()
        .describe(
          'A pergunta ou termo de busca, ex: "Como qualificar transportadoras com baixo volume?" ou "ICP ideal da Atlas"',
        ),
    }),
  },
);
