...
const PURPOSES = [
  {
    id: 'b2b_agent',
    title: 'Agente Autônomo B2B',
    desc: 'Processa leads, qualifica BANT e responde objeções.',
  },
  {
    id: 'scraping',
    title: 'Extração de Dados Web (Scraping)',
    desc: 'Raspa dados de páginas web, lida com paginação e salva em JSON.',
  },
  {
    id: 'api_integration',
    title: 'Integração de APIs REST & Webhooks',
    desc: 'Conecta serviços externos com autenticação Bearer e retries.',
  },
  {
    id: 'etl_data',
    title: 'Limpeza & Tratamento de Dados (ETL)',
    desc: 'Lê CSV/Excel, normaliza CNPJ/e-mails e filtra inconsistências.',
  },
  {
    id: 'cron_tasks',
    title: 'Automação de Filas & Tarefas Cron',
    desc: 'Executa rotinas periódicas de manutenção e monitoramento.',
  },
  {
    id: 'sdr_outreach',
    title: 'Prospecção Outbound & Disparo SDR',
    desc: 'Monta cadências e-mail/WhatsApp hiperpersonalizadas.',
  },
];
...
const COMPLEXITIES = [
  ...
  {
    id: 'production',
    label: 'Avançado / Produção (Blindagem Total: Retries, Logs, Env Vars, Exception Handling)',
  },
];
...
export function RobustScriptGenerator() {
  ...
  const [activeDropdown, setActiveDropdown] = useState<
    'language' | 'purpose' | 'framework' | 'complexity' | null
  >(null);
  ...
  const handleGenerate = async () => {
    ...
    setResult(null);
    try {
      const response = await api.post<{ result: { content: string } }>(
      ...
      setError(
        generationError instanceof Error
          ...
          : 'Não foi possível gerar o artefato.',
      );
    } finally {
      setGenerating(false);
    }
      ...
      >
        ...
        <div className="relative z-10 flex flex-col items-center text-center mb-10">
          ...
          <p className="text-sm text-ink-2 max-w-2xl leading-relaxed">
            Compile artefatos prontos para produção em Python, PowerShell, TypeScript e System
            Prompts blindados com retries, logs e tratamento de erros.
          </p>
        </div>
