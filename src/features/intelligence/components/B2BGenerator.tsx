...
export function B2BGenerator() {
  ...
  const handleGenerate = async () => {
    ...
    setResult(null);
    try {
      const response = await api.post<{ result: B2BMatrixResult }>(
      ...
      setError(
        generationError instanceof Error
          ...
          : 'Não foi possível gerar a matriz.',
      );
    } finally {
      setGenerating(false);
    }
  ...
  return (
    <div className="space-y-8">
      ...
      >
        {/* Background effects */}
        <div
          className={`absolute top-0 right-0 w-[500px] h-[500px] ${accent.blobA} rounded-full blur-[120px] pointer-events-none -mt-40 -mr-40 opacity-30`}
        ></div>
        <div
          className={`absolute bottom-0 left-0 w-[400px] h-[400px] ${accent.blobB} rounded-full blur-[100px] pointer-events-none -mb-20 -ml-20 opacity-30`}
        ></div>
        ...
        <div className="relative z-10 flex flex-col items-center text-center mb-10">
          ...
          <p className="text-sm text-ink-2 max-w-2xl leading-relaxed">
            Mapeamento preditivo de ICP. A IA cruza dados de mercado para formular hipóteses de
            dores latentes, perguntas SPIN e contorno tático de objeções de alto nível.
          </p>
        </div>
        ...
        <div className="relative z-10 grid grid-cols-1 md:grid-cols-2 gap-6 mb-8 max-w-4xl mx-auto">
          <div className="group relative">
            <div
              className={`absolute inset-0 bg-gradient-to-r ${accent.gradient} opacity-0 group-hover:opacity-15 rounded-2xl blur-xl transition-opacity duration-500`}
            ></div>
        ...
        {error && (
          <div
            role="alert"
            className="relative z-10 mx-auto mt-5 flex max-w-2xl items-start gap-2 rounded-2xl border border-danger/30 bg-danger/10 p-4 text-left text-sm text-danger-active dark:text-danger font-medium"
          >
          ...
          >
            <div className="space-y-8">
              <motion.div
                ...
                className="bg-surface border border-line rounded-[2.5rem] p-8 shadow-card relative overflow-hidden group"
              >
            ...
            <motion.div
              variants={itemVariants}
              className="bg-surface border border-line rounded-[2.5rem] p-8 shadow-card relative overflow-hidden group h-fit"
            >
                ...
                {result.objections.map((obj, i) => (
                  ...
                  >
                    ...
                    <div>
                      ...
                      <p className="text-sm font-bold text-ink leading-relaxed bg-surface p-4 rounded-xl border border-line">
                        &quot;{obj.objection}&quot;
                      </p>
                    </div>
                    <div>
                      ...
                      <p className="text-sm text-ink font-medium leading-relaxed bg-surface p-4 rounded-xl border border-line">
                        &quot;{obj.rebuttal}&quot;
                      </p>
                    </div>
                  </motion.div>
