...
const PROVIDERS = [
  ...
  { id: 'deepseek', name: 'DeepSeek AI', models: ['deepseek-r1', 'deepseek-v3', 'deepseek-coder'] },
  {
    ...
    models: ['llama-3.3-70b', 'qwen-2.5-coder-32b', 'mistral-large-2'],
  },
];
...
const ROLES = [
  {
    id: 'sdr',
    title: 'SDR / Qualificador de Leads',
    desc: 'Engaja inbound, qualifica BANT e agenda reuniões.',
  },
  {
    id: 'bdr',
    title: 'BDR / Gerador de Outbound',
    desc: 'Prospecção ativa cold email, pesquisa de perfil ICP e decisão.',
  },
  {
    id: 'credit',
    title: 'Analista de Risco & Crédito',
    desc: 'Valida CNPJ, score fiscal, protestos e balanços.',
  },
  {
    id: 'cs',
    title: 'Customer Success (Onboarding)',
    desc: 'Guia novos clientes, previne churn e monitora uso.',
  },
  {
    id: 'support',
    title: 'Suporte Técnico N2 / Soluções',
    desc: 'Resolve dúvidas técnicas e integrações de API.',
  },
  {
    id: 'executive',
    title: 'Copiloto Comercial Executivo',
    desc: 'Prepara battlecards, simula negociações e cria propostas.',
  },
];
...
const MEMORIES = [
  {
    id: 'pgvector-rag',
    title: 'pgvector + RAG Vetorial (Recomendado)',
    desc: 'Acesso instantâneo a playbooks, produtos e histórico do CRM.',
  },
  {
    id: 'redis-session',
    title: 'Redis Memory (Sessão Curta)',
    desc: 'Mantém o contexto durante a conversa ativa do lead.',
  },
  {
    id: 'crm-history',
    title: 'CRM Full Context Window',
    desc: 'Injeta últimas 20 interações do lead no prompt.',
  },
  {
    id: 'ephemeral',
    title: 'Sem Memória (Stateless)',
    desc: 'Respostas isoladas ultrarrápidas sem histórico.',
  },
];
...
export function SuperagentCreator() {
  ...
  const [activeTabOutput, setActiveTabOutput] = useState<
    'prompt' | 'json' | 'python' | 'powershell'
  >('prompt');
  ...
  const [activeDropdown, setActiveDropdown] = useState<
    'provider' | 'model' | 'role' | 'temp' | 'memory' | null
  >(null);
    ...
    try {
        ...
        '/api/intelligence/studio',
        {
          ...
          inputs: {
            ...
            tools: selectedTools.map(
              (toolId) => TOOLS_LIST.find((item) => item.id === toolId)?.name || toolId,
            ),
          },
        },
        { timeoutMs: 90_000 },
      ...
      setError(
        generationError instanceof Error
          ...
          : 'Não foi possível gerar o projeto do agente.',
      );
    } finally {
      setGenerating(false);
    }
  ...
  return (
    <div className="space-y-8">
      ...
      >
        ...
        <div
          className={`absolute top-0 right-0 w-[600px] h-[600px] ${accent.blobA} rounded-full blur-[150px] pointer-events-none -mt-40 -mr-40`}
        ></div>
        <div
          className={`absolute bottom-0 left-0 w-[500px] h-[500px] ${accent.blobB} rounded-full blur-[120px] pointer-events-none -mb-40 -ml-40`}
        ></div>
        ...
        <div className="relative z-10 flex flex-col items-center text-center mb-10">
          ...
          <div
            className={`inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-surface-2 border border-line ${accent.text} mb-4 backdrop-blur-md`}
          >
            <Network size={14} />
          ...
          <p className="text-sm text-ink-2 max-w-2xl leading-relaxed">
            Configure e versione agentes autônomos com motores neurais avançados. Defina provedor,
            modelo, temperatura, memória RAG e ferramentas executáveis.
          </p>
        </div>
        ...
        <div className="relative z-10 grid grid-cols-1 md:grid-cols-2 gap-6 max-w-5xl mx-auto mb-8">
          ...
          <div className="bg-surface-2 border border-line rounded-2xl p-6 backdrop-blur-md">
            <p
              className={`flex items-center gap-2 text-[10px] tracking-widest font-black uppercase mb-3 ${accent.textSoft}`}
            >
          ...
          <div className="bg-surface-2 border border-line rounded-2xl p-6 backdrop-blur-md relative">
            ...
            <p className="mt-2 text-[10px] text-ink-2">
              Alvo do projeto; a implantação exige a credencial desse provedor.
            </p>
            <AnimatePresence>
          ...
          <div className="bg-surface-2 border border-line rounded-2xl p-6 backdrop-blur-md relative">
            ...
            <p className="mt-2 text-[10px] text-ink-2">
              O Groq configurado gera os artefatos; este modelo será usado pelo agente implantado.
            </p>
            <AnimatePresence>
              ...
              {activeDropdown === 'temp' && (
                ...
                >
                  {TEMPERATURES.map((t) => (
                    ...
                    >
                      {t.label}{' '}
                      {temperature === t.value && <Check size={16} className="text-amber-400" />}
                    </button>
                  ))}
                </motion.div>
              )}
                ...
                >
                  {MEMORIES.map((m) => (
                    ...
                    >
                      <div className="flex justify-between items-center font-bold text-ink w-full">
                        {m.title}{' '}
                        {memory === m.id && <Check size={16} className="text-emerald-400" />}
                      </div>
        ...
        <div className="relative z-10 max-w-5xl mx-auto mb-10 bg-surface-2 border border-line rounded-2xl p-6 backdrop-blur-md">
          <p
            className={`flex items-center gap-2 text-[10px] tracking-widest font-black uppercase mb-4 ${accent.textSoft}`}
          >
