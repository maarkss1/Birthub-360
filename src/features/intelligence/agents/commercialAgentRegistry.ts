...
export const COMMERCIAL_AGENT_REGISTRY: CommercialAgentDefinition[] = [
  {
    ...
    capabilities: ['icp_fit', 'account_research', 'market_signals', 'lead_prioritization', 'handoff_to_bdr'],
  ...
  },
  {
    ...
    capabilities: ['outbound_strategy', 'first_touch', 'account_mapping', 'icebreaker', 'cadence_start'],
  ...
  },
  {
    ...
    mission: 'Qualificar leads com evidência e converter interesse em oportunidade realmente trabalhável.',
    capabilities: ['lead_qualification', 'meeting_preparation', 'qualification_matrix', 'crm_update_proposal', 'handoff_to_closer'],
    handoffs: ['closer-sales', 'coordinator-commercial', 'bitrix-guardian'],
    bindings: ['src/features/intelligence/agents/sdrQualification.agent.ts (SDRQualificationAgent, já em produção)'],
    agentModule: null,
  },
  {
    id: 'closer-sales',
    ...
    risk: 'LOW',
    requiresApproval: true,
    mission: 'Aumentar qualidade de negociação e fechamento protegendo margem e probabilidade real.',
    capabilities: ['deal_strategy', 'objection_handling', 'proposal_strategy', 'decision_committee', 'close_plan', 'risk_assessment'],
    handoffs: ['contract-signature', 'coordinator-commercial', 'revenue-intelligence', 'bitrix-guardian'],
    bindings: ['src/features/intelligence/agents/closer.agent.ts (CloserAgent, já em produção — nunca move deal para ganho, exige evento verificável)'],
    agentModule: null,
  },
  {
    ...
    risk: 'MEDIUM',
    requiresApproval: true,
    mission: 'Narrar o ritmo diário do time a partir de alertas, aging e indicadores já calculados, apontando gargalos e próxima ação.',
    capabilities: ['daily_control', 'leading_indicators', 'sla_monitoring', 'next_action_control', 'team_coaching', 'agent_orchestration'],
  ...
  },
  {
    ...
    capabilities: ['forecast_review', 'pipeline_review', 'team_performance', 'bottleneck_analysis', 'coaching_priorities', 'goal_management'],
  ...
  },
  {
    ...
    risk: 'LOW',
    requiresApproval: false,
    mission: 'Responder objetivamente se a máquina comercial sustenta a meta, traduzindo operação em decisão executiva.',
    capabilities: ['executive_summary', 'scenario_analysis', 'risk_prioritization', '90d_outlook', 'strategic_decisions'],
    handoffs: ['manager-commercial', 'revenue-intelligence', 'billing-revenue', 'churn-retention', 'contract-signature'],
    bindings: [
  ...
  },
  {
    ...
    risk: 'MEDIUM',
    requiresApproval: true,
    mission: 'Reconciliar vendido x faturado quando houver fonte confiável — nunca fabricar o faturado.',
    capabilities: ['sold_vs_billed', 'mrr_reconciliation', 'billing_gap', 'revenue_trend', 'revenue_alerts'],
  ...
  },
  {
    ...
    risk: 'MEDIUM',
    requiresApproval: true,
    mission: 'Identificar risco de cancelamento e propor plano de retenção com evidência, sem recalcular o motor já existente.',
    capabilities: ['churn_risk', 'health_score', 'retention_plan', 'revenue_at_risk', 'account_priority'],
    handoffs: ['manager-commercial', 'executive-director', 'billing-revenue', 'coordinator-commercial'],
    bindings: [
  ...
  },
  {
    ...
    risk: 'MEDIUM',
    requiresApproval: true,
    mission: 'Avaliar prontidão de um contrato e narrar o status real de assinatura, sem nunca assinar ou enviar autonomamente.',
    capabilities: ['contract_readiness', 'data_validation', 'signature_status', 'contract_blockers', 'handoff_to_billing'],
  ...
  },
  {
    ...
    risk: 'MEDIUM',
    requiresApproval: true,
    mission: 'Diagnosticar a saúde da sincronização Bitrix x Central a partir de dados reais — nunca grava nada (caminho de leitura, sem writeback).',
    capabilities: ['bitrix_sync_health', 'crm_hygiene', 'stage_validation', 'duplicate_detection', 'activity_quality', 'bitrix_note'],
    handoffs: ['coordinator-commercial', 'manager-commercial', 'revenue-intelligence', 'sdr-qualification', 'closer-sales'],
    bindings: [
  ...
  },
  {
    ...
    risk: 'LOW',
    requiresApproval: false,
    mission: 'Traduzir métricas do cockpit comercial em previsibilidade, separando Pipeline de Forecast.',
    capabilities: [
      'new_mrr', 'closed_mrr', 'attainment', 'commit', 'best_case', 'forecast', 'forecast_gap',
      'pipeline_total', 'eligible_pipeline', 'pipeline_coverage', 'win_rate', 'sales_cycle', 'aging',
      'loss_reasons', 'leading_indicators', 'forecast_accuracy',
    ],
    handoffs: ['manager-commercial', 'executive-director', 'coordinator-commercial', 'bitrix-guardian', 'billing-revenue'],
    bindings: [
...
export function getCommercialAgentDefinition(id: CommercialAgentId): CommercialAgentDefinition | undefined {
