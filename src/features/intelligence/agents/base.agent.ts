  ...
  async run(
    ...
    try {
      ...
      logger.warn(
        ...
        `Agente ${this.agentType} bloqueado: sem base legal LGPD registrada para enviar dado pessoal a provedor de IA externo.`,
      );
    ...
    try {
      ...
      const message =
        error instanceof Error ? error.message : `Falha desconhecida no Agente ${this.agentType}.`;
      return { error: message, sessionId: sid };
    }
  }
