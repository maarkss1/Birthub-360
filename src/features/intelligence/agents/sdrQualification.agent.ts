...
<processo_pensamento>
Antes de invocar 'update_lead_qualification' e gerar a resposta final, reflita internamente.
Utilize a tag <thought> para estruturar sua análise:
1. Avaliação do Fit Score e perfil do ICP.
2. Identificação de decisores e objeções prováveis.
3. Decisão de avanço, arquivamento ou nutrição e o porquê.
</processo_pensamento>
...
Trabalhe silenciosamente e não faça perguntas ao usuário. Aja até completar a tarefa chamando as ferramentas apropriadas. ${SWARM_OUTPUT_CONTRACT}
  ...
  async run(leadId: string, sessionId?: string, instruction?: string) {
    ...
    try {
      ...
      logger.warn(
        ...
        'SDR Agent bloqueado: sem base legal LGPD registrada para enviar dado pessoal a provedor de IA externo.',
      );
    ...
    const rehydrate = (text: string) =>
      contactName ? rehydratePii(text, [{ token: '[NOME_DO_CONTATO]', value: contactName }]) : text;
    ...
    try {
      ...
      // execução assíncrona).
      logger.error(
        { err: error, leadId, sessionId: sid },
        'Failed to persist SDR agent memory after successful run',
      );
      return {
        success: false,
        error: 'Qualificação concluída, mas falha ao persistir o resultado.',
      };
    }

    const lastMessage = messages[messages.length - 1];
    const rawDetailedContent = lastMessage?.content
      ? lastMessage.content.toString()
      : 'Análise concluída silenciosamente.';
