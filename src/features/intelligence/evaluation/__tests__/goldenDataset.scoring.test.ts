...
import { evaluateGoldenDataset, scoreGoldenCase } from '../goldenDataset.scoring';
import { loadGoldenDataset } from '../goldenDataset.service';
import type { GoldenCase, LeadQualificationGoldenCase } from '../goldenDataset.types';
  ...
  switch (goldenCase.category) {
    ...
    case 'roleplay':
      return {
        ...
        objectionHandling:
          (goldenCase.expected.minObjectionHandling + goldenCase.expected.maxObjectionHandling) / 2,
      };
    ...
    case 'rag':
      return {
        sourceReferences: goldenCase.expected.expectedCitedChunkIds.map((chunkId: string) => ({
          chunkId,
        })),
      };
    case 'tool_use':
...
describe('Golden Dataset automatic scoring gate', () => {
  ...
  it('usa juiz semântico e bloqueia risco alto de alucinação', async () => {
    ...
    const result = await scoreGoldenCase(goldenCase, idealOutput(goldenCase), {
      judge: async () => ({
        semanticScore: 1,
        factualityScore: 1,
