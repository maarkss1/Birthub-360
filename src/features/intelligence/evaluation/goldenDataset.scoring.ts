...
const tokens = (value: unknown): Set<string> =>
  new Set(normalize(value).match(/[a-z0-9]{3,}/g) ?? []);
...
function deterministicScore(
  ...
  switch (goldenCase.category) {
    case 'lead_qualification': {
      ...
      if (!rangeOk)
        reasons.push(
          `score fora de ${goldenCase.expected.minScore}-${goldenCase.expected.maxScore}`,
        );
    ...
    case 'cold_email': {
      ...
      const keywordCoverage =
        goldenCase.expected.mustMentionKeywords.filter((keyword) =>
          text.includes(normalize(keyword)),
        ).length / goldenCase.expected.mustMentionKeywords.length;
    ...
    case 'objection_handling': {
      ...
      const strategyCoverage =
        goldenCase.expected.mustAddressStrategies.filter((strategy) =>
          text.includes(normalize(strategy)),
        ).length / goldenCase.expected.mustAddressStrategies.length;
      const similarity = jaccard(text, goldenCase.expected.referenceAnswer);
    ...
    case 'roleplay': {
      ...
      const objectionHandlingOk =
        Number.isFinite(objectionHandling) &&
...
export async function scoreGoldenCase(
  ...
  if (
    judge &&
    ['cold_email', 'objection_handling', 'summary', 'rag'].includes(goldenCase.category)
  ) {
  ...
  const finalScore = clamp01(
    judgeQuality == null ? deterministic.score : deterministic.score * 0.6 + judgeQuality * 0.4,
  );
  const hallucinationOk =
    !semantic || semantic.hallucinationRisk <= thresholds.maxHallucinationRisk;
...
export async function evaluateGoldenDataset(
  ...
  const passed =
    overallScore >= thresholds.overallMinimum &&
