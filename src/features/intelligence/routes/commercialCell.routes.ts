 ...
 * falha 1000). Registrar uma exceção em `HOTSPOT_EXCEPTIONS.md` seria esconder o sintoma — a
 * Célula Comercial já era um bloco coeso e autocontido dentro de `agent.routes.ts`, com seus
 * próprios contratos estruturais de DI, então virou módulo próprio. Montado como sub-router em
 * `agent.routes.ts`: TODAS as URLs públicas continuam idênticas
 * (`/api/agent/commercial-cell/...`), nenhuma rota mudou de caminho, método ou resposta.
 *
 * Padrão de todas as rotas deste arquivo (estabelecido na onda 43 pelos 3 primeiros agentes e
 * seguido pelos 5 conectados na onda 6):
 *   1. o motor real é resolvido por DI container com um contrato estrutural LOCAL — nunca import
 *      direto de outra feature (`no-cross-feature-imports`, `npm run test:architecture`);
 *   2. `organizationId` vem sempre de `req.user`, nunca do body/querystring;
...
// sub-router precisa aplicá-lo por rota, exatamente como antes da extração.
...
export const commercialCellRoutes = router;
