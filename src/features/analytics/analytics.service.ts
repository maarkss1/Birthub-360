...
export class AnalyticsService {
  async overview(organizationId: string, now = new Date()): Promise<OverviewMetrics> {
    ...
    const [
      ...
      prisma.lead.count({
        where: { ...scope, status: { in: CLOSED_LOST_STATUSES }, closedAt: { gte: monthStart } },
      }),
      prisma.lead.count({ where: { ...scope, status: WON } }),
      prisma.lead.aggregate({
        where: {
          ...scope,
          status: { notIn: [WON, ...CLOSED_LOST_STATUSES] },
          score: { not: null },
        },
        _avg: { score: true },
      }),
  ...
  async monthly(organizationId: string, months = 6, now = new Date()): Promise<MonthlyPoint[]> {
    ...
    const [created, closed] = await Promise.all([
      ...
      prisma.lead.findMany({
        where: {
          ...scope,
          status: { in: [WON, ...CLOSED_LOST_STATUSES] },
          closedAt: { gte: since },
        },
  ...
  async dashboard(
    ...
    const [
      ...
      prisma.lead.groupBy({
        by: ['lossReason'],
        where: { ...scope, status: { in: CLOSED_LOST_STATUSES } },
        _count: { _all: true },
      }),
    ]);
  ...
  private async performanceReport(organizationId: string, scope: any) {
    ...
    const qualifiedStats = await prisma.lead.groupBy({
      ...
      where: {
        ...scope,
        status: { notIn: ['Lead_Recebido', 'Cadencia_Iniciada', 'Lead_Desqualificado'] },
      },
      _count: { _all: true },
    });
