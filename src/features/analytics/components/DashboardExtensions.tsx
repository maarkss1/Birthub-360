import { SERIES } from '../../../shared/constants/chartPalette.js';
...
export function HeatmapWidget({ data }: { data: AnalyticsDashboard['callHeatmap'] }) {
  ...
  data.forEach((d) => {
    heatGrid[`${d.dayOfWeek}-${d.hour}`] = d.count;
  });
...
export function AgentPerformanceWidget({
  ...
  data: AnalyticsDashboard['performanceReport'];
}) {
  if (!data || data.length === 0) {
    return (
      <div className="text-center py-8 text-ink-2 text-sm">👥 Sem dados de performance ainda</div>
    );
  }
        ...
        <div key={i} className="flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center justify-between mb-1">
              ...
              <span className="text-xs font-bold text-danger-active dark:text-danger ml-2 shrink-0">
                {d.count}
              </span>
            </div>
...
export function TmqTile({ value }: { value: number | null }) {
  return (
    <div className="flex flex-col items-center justify-center py-2 text-center">
      ...
      {value !== null && value <= 3 && (
        <span className="text-[10px] text-success-active dark:text-success font-semibold mt-1">
          ✓ Ótimo
        </span>
      )}
      {value !== null && value > 7 && (
        <span className="text-[10px] text-danger-active dark:text-danger font-semibold mt-1">
          ⚠ Acima da meta
        </span>
      )}
    </div>
