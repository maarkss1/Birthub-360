...
export class LivingPlaybookController {
  ...
  /** Anuncia uma sugestão aprovada pro time inteiro (in-app). */
  broadcast = async (req: Request, res: Response, next: NextFunction) => {
    try {
      const { organizationId: orgId } = (req as AuthRequest).user;
      const { sellerName, segment, patternTitle, suggestedScript } = req.body ?? {};
      if (
      ...
      const notification = await broadcastWinningPattern(orgId, {
        sellerName,
        segment,
        patternTitle,
        suggestedScript,
