...
export function DesignLabPage({
  ...
  section: 'command-language' | 'components-v2';
}) {
  return (
    <iframe
      title={section === 'command-language' ? 'Birthub Command Language — Design Lab' : 'Birthub Components V2 — Design Lab'}
      src={`/design-lab/assets/${section}.html`}
      allow="microphone 'self'"
      style={{ display: 'block', width: '100%', height: '100dvh', border: 0, background: '#0c1019' }}
    />
  );
}
