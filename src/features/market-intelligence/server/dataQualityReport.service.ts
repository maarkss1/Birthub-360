...
function buildSnapshotFreshness(input: DataQualityReportRawInput): SnapshotFreshnessSection {
  ...
  const ageDaysList = input.latestSnapshots.map(
    (row) => (nowMs - row.generatedAt.getTime()) / 86_400_000,
  );
  const averageAgeDays = round2(
    ageDaysList.reduce((sum, age) => sum + age, 0) / ageDaysList.length,
  );
...
function buildDecisionMakerVerification(
  input: DataQualityReportRawInput,
): DecisionMakerVerificationSection {
...
function buildRelationshipVerification(
  input: DataQualityReportRawInput,
): RelationshipVerificationSection {
  ...
  const verifiedCount =
    input.relationshipByStatus.find((group) => group.key === 'Verified')?.count ?? 0;
  return {
    available: true,
    reason: null,
...
export async function fetchDataQualityReportInputs(
  ...
  const [
    ...
    db.intelligenceEvidence.groupBy({
      by: ['evidenceType'],
      where: { organizationId },
      _count: { _all: true },
    }),
    ...
    db.economicRelationship.groupBy({
      by: ['status'],
      where: { organizationId },
      _count: { _all: true },
    }),
  ...
  return {
    ...
    totalAccounts,
    latestSnapshots,
    evidenceByType: evidenceByTypeRaw.map((row) => ({
      key: row.evidenceType,
    ...
    decisionMakerTotal,
    decisionMakerVerifiedCount,
    decisionMakerByStatus: decisionMakerByStatusRaw.map((row) => ({
      key: row.status,
      count: row._count._all,
    })),
    relationshipTotal,
    relationshipByStatus: relationshipByStatusRaw.map((row) => ({
      key: row.status,
      count: row._count._all,
    })),
  };
