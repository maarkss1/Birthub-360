...
): Record<string, { value: Prisma.JsonValue; evidenceType: 'FACT'; source: string }> {
...
export class AccountIntelligenceService implements AccountIntelligenceServiceContract {
  ...
  async getIntelligence(accountId: string) {
    ...
    const [
      ...
      this.db.accountSignal.count({
        where: { companyId: accountId, organizationId: this.organizationId },
      }),
      this.db.decisionMaker.count({
        where: { companyId: accountId, organizationId: this.organizationId },
      }),
      ...
      this.db.accountRecommendation.count({
        where: { companyId: accountId, organizationId: this.organizationId },
      }),
      this.db.intelligenceEvidence.count({
        where: { companyId: accountId, organizationId: this.organizationId },
      }),
    ]);
  ...
  async refresh(accountId: string): Promise<RefreshResult> {
    ...
    const status =
      company.enrichmentStatus === 'Falhou' || Object.keys(facts).length < 5
        ? 'Partial'
        : 'Complete';
  ...
  async listEvidence(accountId: string, query: EvidenceQuery): Promise<PaginatedResult<unknown>> {
    ...
    const items = rows.map((row) => ({
      ...row,
      value: row.value === null ? null : sanitizeJson(row.value),
    }));
    return toPaginated(items, total, query);
  }
