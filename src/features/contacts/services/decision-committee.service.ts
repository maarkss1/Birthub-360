  ...
  async mapCommittee(
    ...
    if (!contacts || contacts.length === 0) {
      return {
        members: [],
        primaryTarget: '',
        recommendations: ['Nenhum contato fornecido para análise.'],
      };
    }
  ...
  "recommendations": [
    ...
    try {
      const response = await model.invoke([
        ...
        new HumanMessage(
          `Contexto da Empresa: ${companyContext || 'Empresa de Transporte e Logística'}\n\nContatos:\n${JSON.stringify(contacts, null, 2)}`,
        ),
      ]);
      ...
      return {
        ...
        recommendations: [
          'Análise de IA temporariamente indisponível. Classificação de contingência aplicada.',
        ],
      };
    }
  }
