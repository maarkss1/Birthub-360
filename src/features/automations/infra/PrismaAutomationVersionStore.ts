  ...
  async record(input: AutomationVersionInput): Promise<void> {
    ...
    await prisma.automationVersion.create({
      data: {
        ...
        trigger: toPrismaAutomationTrigger(
          input.trigger,
        ) as Prisma.AutomationVersionCreateInput['trigger'],
        conditions: (input.conditions ?? undefined) as Prisma.InputJsonValue | undefined,
        action: toPrismaAutomationAction(
          input.action,
        ) as Prisma.AutomationVersionCreateInput['action'],
