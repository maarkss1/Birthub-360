...
export async function runColdLeadsScan(): Promise<{
  runId: string;
  ...
  try {
    ...
    if (organizationIds.length === 0) {
      logger.info(
        { runId },
        'Cold leads scan: nenhuma organização habilitada para o piloto automático de IA.',
      );
      return { runId, organizations: 0, scanned: 0, failures: 0 };
    }
    ...
    for (const organizationId of organizationIds) {
      ...
      const failures = await requestContext.run({ tenantId: organizationId }, async () => {
        ...
        return withConcurrency(coldLeads, SEARCH_CONCURRENCY, async (lead) => {
          ...
          logger.info(
            { runId, organizationId, leadId: lead.id, insightsCount: insights.length },
            'RAG insights for cold lead',
          );
        });
      });
