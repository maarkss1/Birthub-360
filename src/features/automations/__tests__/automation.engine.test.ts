...
describe('renderTemplate', () => {
  it('substitui o placeholder pelo valor do evento', () => {
    expect(renderTemplate('Lead {{status}} atualizado', { status: 'Proposta' })).toBe(
      'Lead Proposta atualizado',
    );
  });
...
describe('matchesConditions — operador numérico (regras de estagnação)', () => {
  it('gte: casa quando o valor real é maior ou igual ao limiar', () => {
    expect(
      matchesConditions({ daysSinceLastInteraction: { gte: 3 } }, { daysSinceLastInteraction: 3 }),
    ).toBe(true);
    expect(
      matchesConditions({ daysSinceLastInteraction: { gte: 3 } }, { daysSinceLastInteraction: 10 }),
    ).toBe(true);
    expect(
      matchesConditions({ daysSinceLastInteraction: { gte: 3 } }, { daysSinceLastInteraction: 2 }),
    ).toBe(false);
  });
  ...
  it('não casa quando o campo derivado não está presente no evento — impede que um evento em tempo real dispare uma regra de estagnação por engano', () => {
    ...
    expect(
      matchesConditions({ daysSinceLastInteraction: { gte: 3 } }, { status: 'Proposta Enviada' }),
    ).toBe(false);
  });
  ...
  it('combina operador numérico com igualdade simples no mesmo objeto de condições (AND)', () => {
    ...
    expect(
      matchesConditions(conditions, { status: 'Proposta Enviada', daysSinceLastInteraction: 5 }),
    ).toBe(true);
    expect(
      matchesConditions(conditions, { status: 'Outra Etapa', daysSinceLastInteraction: 5 }),
    ).toBe(false);
  });
