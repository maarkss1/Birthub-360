...
export const automationsApi = {
  ...
  update: (id: string, patch: Partial<AutomationDraft>) =>
    api.put<Automation>(`/api/automations/${id}`, patch),
...
export function describeAutomation(a: Automation): string {
  const condicoes = a.conditions
    ? Object.entries(a.conditions).filter(([, v]) => v != null && v !== '')
    : [];
  const filtro =
    condicoes.length > 0
      ? ` (${condicoes
          .map(([k, v]) => {
            if (isOperatorValue(v)) {
              const [operator, threshold] = Object.entries(v)[0] as [
                keyof AutomationConditionOperator,
                number,
              ];
