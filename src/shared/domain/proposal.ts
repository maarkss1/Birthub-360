...
export function computeLineItemTotal(
  item: Pick<ProposalLineItem, 'quantity' | 'unitPrice' | 'discountPercent' | 'taxPercent'>,
): number {
...
export function computeSnapshotTotals(lineItems: ProposalLineItem[]): {
  subtotal: number;
  total: number;
} {
  const subtotal =
    Math.round(lineItems.reduce((sum, item) => sum + item.quantity * item.unitPrice, 0) * 100) /
    100;
  const total =
    Math.round(lineItems.reduce((sum, item) => sum + computeLineItemTotal(item), 0) * 100) / 100;
