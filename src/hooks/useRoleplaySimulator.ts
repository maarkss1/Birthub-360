...
export function useRoleplaySimulator(
  ...
  const [roleplayScore, setRoleplayScore] = useState<{
    clarity: number;
  ...
  const startRoleplay = () => {
    ...
    const randomObj =
      brandObjs.length > 0
        ? brandObjs[
            Math.floor(
              Number(`0.${crypto.getRandomValues(new Uint32Array(1))[0]}`) * brandObjs.length,
            )
          ]
        : {
            segment: 'sua operação',
            persona: 'comprador',
            objectionText: 'ainda não avaliamos uma mudança de fornecedor.',
          };
