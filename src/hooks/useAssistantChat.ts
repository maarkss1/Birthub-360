...
export function useAssistantChat(
  ...
  useEffect(() => {
    ...
    api
      .get<AssistantHistoryMessage[]>(`/api/intelligence/chatbook/history?brand=${selectedBrand}`)
      .then((history) => {
        if (cancelled) return;
        if (!history.length) {
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    const matchedObjection = objections.find(
      (o) =>
    ...
    const matchedQual = qualifications.find(
      (q) =>
    ...
    setMessages((prev) => [
      ...prev,
      { id: botMsgId, sender: 'bot', text: '', timestamp: timestamp(), source: searchMode },
    ]);
    ...
    try {
      const response = await fetch(
        '/api/intelligence/studio/stream',
        sseRequestInit(
          {
            kind: 'assistant',
            brand: {
      ...
      await readSseStream(response, (evt) => {
        if (evt.event === 'delta') {
          ...
          setMessages((prev) =>
            prev.map((m) => (m.id === botMsgId ? { ...m, text: m.text + delta } : m)),
          );
