...
export function useBitrixIntegration() {
  ...
  const fetchBitrixConnections = useCallback(async () => {
    try {
      ...
      if (data.success) {
        ...
        setSelectedBitrixConnectionId((prev) =>
  ...
  const handleBitrixConnect = async () => {
    ...
    try {
      const res = await fetch('/api/bitrix/connect', {
        ...
        body: JSON.stringify({
          webhookUrl: bitrixWebhookInput.trim(),
          label: bitrixLabelInput.trim() || undefined,
        }),
      });
      const data = await res.json();
      if (data.success) {
  ...
  const handleGenerateWebhookSecret = async (
    connectionId: string,
  ): Promise<{ webhookSecret: string; webhookReceiverUrl: string } | null> => {
    setBitrixLoading(true);
    try {
      const res = await fetch(`/api/bitrix/connections/${connectionId}/webhook-secret`, {
        method: 'POST',
      });
      const data = await res.json();
      if (!data.success) {
  ...
  const handleToggleInboundEvents = async (connectionId: string, enabled: boolean) => {
    ...
    try {
      ...
      if (!data.success) {
        ...
        toast.success(
          enabled
            ? 'Recebimento de eventos do Bitrix24 ativado.'
            : 'Recebimento de eventos do Bitrix24 desativado.',
        );
      }
      await fetchBitrixConnections();
    } catch (error) {
