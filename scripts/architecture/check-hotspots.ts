...
function main(): void {
  ...
  if (okExceptions.length > 0) {
    console.log(
      `ℹ️  ${okExceptions.length} arquivo(s) cobertos por exceção ativa em HOTSPOT_EXCEPTIONS.md:`,
    );
    for (const v of okExceptions) {
      if (v.status === 'ok-exception') {
        console.log(
          `   - ${v.file}: ${v.lines} linhas (limite excepcional: ${v.exception.limitLines}, dono: ${v.exception.owner}, reavaliar até: ${v.exception.reavaliarAte})`,
        );
      }
    }
  }
  ...
  if (staleExceptions.length > 0) {
    console.warn(
      `⚠️  ${staleExceptions.length} exceção(ões) em HOTSPOT_EXCEPTIONS.md não correspondem a nenhum arquivo que hoje precise dela (arquivo removido ou encolhido) — considere limpar o doc:`,
    );
