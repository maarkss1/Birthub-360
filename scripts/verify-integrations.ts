...
async function verifyApollo(): Promise<CheckResult> {
  if (!getPaidProspectingKey('APOLLO_API_KEY')) {
    return SKIPPED(
      'APOLLO_API_KEY ausente ou PROSPECTING_PROVIDER_MODE != hybrid — provedor pago desativado por padrão',
    );
  }
...
async function verifyHunter(): Promise<CheckResult> {
  ...
  if (!apiKey) {
    return SKIPPED(
      'HUNTER_API_KEY ausente ou PROSPECTING_PROVIDER_MODE != hybrid — provedor pago desativado por padrão',
    );
  }
  try {
    ...
    const data = (await response.json()) as {
      data?: { plan_name?: string; requests?: { searches?: { available?: number } } };
    };
    return {
      ok: true,
...
async function verifyBitrix(): Promise<CheckResult> {
  ...
  try {
    ...
    if (!data || data.error)
      return { ok: false, detail: data?.error || 'resposta inesperada do portal' };
...
async function verifyLiteLLM(): Promise<CheckResult> {
  ...
  try {
    const response = await fetch(`${baseUrl.replace(/\/$/, '')}/health/liveliness`, {
      headers: process.env.LITELLM_KEY
        ? { Authorization: `Bearer ${process.env.LITELLM_KEY}` }
        : {},
    });
    return {
      ok: response.ok,
      detail: response.ok ? 'gateway respondendo' : `HTTP ${response.status}`,
    };
  } catch (error) {
    return { ok: false, detail: error instanceof Error ? error.message : 'falha de rede' };
  }
...
async function verifyLangfuse(): Promise<CheckResult> {
  ...
  if (!process.env.LANGFUSE_PUBLIC_KEY || !process.env.LANGFUSE_SECRET_KEY || !baseUrl) {
    return SKIPPED(
      'LANGFUSE_PUBLIC_KEY/SECRET_KEY/BASEURL ausentes — tracing de observabilidade desativado',
    );
  }
  try {
    ...
    return {
      ok: response.ok,
      detail: response.ok ? 'serviço saudável' : `HTTP ${response.status}`,
    };
  } catch (error) {
    return { ok: false, detail: error instanceof Error ? error.message : 'falha de rede' };
  }
