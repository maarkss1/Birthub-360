# Remediação Técnica — PROSPECTOR-BIRTH HUB 360
...
| `@xenova/transformers` / `sharp` (aninhado) | ~~alta~~ **CORRIGIDA** | `@xenova/transformers@2.17.2` (última versão publicada) bundlava `sharp@0.32.6` (vulnerável a CVEs libvips, corrigido só em `sharp≥0.35.0`) | Tecnicamente sim (roda server-side, `src/lib/ai/local-embeddings.ts`), mas **não explorável na prática** — ver coluna "Por quê" | O pipeline usado é exclusivamente `feature-extraction` de texto (`embedLocal`) — `sharp` só é acionado pelo `@xenova/transformers` em pré-processamento de imagem, caminho nunca exercitado por este código. Não havia upgrade direto do pacote (2.17.2 seguia sendo a última versão publicada, e o sucessor oficial `@huggingface/transformers@4.2.0` bundla `sharp@^0.34.5`, abaixo do piso seguro `0.35.0`) | **Corrigida via `overrides` do npm** (`package.json`, commit `8e283377`, 2026-08-13): `"@xenova/transformers": { "sharp": "^0.35.3" }` força a dependência aninhada para uma versão já corrigida, sem precisar de upgrade do pacote pai. Confirmado hoje (2026-09-19): `npm ls sharp` resolve `sharp@0.35.4` em todo lugar (deduped), e `npm audit` não reporta mais nenhum achado para `sharp`/`@xenova` — esta linha da tabela estava desatualizada em relação ao `package.json` real e foi corrigida agora |
...
- ~~**`@xenova/transformers`/`sharp`**: ver tabela de Segurança — sem fix seguro disponível a
  montante; risco aceito e documentado.~~ **Corrigida em 2026-08-13** (commit `8e283377`) via
  `overrides` no `package.json` — não era mais dívida residual quando esta seção foi escrita
  (12/09), só não tinha sido atualizada aqui. Ver tabela de Segurança acima.
...
?? REMEDIACAO_FINAL.md (este relatório)
```
